...
/** IRQ digital noise filter operation mode. */
typedef enum e_external_irq_filter_mode
{
    EXTERNAL_IRQ_FILTER_MODE_3_CYCLES = 0, ///< Digital filter using 3 consecutive level matches.
    EXTERNAL_IRQ_FILTER_MODE_4_CYCLES = 1, ///< Digital filter using 4 consecutive level matches.
} e_external_irq_filter_mode_t;
...
typedef struct st_icu_extended_cfg
{
    ...
    e_external_irq_filter_mode_t    filter_mode; ///< Digital filter sampling mode (number of consecutive level matches).
} icu_extended_cfg_t;
...
typedef struct st_icu_instance_ctrl
{
    ...
    IRQn_Type irq;                                              ///< ICU interrupt number
