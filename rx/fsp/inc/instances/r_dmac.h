...
/* Handle software trigger operation when DMAC_SOFTWARE_TRIGGER is selected as DMAC activation source */
#define DMAC_SOFTWARE_TRIGGER              ((IRQn_Type) - 1)
...
typedef struct st_dmac_extended_cfg
{
    ...
    /* Set to true to retain interrupt status flag and issues an interrupt at the end of transfer,
     * set to false to clear the interrupt status flag at the beginning of transfer. */
    bool interrupt_status_flag_retained;
} dmac_extended_cfg_t;
