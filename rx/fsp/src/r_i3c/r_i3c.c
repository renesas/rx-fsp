...
#define I3C_ICRCR_FIFO_FLUSH_Msk                     (R_I3C0_ICRCR_CQRST_Msk | R_I3C0_ICRCR_TBRST_Msk | \
                                                      R_I3C0_ICRCR_RBRST_Msk)
...
static uint32_t i3c_clock_frequency_get(void);
...
fsp_err_t R_I3C_Open (i3c_ctrl_t * const p_api_ctrl, i3c_cfg_t const * const p_cfg)
{
     ...
     */
    p_ctrl->p_reg->ICCR_b.ICE = 0;
    p_ctrl->p_reg->ICRCR      = 1U;

    /* The field will be cleared automatically upon reset completion (See "ICRCR : Reset Control Register" description in the I3C section of the relevant hardware manual). */
    FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->ICRCR, 0U);

    /* Set internal reset to one before setting I3C mode. */
    p_ctrl->p_reg->ICRCR = R_I3C0_ICRCR_ISRST_Msk;

    /* Set I3C mode. */
    p_ctrl->p_reg->ICMR = 0U;

    /* Clear internal reset. */
    p_ctrl->p_reg->ICRCR = 0;
...
fsp_err_t R_I3C_Enable (i3c_ctrl_t * const p_api_ctrl)
{
     ...
     */
    uint32_t icqbtcr = 6U << R_I3C0_ICQBTCR_IDSS_Pos;
     ...
     */
    uint32_t iccsier = R_I3C0_ICCSIER_RIE_Msk |
                       R_I3C0_ICCSIER_RQFIE_Msk |
                       R_I3C0_ICCSIER_SQFIE_Msk;
    ...
    if (I3C_DEVICE_TYPE_MAIN_MASTER == p_ctrl->p_cfg->device_type)
    {
        ...
        iccsier |= R_I3C0_ICCSIER_IQEFIE_Msk;

        /* Write the standard and extended bitrate settings. */
        p_ctrl->p_reg->ICSBR = p_extend->bitrate_settings.stdbr;
        p_ctrl->p_reg->ICEBR = p_extend->bitrate_settings.extbr;
    }
    else
#endif
    {
         ...
         * IBI transmit FIFO.
         */
        icqbtcr |= 1U << R_I3C0_ICQBTCR_IQTH_Pos;

        /* Baudrate registers are not used in slave mode. */
        p_ctrl->p_reg->ICSBR = 0;
        p_ctrl->p_reg->ICEBR = 0;
#endif
    }
     ...
     */
    p_ctrl->p_reg->ICQBTCR = icqbtcr;

    /* Set the Normal Transfer Data Buffer threshold for transmit and receive (The only supported configuration
     * is two entries). */
    p_ctrl->p_reg->ICDBTCR = 0;

    /* Set the Receive Status Queue Threshold to zero (Interrupt when there is one entry in the queue). */
    p_ctrl->p_reg->ICSQTCR = 0;

    /* Enable the Internal Error Status Flag. */
    p_ctrl->p_reg->ICISER = R_I3C0_ICISER_BERDE_Msk;

    /* Enable all Bus Status Flags. */
    p_ctrl->p_reg->ICSER = (R_I3C0_ICSER_STDE_Msk |
                            R_I3C0_ICSER_SPDE_Msk |
                            R_I3C0_ICSER_HDRXDE_Msk |
                            R_I3C0_ICSER_TMOE_Msk);

    /* Enable all Transfer Status Flags. */
    p_ctrl->p_reg->ICCSER = (R_I3C0_ICCSER_TDE_Msk |
                             R_I3C0_ICCSER_RDE_Msk |
                             R_I3C0_ICCSER_IQEFDE_Msk |
                             R_I3C0_ICCSER_CQEDE_Msk |
                             R_I3C0_ICCSER_RQFDE_Msk |
                             R_I3C0_ICCSER_DTADE_Msk |
                             R_I3C0_ICCSER_DTEDE_Msk |
                             R_I3C0_ICCSER_SQFDE_Msk);

    /* Enable the Internal Error Interrupt Requests. */
    p_ctrl->p_reg->ICISIER = R_I3C0_ICISIER_BERIE_Msk;

    /* Enable HDR exit pattern Interrupt Requests. */
    p_ctrl->p_reg->ICSIER = R_I3C0_ICSIER_HDRXIE_Msk | R_I3C0_ICSIER_TMOIE_Msk;

    /* Write settings to the Normal Transfer Interrupt Enable Register. */
    p_ctrl->p_reg->ICCSIER = iccsier;

#if BSP_FEATURE_I3C_HAS_HOT_JOIN == 1

    /* Write the Hot-Join Acknowledge settings. */
    p_ctrl->p_reg->ICCR_b.HJC = !p_extend->ibi_control.hot_join_acknowledge;
#endif

    /* Calculate the value of the IBI Notify Control Register. */
    uint32_t icincr = (uint32_t) (p_extend->ibi_control.notify_rejected_mastership_requests << R_I3C0_ICINCR_RCRRN_Pos);
    icincr |= (uint32_t) (p_extend->ibi_control.notify_rejected_interrupt_requests << R_I3C0_ICINCR_RTIRN_Pos);
#if BSP_FEATURE_I3C_HAS_HOT_JOIN == 1
    icincr |=
        (uint32_t) (p_extend->ibi_control.notify_rejected_hot_join_requests << R_I3C0_ICINCR_RHJRN_Pos);
#endif

    /* Write settings to the IBI Notify Control Register. */
    p_ctrl->p_reg->ICINCR = icincr;

    /* Calculate the value of the SCL Clock Stalling Control Register. */
    uint32_t icstcr = (uint32_t) (p_extend->bitrate_settings.clock_stalling.assigned_address_phase_enable <<
                                  R_I3C0_ICSTCR_AASE_Pos);

#if BSP_FEATURE_I3C_SUPPORT_TBSE == 1
    icstcr |= (uint32_t) (p_extend->bitrate_settings.clock_stalling.transition_phase_enable <<
                          R_I3C0_ICSTCR_TBSE_Pos);
#endif
    icstcr |= (uint32_t) (p_extend->bitrate_settings.clock_stalling.parity_phase_enable << R_I3C0_ICSTCR_PBSE_Pos);
    icstcr |= (uint32_t) (p_extend->bitrate_settings.clock_stalling.ack_phase_enable << R_I3C0_ICSTCR_APSE_Pos);
    icstcr |=
        (uint32_t) (p_extend->bitrate_settings.clock_stalling.clock_stalling_time << R_I3C0_ICSTCR_STT_Pos);

    /* Write clock stalling settings to the SCL CLock Stalling Control Register. */
    p_ctrl->p_reg->ICSTCR = icstcr;
    ...
    uint32_t ictevr =
        (uint32_t) (p_extend->slave_command_response_info.inband_interrupt_enable << R_I3C0_ICTEVR_ENINT_Pos);
    ictevr |= (uint32_t) (p_extend->slave_command_response_info.mastership_request_enable << R_I3C0_ICTEVR_ENCR_Pos);
 #if BSP_FEATURE_I3C_HAS_HOT_JOIN == 1
    ictevr |= (uint32_t) (p_extend->slave_command_response_info.hotjoin_request_enable << R_I3C0_ICTEVR_ENHJ_Pos);
 #endif

    /* Calculate the value of the CCC Max Data Speed Read Register. */
    uint32_t icmrsr = (uint32_t) p_extend->slave_command_response_info.read_data_rate << R_I3C0_ICMRSR_MSRDR_Pos;
    icmrsr |= (uint32_t) p_extend->slave_command_response_info.clock_data_turnaround << R_I3C0_ICMRSR_TSCO_Pos;

    /* Calculate the value of the CCC Max Data Speed Rurnaround Register. */
    uint32_t icmttr = p_extend->slave_command_response_info.read_turnaround_time << R_I3C0_ICMTTR_MRTT_Pos;
    icmttr |= (uint32_t) p_extend->slave_command_response_info.read_turnaround_time_enable << R_I3C0_ICMTTR_MRTTE_Pos;

    /* Calculate the value of the CCC Exchange Timing Support Information Register. */
    uint32_t ictsir =
        (uint32_t) (p_extend->slave_command_response_info.oscillator_frequency << R_I3C0_ICTSIR_FREQ_Pos) &
        R_I3C0_ICTSIR_FREQ_Msk;
    ictsir |= (uint32_t) (p_extend->slave_command_response_info.oscillator_inaccuracy << R_I3C0_ICTSIR_INAC_Pos) &
              R_I3C0_ICTSIR_INAC_Msk;

    uint32_t icmrlr = (uint32_t) p_extend->slave_command_response_info.read_length;
    icmrlr |= (uint32_t) p_extend->slave_command_response_info.ibi_payload_length << R_I3C0_ICMRLR_IBIPL_Pos;
    ...
    p_ctrl->p_reg->ICTEVR = ictevr;
    p_ctrl->p_reg->ICASR  = (1U << p_extend->slave_command_response_info.activity_state);
    p_ctrl->p_reg->ICMWLR = (uint32_t) p_extend->slave_command_response_info.write_length;
    p_ctrl->p_reg->ICMRLR = icmrlr;
    p_ctrl->p_reg->ICDSR  =
        (uint32_t) (p_extend->slave_command_response_info.activity_state << R_I3C0_ICDSR_CAS_Pos);
    p_ctrl->p_reg->ICMWSR = (uint32_t) p_extend->slave_command_response_info.write_data_rate;
    p_ctrl->p_reg->ICMRSR = icmrsr;
    p_ctrl->p_reg->ICMTTR = icmttr;
    p_ctrl->p_reg->ICTSIR = ictsir;
    ...
    /* Enable the I3C Bus. */
    p_ctrl->p_reg->ICCR |= R_I3C0_ICCR_ICE_Msk;

    /* Enable the command response status buffer full IRQ. */
    R_BSP_IrqCfgEnable(p_extend->resp_irq, (uint32_t) p_extend->ipl, p_ctrl);

    /* Enable the receive data buffer full IRQ. */
    R_BSP_IrqCfgEnable(p_extend->rx_irq, (uint32_t) p_extend->ipl, p_ctrl);

    /* Enable the transmit data buffer empty IRQ. */
    R_BSP_IrqCfgEnable(p_extend->tx_irq, (uint32_t) p_extend->ipl, p_ctrl);

#if I3C_CFG_SLAVE_SUPPORT

    /* Enable the receive status buffer full IRQ. */
    R_BSP_IrqCfgEnable(p_extend->rcv_irq, (uint32_t) p_extend->ipl, p_ctrl);
#endif

    /* Enable the IBI IRQ. */
    R_BSP_IrqCfgEnable(p_extend->ibi_irq, (uint32_t) p_extend->ipl, p_ctrl);

    /* Enable the Error and Event IRQ. */
    R_BSP_IrqCfgEnable(p_extend->eei_irq, (uint32_t) p_extend->eei_ipl, p_ctrl);
...
fsp_err_t R_I3C_DeviceCfgSet (i3c_ctrl_t * const p_api_ctrl, i3c_device_cfg_t const * const p_device_cfg)
{
    ...
    if (I3C_DEVICE_TYPE_MAIN_MASTER == p_ctrl->p_cfg->device_type)
    {
         ...
         * in the I3C section of the relevant hardware manual).
         */
        uint32_t iccar = (uint32_t) p_device_cfg->dynamic_address << R_I3C0_ICCAR_DADR_Pos;
        iccar               |= R_I3C0_ICCAR_DAV_Msk;
        p_ctrl->p_reg->ICCAR = iccar;
    }
    else
#endif
    {
        ...
        FSP_ERROR_RETURN(1U != p_device_cfg->slave_info.bcr_u.bcr_b.device_role, FSP_ERR_UNSUPPORTED);
        ...
        uint32_t icdar0 = (uint32_t) p_device_cfg->static_address & R_I3C0_ICDAR0_SADR_Msk;

        /* Configure the device dynamic address. */
        icdar0 |= (uint32_t) (i3c_address_parity(p_device_cfg->dynamic_address) << R_I3C0_ICDAR0_DADR_Pos) &
                  R_I3C0_ICDAR0_DADR_Msk;

        /* Configure the IBI payload setting based on the BCR register. */
        icdar0 |= (uint32_t) (p_device_cfg->slave_info.bcr_u.bcr_b.ibi_payload << R_I3C0_ICDAR0_IBIPL_Pos) &
                  R_I3C0_ICDAR0_IBIPL_Msk;

        /* Set the slave address to valid. */
        p_ctrl->p_reg->ICTCR_b.TA0DE = 1;

        /* Write settings to the Slave Device Address Table Register. */
        p_ctrl->p_reg->ICDAR0 = icdar0;

        /* Write the BCR and DCR settings to the Slave Device Characteristic Table Register. */
        uint32_t icdctr = (uint32_t) p_device_cfg->slave_info.dcr;
        icdctr               |= (uint32_t) p_device_cfg->slave_info.bcr_u.bcr << R_I3C0_ICDCTR_LIMIT_Pos;
        p_ctrl->p_reg->ICDCTR = icdctr;
                                      ...
                                      i3c_device_table_cfg_t const * const p_device_table_cfg)
{
    ...
    uint32_t volatile * p_ictdatrm_reg = NULL;
    uint32_t            ictdatrm       = 0;

    if (I3C_DEVICE_INDEX_EXTENDED_DEVICE == device_index)
    {
        p_ictdatrm_reg = &p_ctrl->p_reg->ICEDATR;
    }
    else
    {
        /* Compute the address of the DATBASn register. */
        uint32_t address_offset = (uint32_t) (&p_ctrl->p_reg->ICTDATR[1] - &p_ctrl->p_reg->ICTDATR[0]);
        p_ictdatrm_reg = (uint32_t *) (&p_ctrl->p_reg->ICTDATR[0] + device_index * address_offset);

        /* Configure the IBI settings for this device. */
        ictdatrm |= (uint32_t) (p_device_table_cfg->ibi_payload << R_I3C0_ICTDATR_IBIPL_Pos) &
                    R_I3C0_ICTDATR_IBIPL_Msk;
        ictdatrm |= (uint32_t) (!p_device_table_cfg->ibi_accept << R_I3C0_ICTDATR_TIRRJ_Pos) &
                    R_I3C0_ICTDATR_TIRRJ_Msk;
        ictdatrm |= (uint32_t) (!p_device_table_cfg->master_request_accept << R_I3C0_ICTDATR_CRRRJ_Pos) &
                    R_I3C0_ICTDATR_CRRRJ_Msk;
    }

    /* Configure the device static address. */
    ictdatrm |= (uint32_t) (p_device_table_cfg->static_address << R_I3C0_ICTDATR_SADR_Pos) &
                R_I3C0_ICTDATR_SADR_Msk;

    /* Configure the device dynamic address. */
    ictdatrm |=
        (uint32_t) (i3c_address_parity(p_device_table_cfg->dynamic_address) << R_I3C0_ICTDATR_DADR_Pos) &
        R_I3C0_ICTDATR_DADR_Msk;
    ...
    if (I3C_DEVICE_PROTOCOL_I2C == p_device_table_cfg->device_protocol)
    {
        ...
        ictdatrm |= R_I3C0_ICTDATR_TYPE_Msk;
    }

    /* Write to the ICTDATRm register. */
    *p_ictdatrm_reg = ictdatrm;
...
fsp_err_t R_I3C_SlaveStatusSet (i3c_ctrl_t * const p_api_ctrl, i3c_device_status_t status)
{
    ...
    uint32_t icdsr = p_ctrl->p_reg->ICDSR;
    icdsr &= ~(R_I3C0_ICDSR_PNDINT_Msk | R_I3C0_ICDSR_VRSV_Msk);

    /* Write the new Pending Interrupt and Vendor Reserved fields. */
    icdsr |= (uint32_t) status.pending_interrupt & R_I3C0_ICDSR_PNDINT_Msk;
    icdsr |= (uint32_t) (status.vendor_status << R_I3C0_ICDSR_VRSV_Pos) & R_I3C0_ICDSR_VRSV_Msk;

    p_ctrl->p_reg->ICDSR = icdsr;
 ...
 *                                        enabled when sending the GETACCCR command.
 **********************************************************************************************************************/
fsp_err_t R_I3C_CommandSend (i3c_ctrl_t * const p_api_ctrl, i3c_command_descriptor_t const * const p_command_descriptor)
{
    ...
    if (I3C_CCC_DIRECT_GETACCMST == p_command_descriptor->command_code)
    {
        ...
        p_ctrl->p_reg->ICCSIER_b.IQEFIE = 0;
    }
 #endif
     ...
     * the most significant word (See section "Command Descriptor" in the I3C Operation section of the relevant hardware manual).
     */
    p_ctrl->p_reg->ICCQR = cmd1;
    p_ctrl->p_reg->ICCQR = cmd2;
    ...
    if (!p_command_descriptor->rnw && (4 < p_command_descriptor->length))
    {
        ...
        if ((BSP_FEATURE_I3C_ICDR_DEPTH * sizeof(uint32_t)) < p_command_descriptor->length)
        {
            /* Enable the Write Buffer Empty IRQ. */
            p_ctrl->p_reg->ICCSIER_b.TIE = 1;
        }
    }
...
fsp_err_t R_I3C_Write (i3c_ctrl_t * const p_api_ctrl, uint8_t const * const p_data, uint32_t length, bool restart)
{
    ...
    {
 #if BSP_FEATURE_I3C_SUPPORT_IIC == 1

        /* In slave mode, ensure that a write transfer is not currently in progress. */
        FSP_ERROR_RETURN(0U == (p_ctrl->p_reg->ICMMR & R_I3C0_ICMMR_TRS_Msk), FSP_ERR_IN_USE);
 #endif
    }
    else
#endif
    ...
    if (I3C_INTERNAL_STATE_SLAVE_IDLE == p_ctrl->internal_state)
    {
        if (p_ctrl->write_buffer_descriptor.p_buffer != NULL)
        {
             ...
             * can be written.
             */
            p_ctrl->p_reg->ICRCR = R_I3C0_ICRCR_TBRST_Msk;
        ...
        p_ctrl->p_reg->ICTDLR = (length << 16U);
    }
#endif
    ...
    if ((length > 4) || (I3C_INTERNAL_STATE_SLAVE_IDLE == p_ctrl->internal_state))
    {
        ...
        /* If there is still data remaining in the transfer then it will be written in the Write Buffer Empty IRQ. */
        if ((BSP_FEATURE_I3C_ICDR_DEPTH * sizeof(uint32_t)) < length)
        {
            /* Enable the Write Buffer Empty IRQ. */
            p_ctrl->p_reg->ICCSIER_b.TIE = 1;
        }
    ...
    if (I3C_INTERNAL_STATE_MASTER_IDLE == p_ctrl->internal_state)
    {
         ...
         * the most significant word (See section "Command Descriptor" in the I3C Operation section of the relevant hardware manual).
         */
        uint32_t cmd1 = i3c_xfer_command_calculate(p_ctrl->device_index,
                                                   false,
                                                   (uint32_t) p_ctrl->device_bitrate_mode,
                                                   restart);
...
fsp_err_t R_I3C_Read (i3c_ctrl_t * const p_api_ctrl, uint8_t * const p_data, uint32_t length, bool restart)
{
    ...
    if (I3C_INTERNAL_STATE_MASTER_IDLE == p_ctrl->internal_state)
    {
         ...
         * the most significant word (See section "Command Descriptor" in the I3C Operation section of the relevant hardware manual).
         */
        p_ctrl->p_reg->ICCQR = i3c_xfer_command_calculate(p_ctrl->device_index,
                                                          true,
                                                          (uint32_t) p_ctrl->device_bitrate_mode,
                                                          restart);
        p_ctrl->p_reg->ICCQR = (length << I3C_CMD_DESC_XFER_LENGTH_Pos) & I3C_CMD_DESC_XFER_LENGTH_Msk;
                          ...
                          uint32_t              length)
{
    ...
    if ((I3C_IBI_TYPE_INTERRUPT == ibi_type) && (0 != (R_I3C0_ICDAR0_IBIPL_Msk & p_ctrl->p_reg->ICDAR0)))
    {
        FSP_ASSERT(NULL != p_data);
        FSP_ASSERT(0 != length);
    }
    ...
    if (I3C_IBI_TYPE_INTERRUPT == ibi_type)
    {
        command_descriptor |= I3C_CMD_DESC_XFER_RNW_Msk;
    }

 #if BSP_FEATURE_I3C_HAS_HOT_JOIN == 1
    else if (I3C_IBI_TYPE_HOT_JOIN == ibi_type)
    {
    ...
    if (0 < length)
    {
        ...
        p_ctrl->p_reg->ICIQR = i3c_next_data_word_calculate(
...
/*******************************************************************************************************************//**
 * Get the I3C functional clock frequency in Hz.
 *
 * @return                       I3C clock frequency in Hz.
 **********************************************************************************************************************/
static uint32_t i3c_clock_frequency_get (void)
{
#if BSP_FEATURE_BSP_HAS_I3C_CLOCK

    return R_BSP_SourceClockHzGet((fsp_priv_source_clock_t) R_SYSTEM->I3CCKCR_b.I3CCKSEL) /
           R_FSP_ClockDividerGet((uint32_t) R_SYSTEM->I3CCKDIVCR);
#else
    uint32_t pclka_frequency = (SystemCoreClock << R_SYSTEM->SCKDIVCR_b.ICK);
    pclka_frequency >>= R_SYSTEM->SCKDIVCR_b.PCKA;

    return pclka_frequency;
#endif
}
...
static inline void i3c_fifo_read (i3c_instance_ctrl_t * p_ctrl, uint32_t bytes)
{
    ...
    while (bytes_remaining > 0)
    {
        ...
        uint32_t rx_data = p_ctrl->p_reg->ICDR;

        /* Clear the Read Buffer Full status flag. */
        p_ctrl->p_reg->ICCSR_b.RDRF = 0;
                                          ...
                                          i3c_event_t                    buffer_full_event)
{
    ...
    p_buffer_descriptor->p_buffer[p_buffer_descriptor->count]      = (uint8_t) (data_word >> 0U);
    p_buffer_descriptor->p_buffer[p_buffer_descriptor->count + 1U] = (uint8_t) (data_word >> 8U);
    p_buffer_descriptor->p_buffer[p_buffer_descriptor->count + 2U] = (uint8_t) (data_word >> 16U);
    p_buffer_descriptor->p_buffer[p_buffer_descriptor->count + 3U] = (uint8_t) (data_word >> 24U);
...
static inline uint32_t i3c_read_bytes_remaining_calculate (i3c_instance_ctrl_t * p_ctrl, uint32_t data_length)
{
    ...
    if (p_ctrl->p_reg->ICDBSR_b.RBFL > 0)
    {
...
BSP_FORCE_INLINE static inline void i3c_fifo_write (i3c_instance_ctrl_t * p_ctrl)
{
    do
    {
        ...
        p_ctrl->p_reg->ICDR = p_ctrl->next_word;
    ...
    } while ((p_ctrl->p_reg->ICDBSR & UINT8_MAX));

    /* Clear the Transmit Buffer Empty status flag. */
    p_ctrl->p_reg->ICCSR_b.TDRE = 0;
}
    ...
    i3c_write_buffer_descriptor_t * p_buffer_descriptor)
{
    ...
    data_word = ((uint32_t) p_transfer_data[count] << 0U) |
                ((uint32_t) p_transfer_data[count + 1U] << 8U) |
                ((uint32_t) p_transfer_data[count + 2U] << 16U) |
                ((uint32_t) p_transfer_data[count + 3U] << 24U);
    count += 4;
#endif
...
void i3c_resp_isr (void)
{
    ...
    switch (internal_state)
    {
        ...
        case I3C_INTERNAL_STATE_MASTER_COMMAND_READ:
        {
            ...
            {
                if (0U == (p_ctrl->p_reg->ICMMR & R_I3C0_ICMMR_ACF_Msk))
                {
                    /* If the GETACCCR command was successful, transition the driver to slave mode. */
                    p_ctrl->internal_state = I3C_INTERNAL_STATE_SLAVE_IDLE;
    ...
    uint32_t iccsr = p_ctrl->p_reg->ICCSR;
     ...
     */
    if (0 != (iccsr & R_I3C0_ICCSR_DTAF_Msk))
    {
        p_ctrl->p_reg->ICCR_b.ABORT = 0;
    }
    ...
    if ((0 != (iccsr & R_I3C0_ICCSR_DTEF_Msk)) || error_recovery_case_2)
    {
#if I3C_CFG_SLAVE_SUPPORT
        if (I3C_INTERNAL_STATE_SLAVE_IDLE == p_ctrl->internal_state)
        {
    ...
    p_ctrl->p_reg->ICCSR &= ~(R_I3C0_ICCSR_DTEF_Msk | R_I3C0_ICCSR_DTAF_Msk);
...
void i3c_rx_isr (void)
{
     ...
     * during the transfer.
     */
    if (0 == (p_ctrl->p_reg->ICCSR & (R_I3C0_ICCSR_SQFF_Msk | R_I3C0_ICCSER_RQFDE_Msk)))
    {
        i3c_fifo_read(p_ctrl, p_ctrl->p_reg->ICDBSR_b.RBFL * sizeof(uint32_t));
    }
...
void i3c_rcv_isr (void)
{
    ...
    else
    {
            ...
            (I3C_CCC_DIRECT_SETDASA == command_code))
        {
            ...
            if (p_ctrl->p_reg->ICDAMR0_b.DAV)
            {
                /* Provide the assigned dynamic address in the callback. */
                uint32_t dynamic_address_w = (p_ctrl->p_reg->ICDAMR0 & R_I3C0_ICDAMR0_TADR_Msk);
                callback_args.dynamic_address = (uint8_t) ((dynamic_address_w >> R_I3C0_ICDAMR0_TADR_Pos) & UINT8_MAX);
            }
        }
        else
        {
            ...
            {
                if (0 != (p_ctrl->p_reg->ICMMR & R_I3C0_ICMMR_ACF_Msk))
                {
                    /* On success of the GETACCCR command, the driver will transition to master mode. */
                    p_ctrl->internal_state = I3C_INTERNAL_STATE_MASTER_IDLE;

                    /*
                     * Configure IBI queue threshold so that an IBI IRQ is generated when there is 1 IBI status descriptor in the FIFO.
                     */
                    p_ctrl->p_reg->ICQBTCR_b.IQTH = 0;
     ...
     * in the I3C section of the relevant hardware manual. */
    if ((0 != (iccsr & (R_I3C0_ICCSR_DTEF_Msk | R_I3C0_ICCSR_DTAF_Msk))) && (0U == p_ctrl->p_reg->ICSQSR_b.SQFL))
    {
        if (I3C_INTERNAL_STATE_SLAVE_IDLE == p_ctrl->internal_state)
        {
...
void i3c_ibi_isr (void)
{
...
#endif
    {
        ...
        if (0U != (I3C_IBI_STATUS_DESC_LAST_STATUS_Msk & ibi_status))
        {
            ...
            uint8_t ibi_address = (uint8_t) ((ibi_status & I3C_IBI_STATUS_DESC_IBI_ID_Msk) >>
                                             (I3C_IBI_STATUS_DESC_IBI_ID_Pos + 1));

            /* Get the RNW bit of the IBI transfer. */
            uint8_t ibi_rnw = (uint8_t) ((ibi_status >> I3C_IBI_STATUS_DESC_IBI_ID_Pos) & 1U);
 ...
 #if BSP_FEATURE_I3C_HAS_HOT_JOIN == 1
            else if (I3C_HOT_JOIN_ADDRESS == ibi_address)
            {
 ...
 *                             the Receive Status Queue (See  "ICISR: Internal Status Register" description in the I3C section
 *                             of the relevant hardware manual).
...
void i3c_master_error_recovery (i3c_instance_ctrl_t * p_ctrl, bool error_recovery_case_2)
{
    ...
    p_ctrl->p_reg->ICRCR = I3C_ICRCR_FIFO_FLUSH_Msk;

    /* The field will be cleared automatically upon reset completion (See "ICRCR : Reset Control Register" description in the I3C section
     * of the relevant hardware manual). */
    FSP_HARDWARE_REGISTER_WAIT((p_ctrl->p_reg->ICRCR & I3C_ICRCR_FIFO_FLUSH_Msk), 0U);

    /* Wait for the bus available condition. */
    while (1)
    {
        /* If SDA is pulled low, then a slave device started an IBI during error recovery. */
        if (0U == p_ctrl->p_reg->ICIMR_b.SDAI)
        {
            break;
        }

        /* Check the bus available condition. */
        if (1 == p_ctrl->p_reg->ICBSR_b.BAVL)
        {
            break;
        }
    }
    ...
    if (error_recovery_case_2)
    {
        ...
        p_ctrl->p_reg->ICMMR = (uint32_t) (R_I3C0_ICMMR_ACF_Msk | R_I3C0_ICMMR_WP_Msk);
    ...
    if (0U == p_ctrl->p_reg->ICIMR_b.SDAI)
    {
        /* Calculate the frequency of I3C clock. */
        uint32_t i3c_clock_frequency = i3c_clock_frequency_get();

        i3c_extended_cfg_t * p_extend = (i3c_extended_cfg_t *) p_ctrl->p_cfg->p_extend;

        /* Get the low and high period in I3CCLK ticks. */
        uint32_t i3c_low_period = (p_extend->bitrate_settings.stdbr & R_I3C0_ICSBR_ODLW_Msk) >>
                                  R_I3C0_ICSBR_ODLW_Pos;
        uint32_t i3c_high_period = (p_extend->bitrate_settings.stdbr & R_I3C0_ICSBR_ODHW_Msk) >>
                                   R_I3C0_ICSBR_ODHW_Pos;

        /* Calculate the high and low period for SCL. */
        uint32_t high_frequency = i3c_clock_frequency / i3c_high_period;
        uint32_t low_frequency  = i3c_clock_frequency / i3c_low_period;
        ...
        for (uint32_t i = 0; i < 4; i++)
        {
            ...
            if (0 != p_ctrl->p_reg->ICBCR_b.BC)
            {
                bc_zero = false;
            }
        }

        /* If ICBCR is not incrementing, then the master is not aware of the IBI. */
        if (bc_zero)
        {
            /* Write SCL low in order to complete the start condition. */
            p_ctrl->p_reg->ICOCR = R_I3C0_ICOCR_SDAO_Msk | R_I3C0_ICOCR_SOWP_Msk;
            ...
            {
                /* Write SCL high. */
                p_ctrl->p_reg->ICOCR = R_I3C0_ICOCR_SDAO_Msk | R_I3C0_ICOCR_SCLO_Msk | R_I3C0_ICOCR_SOWP_Msk;
            ...
            p_ctrl->p_reg->ICOCR = R_I3C0_ICOCR_SOWP_Msk;

            R_BSP_SoftwareDelay(low_delay_us, BSP_DELAY_UNITS_MICROSECONDS);

            /* Write SCL high. */
            p_ctrl->p_reg->ICOCR = R_I3C0_ICOCR_SCLO_Msk | R_I3C0_ICOCR_SOWP_Msk;
 ...
 #if I3C_ERROR_RECOVERY_VERSION_2 == I3C_CFG_ERROR_RECOVERY_SUPPORT
    {
        ...
        /* The field will be cleared automatically upon reset completion (See section 25.2.5 in the RA2E2 manual R01UH0919EJ0100 - This feature will be extended to support RX74M in a later release). */
        FSP_HARDWARE_REGISTER_WAIT((p_ctrl->p_reg->ICRCR & I3C_ICRCR_FIFO_FLUSH_Msk), 0U);
...
void i3c_slave_error_recovery (i3c_instance_ctrl_t * p_ctrl, i3c_slave_error_recovery_type_t recovery_type)
{
    switch (recovery_type)
    {
        ...
        case I3C_SLAVE_ERROR_RECOVERY_TYPE_READ:
        {
            ...
            p_ctrl->p_reg->ICRCR = R_I3C0_ICRCR_RBRST_Msk;
            break;
        }
        ...
        {
            /* Flush the Command and Rx Data FIFO. */
            p_ctrl->p_reg->ICRCR = R_I3C0_ICRCR_CQRST_Msk;
            break;
        }
 ...
 #if I3C_ERROR_RECOVERY_VERSION_1 == I3C_CFG_ERROR_RECOVERY_SUPPORT
    {
         ...
         * in the I3C section of the relevant hardware manual). */
        FSP_HARDWARE_REGISTER_WAIT((p_ctrl->p_reg->ICBSR & R_I3C0_ICBSR_BAVL_Msk), R_I3C0_ICBSR_BAVL_Msk);

        /* Wait for start condition to be cleared (See Figure "Example of error recovery operation flowchart for I3C slave"
         * in the I3C section of the relevant hardware manual). */
        FSP_HARDWARE_REGISTER_WAIT((p_ctrl->p_reg->ICSR2 & R_I3C0_ICSR2_START_Msk), 0);

        /* Read the current value of DADR. */
        uint32_t icdar0 = p_ctrl->p_reg->ICDAR0;
        ...
        uint32_t expected_bus_available_time =
            (1000000U * p_ctrl->p_reg->ICBATR + i3c_clock_frequency - 1) / i3c_clock_frequency; // NOLINT(readability-magic-numbers)
        ...
        if (0 == (p_ctrl->p_reg->ICBSR & R_I3C0_ICBSR_BAVL_Msk))
        {
            while (0 == (p_ctrl->p_reg->ICSR2 & R_I3C0_ICSR2_START_Msk))
            {
                /* Perform internal software reset. */
                p_ctrl->p_reg->ICRCR = R_I3C0_ICRCR_ISRST_Msk;
