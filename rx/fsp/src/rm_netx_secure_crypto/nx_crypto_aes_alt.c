/**************************************************************************/
/* This file is a copy of the nx_crypto_aes.c file from NetX Duo          */
/* It has been modified to work with Renesas Secure Crypto Engine.        */
/*  \file nx_crypto_aes_alt.c                                             */
/**************************************************************************/
...
{
#if (1U == NETX_SECURE_CRYPTO_NX_CRYPTO_METHODS_AES_ALT)
    return(sce_nx_crypto_aes_decrypt(aes_ptr, input, output, length));
#else
UINT  num_rounds;
UINT  round;
UINT *w;
...
{
#if (1U == NETX_SECURE_CRYPTO_NX_CRYPTO_METHODS_AES_ALT)
    return(sce_nx_crypto_aes_key_set(aes_ptr, key, key_size));
#else
UCHAR *expanded_key;
UINT   i;
...
UINT    status;
    ...
    (1U == BSP_FEATURE_RSIP_RSIP_E50D_SUPPORTED) || (1U == BSP_FEATURE_RSIP_RSIP_E31A_SUPPORTED) || \
    (1U == BSP_FEATURE_RSIP_RSIP_E11A_SUPPORTED))
...
UINT    status;
    ...
    switch (op)
    {
        case NX_CRYPTO_DECRYPT:
        {
             ...
             */
            status = sce_nx_crypto_gcm_decrypt_init(ctx, &(ctx -> nx_crypto_aes_mode_context.gcm),
                                                    (UINT (*)(VOID *, UCHAR *, UCHAR *, UINT))_nx_crypto_aes_encrypt,
