...
USHORT result;
    ...
    (0U == BSP_FEATURE_RSIP_SCE5B_SUPPORTED) && (0U == BSP_FEATURE_RSIP_SCE7_SUPPORTED) && \
    (0U == BSP_FEATURE_RSIP_RSIP_E51A_SUPPORTED) && (0U == BSP_FEATURE_RSIP_RSIP_E50D_SUPPORTED) && \
    (0U == BSP_FEATURE_RSIP_RSIP_E31A_SUPPORTED) && (0U == BSP_FEATURE_RSIP_RSIP_E11A_SUPPORTED))
/**************************************************************************/
/*                                                                        */
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
...
NX_CRYPTO_KEEP static VOID _nx_crypto_gcm_ghash_update(UCHAR *hkey, UCHAR *input, UINT input_length, UCHAR *output)
{
    ...
    (1U == BSP_FEATURE_RSIP_SCE5B_SUPPORTED) || (1U == BSP_FEATURE_RSIP_SCE7_SUPPORTED) || \
    (1U == BSP_FEATURE_RSIP_RSIP_E51A_SUPPORTED) || (1U == BSP_FEATURE_RSIP_RSIP_E50D_SUPPORTED) || \
    (1U == BSP_FEATURE_RSIP_RSIP_E31A_SUPPORTED) || (1U == BSP_FEATURE_RSIP_RSIP_E11A_SUPPORTED))
 ...
 *
 * @note _nx_crypto_gcm_decrypt_init does not exist in NetX Crypto code and is mapped to _nx_crypto_gcm_encrypt_init. 
                                     ...
                                     UINT block_size)
{
    ...
    if (iv_len == NX_CRYPTO_GCM_BLOCK_SIZE - 4)
    {
        /* When the length of IV is 12 then 1 is appended to IV to form j0. */
        /* j0 in increased before GCTR. */
        NX_CRYPTO_MEMCPY(j0, iv, iv_len); /* Use case of NX_CRYPTO_MEMCPY is verified. */
        j0[12] = 0;
        j0[13] = 0;
        j0[14] = 0;
        j0[15] = 1;
    ...
    else
    {
        /* When the length of IV is not 12 then apply GHASH to the IV. */
        NX_CRYPTO_MEMSET(j0, 0, NX_CRYPTO_GCM_BLOCK_SIZE);
        sce_nx_crypto_gcm_ghash_update(hkey, iv, iv_len, j0);

        /* Apply GHASH to the length of IV to form j0.*/
        NX_CRYPTO_MEMSET(tmp_block, 0, NX_CRYPTO_GCM_BLOCK_SIZE);
        tmp_block[NX_CRYPTO_GCM_BLOCK_SIZE - 2] = (UCHAR) (((iv_len << 3) & 0xFF00) >> 8);
        tmp_block[NX_CRYPTO_GCM_BLOCK_SIZE - 1] = (UCHAR) ((iv_len << 3) & 0x00FF);
        sce_nx_crypto_gcm_ghash_update(hkey, tmp_block, NX_CRYPTO_GCM_BLOCK_SIZE, j0);
    }
    /* Apply GHASH to the additional authenticated data. */
    NX_CRYPTO_MEMSET(s, 0, NX_CRYPTO_GCM_BLOCK_SIZE);

    sce_nx_crypto_gcm_ghash_update(hkey, additional_data, additional_len, s);
