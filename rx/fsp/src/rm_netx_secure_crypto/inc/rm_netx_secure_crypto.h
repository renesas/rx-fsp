...
#elif defined(__ICCRX__)               /* IAR compiler for RX */
 #define RM_NETX_CRYPTO_CHANGE_ULONG_ENDIAN(arg) \
    ((((uint32_t) (arg) >> 24) & 0x000000FFU) |  \
     (((uint32_t) (arg) >> 8) & 0x0000FF00U) |   \
     (((uint32_t) (arg) << 8) & 0x00FF0000U) |   \
     (((uint32_t) (arg) << 24) & 0xFF000000U))
#elif defined(__CCRX__)                /* CCRX compiler */
 #define RM_NETX_CRYPTO_CHANGE_ULONG_ENDIAN(arg)    ((unsigned int) (__revl(arg)))
#endif
 ...
 **********************************************************************************************************************/
#if defined(__CCRX__)
typedef long          rm_rand_ret_t;
typedef unsigned long rm_srand_arg_t;
#else
typedef int          rm_rand_ret_t;
typedef unsigned int rm_srand_arg_t;
#endif

#if defined(__CCRX__) && (1U == NETX_SECURE_CRYPTO_SOFTWARE_ONLY)
 #include <stdint.h>

static uint32_t g_nx_crypto_ctr_metadata[
    (sizeof(NX_CRYPTO_AES) + sizeof(uint32_t) - 1U) / sizeof(uint32_t)
];
 #define NX_CRYPTO_DRBG_CTR_CRYPTO_METADATA    ((void *) g_nx_crypto_ctr_metadata)
...
/* TRNG */
rm_rand_ret_t rand(void);
void          srand(rm_srand_arg_t seed);
