...
#define PDC_PRV_MIN_PIXEL_RESOLUTION      0x04U                 /* Min resolution: 4 */
 ...
 */
#define PDC_PERIPHERAL_REG_MAX_WAIT       (0x40U * 0x04U)
 ...
 *                                  4. x_capture_start_pixel + x_capture_pixels is less than 4, OR
 *                                  5. x_capture_start_pixel + x_capture_pixels is greater than 4095, OR
 *                                  6. y_capture_start_pixel + y_capture_pixels is greater than 4095
...
fsp_err_t R_PDC_Open (capture_ctrl_t * const p_ctrl, capture_cfg_t const * const p_cfg)
{
    ...
    FSP_ERROR_RETURN(((p_cfg->x_capture_start_pixel + p_cfg->x_capture_pixels) < (uint16_t) PDC_PRV_MAX_PIXEL_RESOLUTION) &&
                     ((p_cfg->x_capture_start_pixel + p_cfg->x_capture_pixels) >=
                      (uint16_t) PDC_PRV_MIN_PIXEL_RESOLUTION),
                     FSP_ERR_INVALID_ARGUMENT);
#endif
    ...
    R_BSP_IrqCfgEnable(p_extend->pdc_irq, (uint32_t) p_extend->pdc_ipl, p_ctrl);

    /* If DTC is used */
    if (p_extend->transfer_req_irq >= 0)
    {
        R_BSP_IrqCfgEnable(p_extend->transfer_req_irq, (uint32_t) p_extend->transfer_req_ipl, p_ctrl);
    }
