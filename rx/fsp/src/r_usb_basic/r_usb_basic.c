...
fsp_err_t R_USB_EventGet (usb_ctrl_t * const p_ctrl, usb_status_t * event)
{
    ...
    usb_instance_ctrl_t * p_api_ctrl = (usb_instance_ctrl_t *) p_ctrl;

    usb_cstd_usb_task();
    if (g_usb_cstd_event.write_pointer != g_usb_cstd_event.read_pointer)
    {
        *p_api_ctrl = g_usb_cstd_event.ctrl[g_usb_cstd_event.read_pointer];
...
fsp_err_t R_USB_Open (usb_ctrl_t * const p_ctrl, usb_cfg_t const * const p_cfg)
{
...
#if defined(USB_CFG_OTG_USE)
#endif                                 /* !defined (USB_CFG_OTG_USE)*/
    p_api_ctrl->module_number = p_cfg->module_number;
    p_api_ctrl->type          = (usb_class_t) (p_cfg->type & USB_VALUE_7FH);
    p_api_ctrl->p_context     = p_cfg->p_context;
#if (BSP_CFG_RTOS == 1)
    g_usb_apl_callback[p_api_ctrl->module_number] = usb_cstd_usbx_callback;
#else                                  /* #if (BSP_CFG_RTOS == 1) */
    g_usb_apl_callback[p_api_ctrl->module_number] = p_cfg->p_usb_apl_callback;
#endif                                 /* #if (BSP_CFG_RTOS == 1) */

#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
 #if USB_CFG_COMPLIANCE == USB_CFG_ENABLE
    g_usb_compliance_callback[p_api_ctrl->module_number] = p_cfg->usb_complience_cb;
 #endif                                /* #if USB_CFG_COMPLIANCE == USB_CFG_ENABLE */
    ...
    /* Argument Checking */
    FSP_ERROR_RETURN(!((USB_IP0 != p_api_ctrl->module_number) && (USB_IP1 != p_api_ctrl->module_number)),
    ...
    switch ((usb_class_internal_t) p_api_ctrl->type)
    {
        ...
        case USB_CLASS_INTERNAL_DFU:
        {
            ...
            if (USB_IP0 == p_api_ctrl->module_number)
            {
 ...
 #if defined(USB_HIGH_SPEED_MODULE)
            FSP_ERROR_RETURN(!((USB_SPEED_HS == p_cfg->usb_speed) && (USB_IP1 != p_api_ctrl->module_number)),
                             FSP_ERR_USB_PARAMETER)
 #else                                 /* defined (USB_HIGH_SPEED_MODULE) */
            FSP_ERROR_RETURN(USB_SPEED_HS != p_cfg->usb_speed, FSP_ERR_USB_PARAMETER)
 #endif  /* defined (USB_HIGH_SPEED_MODULE) */
...
#if (USB_CFG_DMA == USB_CFG_ENABLE)
    p_api_ctrl->p_transfer_tx = p_cfg->p_transfer_tx;
    p_api_ctrl->p_transfer_rx = p_cfg->p_transfer_rx;
 ...
 #endif                                /* defined(USB_CFG_PMSC_USE) */
#endif
    if (USB_YES == is_init[p_api_ctrl->module_number])
    {
    ...
    {
        if (0 != p_api_ctrl->p_transfer_tx)
        {
            R_DMAC_Open(p_api_ctrl->p_transfer_tx->p_ctrl, p_api_ctrl->p_transfer_tx->p_cfg);
        }

        if (0 != p_api_ctrl->p_transfer_rx)
        {
            R_DMAC_Open(p_api_ctrl->p_transfer_rx->p_ctrl, p_api_ctrl->p_transfer_rx->p_cfg);
        }
    }
    ...
    if (USB_MODE_HOST == p_cfg->usb_mode)
    {
        ...
        g_usb_usbmode[p_api_ctrl->module_number] = USB_MODE_HOST;
#endif
    }
    else
    {
        ...
        g_usb_usbmode[p_api_ctrl->module_number] = USB_MODE_PERI;
#endif
    }
    ...
    is_init[p_api_ctrl->module_number] = USB_YES;

    g_usb_open_class[p_api_ctrl->module_number] = 0;

#if (BSP_CFG_RTOS == 0)
    g_usb_change_device_state[p_api_ctrl->module_number] = 0;
#endif                                 /* (BSP_CFG_RTOS == 0) */

#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
    g_usb_hstd_use_pipe[p_api_ctrl->module_number] = 0;
    memset((void *) &g_usb_hdata[p_api_ctrl->module_number], 0, sizeof(usb_utr_t));
#endif                                 /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
    ...
    {
#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
        utr.ip  = p_api_ctrl->module_number;
        utr.ipp = usb_hstd_get_usb_ip_adr((uint16_t) p_api_ctrl->module_number); /* Get the USB IP base address. */
 ...
 #if !defined(USB_CFG_OTG_USE)
  #if defined(USB_CFG_HHID_USE)
            if (USB_CLASS_INTERNAL_HHID == (usb_class_internal_t) p_api_ctrl->type)
            {
 ...
 #if USB_CFG_TYPEC_CONNECTOR == USB_CFG_DISABLE
 ...
 #else  /*  USB_CFG_TYPEC_CONNECTOR == USB_CFG_DISABLE */
            usb_hstd_vbus_control(&utr, (uint16_t) USB_VBOFF);
 #endif /*  USB_CFG_TYPEC_CONNECTOR == USB_CFG_DISABLE */
    ...
    else
    {
#if ((USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI)
 #if defined(USB_CFG_PMSC_USE)
        g_usb_pmsc_usbip = p_api_ctrl->module_number;
  #if (BSP_CFG_RTOS == 1)
        ...
        err = usb_module_start(p_api_ctrl->module_number);
        if (FSP_SUCCESS == err)
        {
            /* USB driver initialization */
            usb_pdriver_init(p_api_ctrl, p_cfg);

            /* Setting MCU(USB interrupt init) register */
            usb_cpu_usbint_init(p_api_ctrl->module_number, p_cfg);

 #if defined(USB_HIGH_SPEED_MODULE)
            if (USB_SPEED_HS == p_cfg->usb_speed)
            {
                usb_utr_t hse_utr;
                hse_utr.ip = p_api_ctrl->module_number;
                hw_usb_set_hse(&hse_utr);
            }
 #endif                                /* defined (USB_HIGH_SPEED_MODULE) */

            /* Setting USB relation register  */
            hw_usb_pmodule_init(p_api_ctrl->module_number);

 #if (BSP_CFG_RTOS == 1)
  #if defined(USB_HIGH_SPEED_MODULE)
            if (USB_IP1 == p_api_ctrl->module_number)
            {
  ...
  #endif                               /* defined (USB_HIGH_SPEED_MODULE) */
 #endif                                /* #if (BSP_CFG_RTOS == 1) */
            if (USB_ATTACH == usb_pstd_chk_vbsts(p_api_ctrl->module_number))
            {
 #if defined(USB_HIGH_SPEED_MODULE)
                hw_usb_set_cnen(p_api_ctrl->module_number);
 #endif                                /* defined (USB_HIGH_SPEED_MODULE) */
                usb_cpu_delay_xms((uint16_t) 10);
                hw_usb_pset_dprpu(p_api_ctrl->module_number);
            }
        }
#endif                                 /* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_REPI */
    }
    ...
    if (FSP_SUCCESS == err)
    {
        ...
        {
#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << p_api_ctrl->type);  /* Set USB Open device class */
            if (USB_CLASS_INTERNAL_HCDC == (usb_class_internal_t) p_api_ctrl->type)
            {
                g_usb_open_class[p_api_ctrl->module_number] |= (1 << USB_CLASS_INTERNAL_HCDCC); /* Set USB Open device class */
            }

 #if defined(USB_CFG_OTG_USE)
            g_is_A_device[p_api_ctrl->module_number]  = USB_YES;
            _ux_system_otg->ux_system_otg_device_type = UX_OTG_DEVICE_A;

            (*g_p_otg_callback[p_api_ctrl->module_number])(UX_OTG_MODE_HOST);
 #endif                                /* defined(USB_CFG_OTG_USE) */
#endif                                 /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
        }
        else
        {
#if defined(USB_CFG_PCDC_USE)
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_PCDC);
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_PCDCC);
 #if defined(USB_CFG_PCDC2_USE)
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_PCDC2);
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_PCDCC2);
 #endif                                /* defined (USB_CFG_PCDC2_USE) */
#endif                                 /* defined(USB_CFG_PCDC_USE) */

#if defined(USB_CFG_PHID_USE)
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_PHID);
 #if ((USB_CFG_PHID_INT_IN2 != USB_NULL) || (USB_CFG_PHID_INT_OUT2 != USB_NULL))
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_PHID2);
 #endif                                /* ((USB_CFG_PHID_INT_IN2 != USB_NULL) || (USB_CFG_PHID_INT_OUT2 != USB_NULL)) */
#endif                                 /* defined(USB_CFG_PHID_USE) */

#if defined(USB_CFG_PMSC_USE)
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_PMSC);
#endif                                 /* defined(USB_CFG_PMSC_USE) */

#if defined(USB_CFG_PAUD_USE)
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_PAUD);
#endif                                 /* defined(USB_CFG_PAUD_USE) */

#if defined(USB_CFG_PVND_USE)
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_PVND);
#endif                                 /* defined(USB_CFG_PVND_USE) */

#if defined(USB_CFG_PPRN_USE)
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_PPRN);
#endif                                 /* defined(USB_CFG_PPRN_USE) */

#if defined(USB_CFG_DFU_USE)
            g_usb_open_class[p_api_ctrl->module_number] |= (uint16_t) (1 << USB_CLASS_INTERNAL_DFU);
#endif                                 /* defined(USB_CFG_PPRN_USE) */

#if defined(USB_CFG_OTG_USE)
            _ux_system_otg->ux_system_otg_device_type = UX_OTG_DEVICE_B;
            (*g_p_otg_callback[p_api_ctrl->module_number])(UX_OTG_MODE_SLAVE);
#endif                                 /* defined(USB_CFG_OTG_USE) */
        }
    }
...
fsp_err_t R_USB_Close (usb_ctrl_t * const p_ctrl)
{
    ...
    _ux_system_otg->ux_system_otg_device_type = UX_OTG_DEVICE_IDLE;
    g_is_A_device[p_api_ctrl->module_number]  = USB_NO;
    ...
    ret_code = usb_module_stop(p_api_ctrl->module_number);

    if (FSP_SUCCESS == ret_code)
    {
        is_init[p_api_ctrl->module_number] = USB_NO;
        ...
        if ((USB_NO == g_is_usbx_otg_host_class_init[p_api_ctrl->module_number]) ||
            (USB_YES == g_is_A_device[p_api_ctrl->module_number]))
        {
            if (USB_IP1 == p_api_ctrl->module_number)
            {
 #if defined(USB_CFG_HCDC_USE)
        ...
        usb_pstd_clr_pipe_table(p_api_ctrl->module_number);
        ...
        usb_rtos_delete(p_api_ctrl->module_number);
        ...
        g_usb_usbmode[p_api_ctrl->module_number] = 0;
    }
    else
    {
    ...
    if (USB_MODE_HOST == g_usb_usbmode[p_api_ctrl->module_number])
    {
  #if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
        switch ((usb_class_internal_t) p_api_ctrl->type)
        {
 ...
 #if (BSP_CFG_RTOS == 1)
    if (USB_MODE_PERI == g_usb_usbmode[p_api_ctrl->module_number])
    {
  #if ((USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI)
        ux_utr.ip  = p_api_ctrl->module_number;
  ...
  #if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
        ux_utr.ip  = p_api_ctrl->module_number;
        ux_utr.ipp = usb_hstd_get_usb_ip_adr((uint16_t) p_api_ctrl->module_number); /* Get the USB IP base address. */
    ...
    if (FSP_SUCCESS == ret_code)
    {
        ...
        if (USB_MODE_HOST == g_usb_usbmode[p_api_ctrl->module_number])
        {
            ...
            utr.ip  = p_api_ctrl->module_number;
            utr.ipp = usb_hstd_get_usb_ip_adr((uint16_t) utr.ip);

            usb_hstd_driver_release(&utr, class_code);

            usb_hstd_clr_pipe_table((uint16_t) utr.ip, USB_ADDRESS1);
...
fsp_err_t R_USB_Read (usb_ctrl_t * const p_ctrl, uint8_t * p_buf, uint32_t size, uint8_t destination)
{
    ...
    {
#if ((USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI)
        p_api_ctrl->type = (usb_class_t) (destination & USB_VALUE_7FH);
#endif                                 /* #if ((USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI) */
    }
    else
    {
#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
        p_api_ctrl->device_address = destination;
 #if defined(USB_CFG_HCDC_USE)

        /* Add the following code. */
        /* Because the value of "type" member change to "USB_CLASS_INTERNAL_HCDCC" */
        /* when calling R_USB_HCDC_ControlDataRead function. */
        p_api_ctrl->type = (usb_class_t) USB_CLASS_INTERNAL_HCDC;
 #endif                                /* defined(USB_CFG_HCDC_USE) */
#endif                                 /* #if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST) */
    }
    ...
    FSP_ERROR_RETURN(!((USB_CLASS_INTERNAL_PCDCC == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_PVND == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_HVND == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_HMSC == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_PMSC == (usb_class_internal_t) p_api_ctrl->type)),
                     FSP_ERR_USB_PARAMETER)

    check_open_class = g_usb_open_class[p_api_ctrl->module_number];
    check_open_class = (check_open_class & (uint32_t) (1 << p_api_ctrl->type));

    FSP_ERROR_RETURN(USB_NULL != check_open_class, FSP_ERR_USB_PARAMETER) /* Check USB Open device class */
#endif  /* USB_CFG_PARAM_CHECKING_ENABLE */
    ...
    (void) R_USB_InfoGet(p_api_ctrl, &info, p_api_ctrl->device_address);
    if (USB_STATUS_CONFIGURED == info.device_status)
    {
        err = usb_data_read(p_api_ctrl, p_buf, size);
...
fsp_err_t R_USB_Write (usb_ctrl_t * const p_ctrl, uint8_t const * const p_buf, uint32_t size, uint8_t destination)
{
    ...
    FSP_ERROR_RETURN(!((USB_CLASS_INTERNAL_HCDCC == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_PVND == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_HVND == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_HMSC == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_PMSC == (usb_class_internal_t) p_api_ctrl->type)),
    ...
    if (USB_STATUS_CONFIGURED == info.device_status)
    {
        err = usb_data_write(p_api_ctrl, p_buf, size);
...
fsp_err_t R_USB_Stop (usb_ctrl_t * const p_ctrl, usb_transfer_t direction, uint8_t destination)
{
    ...
    FSP_ERROR_RETURN(!((USB_CLASS_INTERNAL_PVND == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_HVND == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_HMSC == (usb_class_internal_t) p_api_ctrl->type) ||
                       (USB_CLASS_INTERNAL_PMSC == (usb_class_internal_t) p_api_ctrl->type)),
                     FSP_ERR_USB_PARAMETER)
    ...
    FSP_ERROR_RETURN(!(((USB_CLASS_INTERNAL_HCDCC == (usb_class_internal_t) p_api_ctrl->type) &&
                       ...
                       ((USB_CLASS_INTERNAL_PCDCC == (usb_class_internal_t) p_api_ctrl->type) &&
    ...
    err = usb_data_stop(p_api_ctrl, direction);
...
fsp_err_t R_USB_Suspend (usb_ctrl_t * const p_ctrl)
{
    ...
    FSP_ERROR_RETURN(USB_MODE_PERI != g_usb_usbmode[p_api_ctrl->module_number], FSP_ERR_USB_FAILED)
 ...
 #if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
    p_api_ctrl->device_address = USB_DEVICEADDR;

    ret_code = R_USB_InfoGet(p_api_ctrl, &info, p_api_ctrl->device_address);
    FSP_ERROR_RETURN(USB_STATUS_CONFIGURED == info.device_status, FSP_ERR_USB_FAILED)

    utr.ip  = p_api_ctrl->module_number;
    utr.ipp = usb_hstd_get_usb_ip_adr((uint16_t) utr.ip);
  #if (BSP_CFG_RTOS == 0)
    if (USB_NULL != (g_usb_change_device_state[p_api_ctrl->module_number] & (1 << USB_STATUS_SUSPEND)))
    {
        return FSP_ERR_USB_BUSY;
    }
    ...
    if (USB_OK == err)
    {
        g_usb_change_device_state[p_api_ctrl->module_number] |= (1 << USB_STATUS_SUSPEND);
    }
    else
    {
  ...
  #else                                /* (BSP_CFG_RTOS == 0) */
    if (USB_YES == gs_usb_suspend_ing[p_api_ctrl->module_number])
    {
        ret_code = FSP_ERR_USB_BUSY;
    }
    else
    {
        gs_usb_suspend_ing[p_api_ctrl->module_number] = USB_YES;
    }
    ...
    gs_usb_suspend_ing[p_api_ctrl->module_number] = USB_NO;
  #endif                               /* (BSP_CFG_RTOS == 0) */
 #endif                                /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
...
fsp_err_t R_USB_Resume (usb_ctrl_t * const p_ctrl)
{
    ...
    FSP_ERROR_RETURN(USB_MODE_PERI != g_usb_usbmode[p_api_ctrl->module_number], FSP_ERR_USB_PARAMETER)
 #endif                                /* USB_CFG_PARAM_CHECKING_ENABLE */
  ...
  #if (BSP_CFG_RTOS == 0)
    if (USB_NULL != (g_usb_change_device_state[p_api_ctrl->module_number] & (1 << USB_STATUS_RESUME)))
    {
        return FSP_ERR_USB_BUSY;
    }
    ...
    err = usb_hstd_change_device_state(&utr,
                                       ...
                                       (uint16_t) p_api_ctrl->device_address);
    if (USB_OK == err)
    {
        g_usb_change_device_state[p_api_ctrl->module_number] |= (1 << USB_STATUS_RESUME);
    }
    else
    {
        ret_code = FSP_ERR_USB_FAILED;
  ...
  #else                                /* (BSP_CFG_RTOS == 0) */
    if (USB_YES == g_usb_resume_ing[p_api_ctrl->module_number])
    {
        ret_code = FSP_ERR_USB_BUSY;
    }
    else
    {
        g_usb_resume_ing[p_api_ctrl->module_number] = USB_YES;
    }
    ...
    g_usb_resume_ing[p_api_ctrl->module_number] = USB_NO;
  #endif                               /* (BSP_CFG_RTOS == 0) */
 #endif                                /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
...
fsp_err_t R_USB_InfoGet (usb_ctrl_t * const p_ctrl, usb_info_t * p_info, uint8_t destination)
{
    ...
    FSP_ERROR_RETURN(0 != g_usb_open_class[p_api_ctrl->module_number], FSP_ERR_USB_FAILED)
    ...
    if (USB_MODE_HOST == g_usb_usbmode[p_api_ctrl->module_number])
    {
        ...
        switch (g_usb_hstd_device_info[utr.ip][p_api_ctrl->device_address][3])
        {
            case USB_IFCLS_CDC:
            {
        ...
        switch (g_usb_hstd_device_info[utr.ip][p_api_ctrl->device_address][4])
        {
            case USB_NOCONNECT:
            {
        ...
        switch (g_usb_hstd_device_info[utr.ip][p_api_ctrl->device_address][1])
        {
        ...
        if (USB_DEVICEADDR == p_api_ctrl->device_address) /* Check Root port address */
        {
    ...
    else
    {
        ...
        status = hw_usb_read_intsts(p_api_ctrl->module_number);
...
fsp_err_t R_USB_PipeRead (usb_ctrl_t * const p_ctrl, uint8_t * p_buf, uint32_t size, uint8_t pipe_number)
{
    ...
    p_api_ctrl->pipe = pipe_number;

 #if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
    p_api_ctrl->device_address = USB_ADDRESS1;
 #endif                                /* USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */

 #if USB_CFG_PARAM_CHECKING_ENABLE
    FSP_ERROR_RETURN(((USB_MIN_PIPE_NO <= p_api_ctrl->pipe) && (p_api_ctrl->pipe <= USB_MAX_PIPE_NO)),
                     FSP_ERR_USB_PARAMETER)
    ...
    FSP_ERROR_RETURN(USB_NULL != (g_usb_open_class[p_api_ctrl->module_number] &
                                  (uint32_t) (1 << p_api_ctrl->type)),
    ...
    if (USB_STATUS_CONFIGURED == info.device_status)
    {
        ...
        if (USB_MODE_HOST == g_usb_usbmode[p_api_ctrl->module_number])
        {
  ...
  #else                                /* (BSP_CFG_RTOS != 0) */
            p_tran_data = &g_usb_hdata[p_api_ctrl->module_number][p_api_ctrl->pipe];
  #endif  /* (BSP_CFG_RTOS != 0) */

            p_tran_data->ip           = p_api_ctrl->module_number;
            p_tran_data->ipp          = usb_hstd_get_usb_ip_adr((uint16_t) p_tran_data->ip);
            p_tran_data->keyword      = p_api_ctrl->pipe;       /* Pipe No */
            ...
            if (0 != p_api_ctrl->p_transfer_tx)
            {
                p_tran_data->p_transfer_tx = p_api_ctrl->p_transfer_tx;
            }
            else
            {
            ...
            if (0 != p_api_ctrl->p_transfer_rx)
            {
                p_tran_data->p_transfer_rx = p_api_ctrl->p_transfer_rx;
            }
            else
            {
        ...
        else
        {
            ...
            p_tran_data = &tran_data;
  #else                                                                     /* (BSP_CFG_RTOS != 0) */
            p_tran_data = &g_usb_pdata[p_api_ctrl->pipe];
  #endif  /* (BSP_CFG_RTOS != 0) */

            p_tran_data->ip           = p_api_ctrl->module_number;          /* USB Module Number */
            p_tran_data->keyword      = p_api_ctrl->pipe;                   /* Pipe No */
            p_tran_data->p_tranadr    = p_buf;                              /* Data address */
            p_tran_data->tranlen      = size;                               /* Data Size */
...
fsp_err_t R_USB_PipeWrite (usb_ctrl_t * const p_ctrl, uint8_t * p_buf, uint32_t size, uint8_t pipe_number)
{
    ...
    FSP_ERROR_RETURN(USB_NULL != (g_usb_open_class[p_api_ctrl->module_number] & (uint32_t) (1 << p_api_ctrl->type)),
                     FSP_ERR_USB_PARAMETER) /* Check USB Open device class */
    ...
    if (USB_STATUS_CONFIGURED == info.device_status)
    {
        ...
        if (USB_MODE_HOST == g_usb_usbmode[p_api_ctrl->module_number])
        {
            ...
            p_tran_data->ip        = p_api_ctrl->module_number;
            p_tran_data->ipp       = usb_hstd_get_usb_ip_adr((uint16_t) p_api_ctrl->module_number);
            p_tran_data->keyword   = p_api_ctrl->pipe;        /* Pipe No */
            p_tran_data->p_tranadr = p_buf;                   /* Data address */
            p_tran_data->tranlen   = size;                    /* Data Size */
...
fsp_err_t R_USB_PipeStop (usb_ctrl_t * const p_ctrl, uint8_t pipe_number)
{
    ...
    utr.ip = p_api_ctrl->module_number; /* Update USB module number */
    ...
    FSP_ERROR_RETURN(USB_PIPE0 != p_api_ctrl->pipe, FSP_ERR_USB_PARAMETER)

    FSP_ERROR_RETURN(USB_MAXPIPE_NUM > p_api_ctrl->pipe, FSP_ERR_USB_PARAMETER)
    ...
    {
 #if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
        err = usb_hstd_transfer_end(&utr, (uint16_t) p_api_ctrl->pipe, (uint16_t) USB_DATA_STOP);
 #endif                                /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
    }
    else
    {
 #if ((USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI)
        err = usb_pstd_transfer_end(&utr, (uint16_t) p_api_ctrl->pipe);
 #endif                                /* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI */
    }
...
fsp_err_t R_USB_UsedPipesGet (usb_ctrl_t * const p_ctrl, uint16_t * p_pipe, uint8_t destination)
{
    ...
    R_USB_InfoGet(p_ctrl, &info, p_api_ctrl->device_address);
    FSP_ERROR_RETURN(USB_STATUS_CONFIGURED == info.device_status, FSP_ERR_USB_FAILED)
    ...
    * p_pipe = ((uint16_t) 1 << USB_PIPE0);
    if (USB_MODE_HOST == g_usb_usbmode[p_api_ctrl->module_number])
    {
        ...
        {
            if (USB_TRUE == g_usb_pipe_table[p_api_ctrl->module_number][pipe_no].use_flag)
            {
                if ((((uint16_t) p_api_ctrl->device_address) << USB_DEVADDRBIT) ==
                    (uint16_t) (g_usb_pipe_table[p_api_ctrl->module_number][pipe_no].pipe_maxp & USB_DEVSEL))
                {
...
fsp_err_t R_USB_PipeInfoGet (usb_ctrl_t * const p_ctrl, usb_pipe_t * p_info, uint8_t pipe_number)
{
    ...
    R_USB_InfoGet(p_api_ctrl, &info, p_api_ctrl->device_address);
    FSP_ERROR_RETURN(USB_STATUS_CONFIGURED == info.device_status, FSP_ERR_USB_FAILED)

    if (USB_MODE_HOST == g_usb_usbmode[p_api_ctrl->module_number])
    {
#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
        p_info->endpoint = usb_hstd_pipe_to_epadr(&utr, (uint16_t) p_api_ctrl->pipe);
        pipe_type        = usb_cstd_get_pipe_type(&utr, (uint16_t) p_api_ctrl->pipe);
        ...
        {
            p_info->maxpacketsize = usb_cstd_get_maxpacket_size(&utr, (uint16_t) p_api_ctrl->pipe); /* Set Max packet size */
            result                = FSP_SUCCESS;
        }
#endif  /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
    }
    else
    {
        ...
        /* Get PIPE Number from Endpoint address */
        p_info->endpoint =
            (uint8_t) (g_usb_pipe_table[p_api_ctrl->module_number][p_api_ctrl->pipe].pipe_cfg & USB_EPNUMFIELD);     /* Set EP num. */
        if (USB_DIR_P_IN == (g_usb_pipe_table[p_api_ctrl->module_number][p_api_ctrl->pipe].pipe_cfg & USB_DIRFIELD)) /* Check dir */
        {
...
fsp_err_t R_USB_PullUp (usb_ctrl_t * const p_ctrl, uint8_t state)
{
    ...
    FSP_ERROR_RETURN(!(USB_MODE_HOST == g_usb_usbmode[p_api_ctrl->module_number]), FSP_ERR_USB_FAILED) /* Support Peri only. */

    hw_usb_pcontrol_dprpu(p_api_ctrl->module_number, state);
                                     ...
                                     uint8_t            device_address)
{
    ...
    result = R_USB_InfoGet(p_api_ctrl, &info, device_address);
    if (FSP_SUCCESS != result)
    {
        return FSP_ERR_USB_FAILED;
    }

    p_api_ctrl->device_address       = device_address;
    p_api_ctrl->setup.request_type   = p_setup->request_type;
    p_api_ctrl->setup.request_value  = p_setup->request_value;
    p_api_ctrl->setup.request_index  = p_setup->request_index;
    p_api_ctrl->setup.request_length = p_setup->request_length;

    if ((p_api_ctrl->setup.request_type & USB_DEV_TO_HOST) == USB_DEV_TO_HOST)
    {
        err = usb_ctrl_read(p_api_ctrl, p_buf, (uint32_t) p_api_ctrl->setup.request_length);  /* Request Control transfer */
    }
    else
    {
        err = usb_ctrl_write(p_api_ctrl, p_buf, (uint32_t) p_api_ctrl->setup.request_length); /* Request Control transfer */
    }
...
fsp_err_t R_USB_PeriControlDataGet (usb_ctrl_t * const p_ctrl, uint8_t * p_buf, uint32_t size)
{
    ...
    result = R_USB_InfoGet(p_api_ctrl, &info, p_api_ctrl->device_address);
    if (FSP_SUCCESS != result)
    {
        return FSP_ERR_USB_FAILED;
    }

    err = usb_ctrl_read(p_api_ctrl, p_buf, size); /* Request Control transfer */
    if (err == USB_QOVR)
    {
        result = FSP_ERR_USB_BUSY;
    }
...
fsp_err_t R_USB_PeriControlDataSet (usb_ctrl_t * const p_ctrl, uint8_t * p_buf, uint32_t size)
{
    ...
    err = usb_ctrl_write(p_api_ctrl, p_buf, size); /* Request Control transfer */
    if (err == USB_QOVR)
    {
        result = FSP_ERR_USB_BUSY;
    }
...
fsp_err_t R_USB_PeriControlStatusSet (usb_ctrl_t * const p_ctrl, usb_setup_status_t status)
{
    ...
    p_api_ctrl->status = (uint16_t) status;

    hw_usb_pcontrol_dcpctr_pid(p_api_ctrl->module_number, (uint16_t) status);
...
fsp_err_t R_USB_ModuleNumberGet (usb_ctrl_t * const p_ctrl, uint8_t * module_number)
{
    ...
    *module_number = p_api_ctrl->module_number;
...
fsp_err_t R_USB_ClassTypeGet (usb_ctrl_t * const p_ctrl, usb_class_t * class_type)
{
    ...
    *class_type = (usb_class_t) (p_api_ctrl->type | USB_VALUE_80H);
...
fsp_err_t R_USB_DeviceAddressGet (usb_ctrl_t * const p_ctrl, uint8_t * device_address)
{
    ...
    *device_address = p_api_ctrl->device_address;
...
fsp_err_t R_USB_DeviceStateGet (usb_ctrl_t * const p_ctrl, uint16_t * state)
{
    ...
    {
#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
        switch (g_usb_hstd_device_info[p_api_ctrl->module_number][p_api_ctrl->device_address][1])
        {
            case USB_POWERED:          /* Power state  */
            {
...
fsp_err_t R_USB_DataSizeGet (usb_ctrl_t * const p_ctrl, uint32_t * data_size)
{
    ...
    *data_size = p_api_ctrl->data_size;
 ...
 * @retval FSP_ERR_ASSERTION        Parameter is NULL error.
 ******************************************************************************/
fsp_err_t R_USB_OtgCallbackSet (usb_ctrl_t * const p_ctrl, usb_otg_callback_t * p_callback)
{
    usb_instance_ctrl_t * p_api_ctrl = (usb_instance_ctrl_t *) p_ctrl;
...
#if (BSP_CFG_RTOS == 1)
 #if defined(USB_CFG_OTG_USE)
    g_p_otg_callback[p_api_ctrl->module_number] = p_callback;
 ...
 ******************************************************************************/
fsp_err_t R_USB_OtgSRP (usb_ctrl_t * const p_ctrl)
{
    usb_instance_ctrl_t * p_api_ctrl = (usb_instance_ctrl_t *) p_ctrl;
