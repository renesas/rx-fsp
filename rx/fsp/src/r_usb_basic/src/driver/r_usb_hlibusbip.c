...
void usb_hstd_data_end (usb_utr_t * ptr, uint16_t pipe, uint16_t status)
{
    ...
    if (USB_NULL != g_p_usb_hstd_pipe[ip][pipe])
    {
        ...
        g_p_usb_hstd_pipe[ip][pipe]->ipp     = usb_hstd_get_usb_ip_adr((uint16_t) ip);
  ...
  #endif                                                   /* #if (BSP_CFG_RTOS == 1) */
        g_p_usb_hstd_pipe[ip][pipe] = (usb_utr_t *) USB_NULL;
        usb_cstd_pipe_msg_re_forward((uint16_t) ip, pipe); /* Get PIPE Transfer wait que and Message send to HCD */
