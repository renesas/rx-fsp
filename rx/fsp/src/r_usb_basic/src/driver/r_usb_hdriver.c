...
static void usb_hstd_otg_mode_to_peri (usb_utr_t * p_utr)
{
    p_utr->ipp = usb_hstd_get_usb_ip_adr((uint16_t) p_utr->ip); /* Get the USB IP base address. */
 ...
 #endif /* #if (BSP_CFG_RTOS == 1) */
{
    ...
    else
    {
        ...
        switch (msginfo)
        {
            ...
            {
                ptr->ipp = usb_hstd_get_usb_ip_adr((uint16_t) ptr->ip); /* Get the USB IP base address. */
            ...
            {
                pipenum = (uint16_t) (ptr->keyword & USB_PIPENM);
...
void usb_hstd_suspend_complete (usb_utr_t * ptr, uint16_t data1, uint16_t data2)
{
  ...
  #endif                               /* defined(VECTOR_NUMBER_USBHS_USBHSR) */
  ...
  #endif                               /* defined(VECTOR_NUMBER_USB0_STATUS) */
 #endif /* defined(USB_HIGH_SPEED_MODULE) */
    }
    else
    {
    ...
    if (0 != dev_addr)
    {
        ...
        for (pipe_no = USB_MIN_PIPE_NO; pipe_no < (USB_MAX_PIPE_NO + 1); pipe_no++)
        {
            ...
            if (USB_TRUE == g_usb_pipe_table[ptr->ip][pipe_no].use_flag)
            {
                ...
                {
                    usb_hstd_set_pipe_reg(ptr, (uint16_t) pipe_no);
                    err = USB_OK;
                }
            }
        }
...
void usb_hvnd_detach (usb_utr_t * ptr, uint16_t dev_addr, uint16_t data2)
{
    ...
    usb_hstd_clr_pipe_table((uint16_t) ptr->ip, dev_addr);
...
static void usb_hvnd_pipe_info (usb_utr_t * p_utr, uint8_t * table, uint16_t speed, uint16_t length)
{
    ...
    while (ofdsc < length)
    {
        ...
        if (USB_DT_ENDPOINT == table[ofdsc + 1])
        {
            ...
            if (USB_NULL != pipe_no)
            {
                usb_hstd_set_pipe_info((uint16_t) p_utr->ip, pipe_no, &ep_tbl);
            }
            else
            {
                return;
...
void usb_hvnd_read_complete (usb_utr_t * ptr, uint16_t data1, uint16_t data2)
{
    ...
    ctrl.device_address = (uint8_t) (usb_hstd_get_devsel(ptr, (uint16_t) ctrl.pipe) >> 12);
  #if (BSP_CFG_RTOS != 0)
    ctrl.p_data = (void *) ptr->cur_task_hdl;
  #endif                               /* #if (BSP_CFG_RTOS != 0) */
                                            ...
                                            usb_pipe_table_reg_t * pipe_table_work)
{
    ...
    if (USB_NULL != pipe_no)
    {
        ...
        if (USB_IP1 == p_utr->ip)
        {
            ...
            pipe_buf = usb_hstd_get_pipe_buf_value((uint16_t) pipe_no);
