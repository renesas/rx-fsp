...
static void usb_peri_class_request_usbx_get_data(usb_setup_t * p_req, usb_utr_t * p_utr);
        ...
        switch ((uint16_t) USB_GET_DT_TYPE(g_usb_pstd_req_value))
        {
            ...
            case USB_DT_CONFIGURATION:
            {
                if ((0 == g_usb_pstd_req_index) && ((uint16_t) 0 == idx))
                {
                    ...
                    len = (uint16_t) (len << 8);
                    len = (uint16_t) (len + ((uint16_t) p_table[2]));
...
/******************************************************************************
 * Function Name   : usb_peri_class_request_usbx_get_data
 * Description     : Class request processing for data stage (USBX)
 * Arguments       : usb_setup_t *p_req : Pointer to usb_setup_t structure
 * Return value    : none
 ******************************************************************************/
static void usb_peri_class_request_usbx_get_data (usb_setup_t * p_req, usb_utr_t * p_utr)
{
    ...
    usb_pstd_ctrl_write((uint32_t) p_req->request_length,
                        (uint8_t *) transfer_request->ux_slave_transfer_request_data_pointer,
                        &tran_data);
}                                      /* End of function usb_peri_class_request_usbx_get_data */
...
void usb_peri_class_request_rwds (usb_setup_t * req, usb_utr_t * p_utr)
{
    ...
    {
        /* Out Data */
        usb_peri_class_request_usbx_get_data(req, p_utr);
    }
