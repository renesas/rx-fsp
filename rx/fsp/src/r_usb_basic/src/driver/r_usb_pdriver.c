...
static void usb_pstd_interrupt (usb_utr_t * p_mess)
{
            ...
            else
            {
                ...
                if (USB_YES == g_is_A_device[p_mess->ip])
                {
                    ...
                    else
                    {
                        ...
                        p_mess->ipp = usb_hstd_get_usb_ip_adr((uint16_t) p_mess->ip);
  ...
  #if defined(USB_CFG_OTG_USE)
                        p_mess->ipp = usb_hstd_get_usb_ip_adr((uint16_t) p_mess->ip); /* Get the USB IP base address. */
...
static void usb_pstd_otg_hnp_process (usb_utr_t * p_utr)
{
    ...
    p_utr->ipp = (usb_regadr_t) usb_hstd_get_usb_ip_adr((uint16_t) p_utr->ip); /* Get the USB IP base address. */
...
uint16_t usb_pstd_get_alternate_num (uint16_t int_num)
{
    ...
    ptr   += ptr[0];
    length = (uint16_t) g_usb_pstd_driver.p_configtbl[3];
    length = (uint16_t) (length << 8);
    length = (uint16_t) (length + ((uint16_t) g_usb_pstd_driver.p_configtbl[2]));
...
void usb_pstd_set_eptbl_index (uint16_t int_num, uint16_t alt_num)
{
    ...
    ptr     = g_usb_pstd_driver.p_configtbl;
    i       = *ptr;
    length  = (uint16_t) ptr[3];
    length  = (uint16_t) (length << 8);
    length  = (uint16_t) (length + ((uint16_t) ptr[2]));
    ptr    += *ptr;
    start   = 0;
    numbers = 0;
    j       = 0;
    ...
    for ( ; i < length; )
    {
        ...
        switch (ptr[1])
        {
            ...
            case USB_DT_INTERFACE:
            {
                ...
                {
                    start = (uint16_t) (start + (uint16_t) ptr[4]);
                }
...
uint16_t usb_pstd_chk_remote (void)
{
    ...
    atr = g_usb_pstd_driver.p_configtbl[7];
...
uint8_t usb_pstd_get_current_power (void)
{
    ...
    tmp = g_usb_pstd_driver.p_configtbl[7];
...
{
    /* Device status */
    tbl[0] = (uint16_t) (hw_usb_read_intsts(ptr->ip) & (uint16_t) (USB_VBSTS | USB_DVSQ));
...
void usb_peri_devdefault (usb_utr_t * ptr, uint16_t mode, uint16_t data2)
{
    ...
    len = (uint16_t) ptable[3];
    len = (uint16_t) (len << 8);
    len = (uint16_t) (len + ((uint16_t) ptable[2]));
