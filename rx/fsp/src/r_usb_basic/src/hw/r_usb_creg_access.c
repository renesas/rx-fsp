...
void hw_usb_clear_sts_brdy (usb_utr_t * ptr, uint16_t pipeno)
{
    if (g_usb_usbmode[ptr->ip] == USB_MODE_PERI)
    {
        ...
        if (USB_CFG_IP0 == ptr->ip)
        {
            USB_M0->BRDYSTS = (uint16_t) ((uint16_t) (~(1 << pipeno)) & BRDYSTS_MASK);
        }
        else
        {
            USB_M1->BRDYSTS = (uint16_t) ((uint16_t) (~(1 << pipeno)) & BRDYSTS_MASK);
        }
#endif                                 /* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_REPI */
    }
    else
    {
#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
        ptr->ipp->BRDYSTS = (uint16_t) ((uint16_t) (~(1 << pipeno)) & BRDYSTS_MASK);
#endif                                 /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
    }
}
...
void hw_usb_clear_status_nrdy (usb_utr_t * ptr, uint16_t pipeno)
{
    ...
    {
#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
        ptr->ipp->NRDYSTS = (uint16_t) ((uint16_t) (~(1 << pipeno)) & NRDYSTS_MASK);
#endif                                 /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
    }
}
...
void hw_usb_clear_status_bemp (usb_utr_t * ptr, uint16_t pipeno)
{
    ...
    {
#if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
        ptr->ipp->BEMPSTS = (uint16_t) ((uint16_t) (~(1 << pipeno)) & BEMPSTS_MASK);
#endif                                 /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
    }
}
