...
void usb_cpu_usbint_init (uint8_t ip_type, usb_cfg_t const * const cfg)
{
    if (USB_IP0 == ip_type)
    {
         ...
         */
#if ((USB_CFG_DTC == USB_CFG_ENABLE) || (USB_CFG_DMA == USB_CFG_ENABLE))
        R_BSP_IrqCfgEnable(cfg->irq_d0, (uint32_t) cfg->ipl_d0, (void *) cfg); /* Enable D0FIFO interrupt */

        R_BSP_IrqCfgEnable(cfg->irq_d1, (uint32_t) cfg->ipl_d1, (void *) cfg); /* Enable D1FIFO interrupt */
#endif  /* ((USB_CFG_DTC == USB_CFG_ENABLE) || (USB_CFG_DMA == USB_CFG_ENABLE)) */

        R_BSP_IrqCfgEnable(cfg->irq_r, (uint32_t) cfg->ipl_r, (void *) cfg);   /* USBR enable */

        R_BSP_IrqCfgEnable(cfg->irq, (uint32_t) cfg->ipl, (void *) cfg);       /* USBI enable */
    ...
    if (ip_type == USB_IP1)
    {
         ...
         */
 #if ((USB_CFG_DTC == USB_CFG_ENABLE) || (USB_CFG_DMA == USB_CFG_ENABLE))
        R_BSP_IrqCfgEnable(cfg->hsirq_d0, (uint32_t) cfg->hsipl_d0, (void *) cfg); /* Enable D0FIFO interrupt */

        R_BSP_IrqCfgEnable(cfg->hsirq_d1, (uint32_t) cfg->hsipl_d1, (void *) cfg); /* Enable D1FIFO interrupt */
 #endif /* ((USB_CFG_DTC == USB_CFG_ENABLE) || (USB_CFG_DMA == USB_CFG_ENABLE)) */

        R_BSP_IrqCfgEnable(cfg->hsirq, (uint32_t) cfg->hsipl, (void *) cfg);       /* USBIR enable */
 #if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
...
void usb_cpu_int_enable (void)
{
    ...
    if (USB_NULL != g_p_usb_cfg_ip0)
    {
        ...
        {
            R_BSP_IrqCfgEnable(g_p_usb_cfg_ip0->irq, (uint32_t) g_p_usb_cfg_ip0->ipl, g_p_usb_cfg_ip0); /* USBI enable */
    ...
    if (USB_NULL != g_p_usb_cfg_ip1)
    {
        ...
        {
            R_BSP_IrqCfgEnable(g_p_usb_cfg_ip1->hsirq, (uint32_t) g_p_usb_cfg_ip1->hsipl, g_p_usb_cfg_ip1); /* USBIR enable */
...
uint16_t usb_chattaring (uint16_t * syssts)
{
    ...
    while (1)
    {
        lnst[0] = (uint16_t) ((*syssts) & USB_LNST);
        usb_cpu_delay_xms((uint16_t) 1); /* 1ms wait */
        lnst[1] = (uint16_t) ((*syssts) & USB_LNST);
        usb_cpu_delay_xms((uint16_t) 1); /* 1ms wait */
        lnst[2] = (uint16_t) ((*syssts) & USB_LNST);
