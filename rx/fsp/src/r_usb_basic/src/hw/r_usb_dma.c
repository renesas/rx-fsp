...
void usb_cstd_dma_send_start (usb_utr_t * ptr, uint16_t pipe, uint16_t useport)
{
    ...
    if (0U != dma_size)
    {
        ...
        else
        {
            ...
            {
                /* fraction size(1-3) */
                g_usb_cstd_dma_fraction_size[ip][ch] =
                    (uint8_t) (g_usb_cstd_dma_size[ip][ch] & USB_BIT_MBW16);
            }
            ...
            else
            {
                ...
                /* fraction size(1-3) */
                g_usb_cstd_dma_fraction_size[ip][ch] =
                    (uint8_t) (g_usb_cstd_dma_size[ip][ch] & USB_BIT_MBW32);
  #else                                /* defined (USB_HIGH_SPEED_MODULE) */
                /* fraction size(1-3) */
...
void usb_cstd_dma_send_restart (usb_utr_t * ptr, uint32_t src, uint32_t data_size, uint8_t pipe)
{
    ...
    {
        hw_usb_write_pipesel(ptr, (uint16_t) pipe);
        usb_dir = hw_usb_read_pipecfg(ptr);
        usb_dir = (uint16_t) (usb_dir & USB_DIRFIELD);
        if (0 == usb_dir)
        {
            useport = USB_D0USE;
        }
    ...
    if (false == usb_check_use_usba_module(ptr))
    {
        usb_cstd_chg_curpipe(ptr, (uint16_t) pipe, useport, USB_FALSE);
    }
...
void usb_cstd_dma_send_continue (usb_utr_t * ptr, uint16_t useport)
{
    ...
    if (p_data_cnt)
    {
        ...
        else
        {
            ...
            if (0U == (*p_data_cnt))
            {
                ...
                if ((hw_usb_read_pipectr(ptr, (uint16_t) pipe) & USB_INBUFM) != USB_INBUFM)
                {
                    ...
                    {
 #if ((USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST)
                        usb_hstd_data_end(ptr, (uint16_t) pipe, (uint16_t) USB_DATA_NONE);
 #endif                                /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
                    }
                    else
                    {
 #if ((USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_PERI)
                        usb_pstd_data_end((uint16_t) pipe, (uint16_t) USB_DATA_NONE, ptr);
 #endif                                /* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_REPI */
                    }
                }
                else
            ...
            else
            {
                ...
                p_src_adr = (uint8_t *) (uintptr_t) ((uint32_t) p_data_ptr + g_usb_cstd_dma_size[ip][channel]);
                ...
                if (USB_IP0 == ip)
                {
                    g_usb_cstd_dma_fraction_size[ip][channel] =
                        (uint8_t) (g_usb_cstd_dma_size[ip][channel] & USB_BIT_MBW16); /* fraction size(1-3) */
  ...
  #if defined(USB_HIGH_SPEED_MODULE)
                    g_usb_cstd_dma_fraction_size[ip][channel] =
                        (uint8_t) (g_usb_cstd_dma_size[ip][channel] & USB_BIT_MBW32);                             /* fraction size(1-3) */
                ...
                if (0 != dma_size)
                {
                    ...
                    /* DMA0 1byte trans */
                    usb_cstd_dma_send_restart(ptr, (uint32_t) p_src_adr, (uint32_t) dma_size, pipe);
                }
                else
                {
                    cpu_write = true;  /* Set flag for CPU FIFO Write */
    ...
    if (true == cpu_write)
    {
        if (USB_MODE_HOST == g_usb_usbmode[ip])
        {
            ...
            gp_usb_hstd_data_ptr[ptr->ip][pipe] = (uint8_t *) (uintptr_t) g_usb_cstd_dma_fraction_adr[ip][channel]; /* fraction data address */
            usb_hstd_buf_to_fifo(ptr, (uint16_t) pipe, useport);
 #endif /* (USB_CFG_MODE & USB_CFG_HOST) == USB_CFG_HOST */
        }
        else
        {
            ...
            usb_pstd_buf_to_fifo((uint16_t) pipe, useport, ptr);
 #endif /* (USB_CFG_MODE & USB_CFG_PERI) == USB_CFG_REPI */
        }
...
void usb_cstd_dma_clear_ir (usb_utr_t * ptr, uint16_t use_port)
{
    ...
    if (USB_IP0 == ip)
    {
        if (USB_D0USE == use_port)
        {
 #if defined(VECTOR_NUMBER_USB0_D0FIFO)
            irq = (IRQn_Type) VECTOR_NUMBER_USB0_D0FIFO;
            R_BSP_IrqStatusClear(irq);
 #endif                                /* defined(VECTOR_NUMBER_USB0_D0FIFO) */
        }
        else
        {
 #if defined(VECTOR_NUMBER_USB0_D1FIFO)
            irq = (IRQn_Type) VECTOR_NUMBER_USB0_D1FIFO;
            R_BSP_IrqStatusClear(irq);
 #endif                                /* defined(VECTOR_NUMBER_USB0_D1FIFO) */
        }
    }
    ...
    else
    {
        ...
        if (USB_D0USE == use_port)
        {
   #if defined(VECTOR_NUMBER_USBHS_D0FIFO)
            irq = (IRQn_Type) VECTOR_NUMBER_USBHS_D0FIFO;
            R_BSP_IrqStatusClear(irq);
   #endif                              /* defined(VECTOR_NUMBER_USBHS_D0FIFO) */
        }
        else
        {
   #if defined(VECTOR_NUMBER_USBHS_D1FIFO)
            irq = (IRQn_Type) VECTOR_NUMBER_USBHS_D1FIFO;
            R_BSP_IrqStatusClear(irq);
   #endif                              /* defined(VECTOR_NUMBER_USBHS_D1FIFO) */
        }
  #endif                               /* defined (USB_HIGH_SPEED_MODULE) */
...
void usb_cstd_dma_send_complete (uint8_t ip_no, uint16_t use_port)
{
 ...
 #if defined(USB_HIGH_SPEED_MODULE)
  #if defined(VECTOR_NUMBER_USBHS_D1FIFO)
        p_cfg = (usb_cfg_t *) R_FSP_IsrContextGet((IRQn_Type) VECTOR_NUMBER_USBHS_D1FIFO);
  #endif                               /* defined(VECTOR_NUMBER_USBHS_D1FIFO) */
 #else /* defined(USB_HIGH_SPEED_MODULE) */
  #if defined(VECTOR_NUMBER_USB0_D1FIFO)
        p_cfg = (usb_cfg_t *) R_FSP_IsrContextGet((IRQn_Type) VECTOR_NUMBER_USB0_D1FIFO);
  #endif                               /* defined(VECTOR_NUMBER_USB0_D1FIFO) */
 #endif                                /* defined(USB_HIGH_SPEED_MODULE) */
    }
    else
...
/* DMA test code start */
        ...
        if (USB_IP1 == ip)
        {
            ptr->p_transfer_rx->p_cfg->p_info->p_src = (void *) (uintptr_t) hw_usb_get_dxfifo_adr(ptr,
                                                                                                  useport,
        ...
        ptr->p_transfer_rx->p_cfg->p_info->p_dest     = (void *) (uintptr_t) des_addr;
...
void usb_cstd_dma_send_setting (usb_utr_t * ptr, uint32_t src_adr, uint16_t useport, uint32_t transfer_size)
{
    ...
    if (USB_NULL != ptr)
    {
        ...
        {
            ptr->p_transfer_tx->p_cfg->p_info->p_dest = (void *) (uintptr_t) hw_usb_get_dxfifo_adr(ptr,
                                                                                                   useport,
                                                                                                   USB_FIFO_ACCESS_TYPE_32BIT);
...
/* DMA test code start */
         ...
         * to 0.
         * Transfer Request source is software. *//* Set Transfer data configuration. */
        ptr->p_transfer_tx->p_cfg->p_info->p_src      = (void *) (uintptr_t) src_adr;
