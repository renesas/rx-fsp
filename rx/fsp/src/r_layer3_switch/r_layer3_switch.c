...
#define LAYER3_SWITCH_ETHA_REG_SIZE                               (0x2000UL)
#define LAYER3_SWITCH_RMAC_REG_SIZE                               (0x2000UL)
...
#define R_MFWD_FWSEQNRC_SEQNR_Msk                                 (0xffffffffUL)
#define R_MFWD_FWPMTRFC0_MTRCM_Msk                                (0xffff0000UL)
...
#define LAYER3_SWITCH_CBS_INTERFERENCE_SIZE_OFFSET                (20U)
...
#if BSP_FEATURE_ESWM_SUPPORT_L2_L3_FORWARDING
...
static uint32_t layer3_switch_bytes_swap_32_func (uint32_t x)
{
 ...
 #define LAYER3_SWITCH_BYTES_SWAP_32(x)    layer3_switch_bytes_swap_32_func(x)
...
#if BSP_FEATURE_ESWM_SUPPORT_CBS
...
static fsp_err_t r_layer3_switch_psfp_meter_filter_init(layer3_switch_instance_ctrl_t               * p_instance_ctrl,
                                                        ...
                                                        layer3_switch_table_entry_cfg_t const * const p_table_entry_cfg);
static bool r_layer3_switch_psfp_is_msdu_filter_hw_id_used(layer3_switch_instance_ctrl_t const * p_instance_ctrl,
                                                           uint32_t                              msdu_filter_hw_id);
static bool r_layer3_switch_psfp_is_meter_filter_hw_id_used(layer3_switch_instance_ctrl_t const * p_instance_ctrl,
                                                            uint32_t                              meter_filter_hw_id);
...
#if BSP_FEATURE_ESWM_SUPPORT_FRAME_PREEMPTION
...
fsp_err_t R_LAYER3_SWITCH_Open (ether_switch_ctrl_t * const p_ctrl, ether_switch_cfg_t const * const p_cfg)
{
    ...
    /* Set endian mode of ESWM bus */
#if defined(__BIG)

    /* Big endian */
    R_ESWM->ETHBUSCR_b.ENDIAN = 1;
#else

    /* Little endian */
    R_ESWM->ETHBUSCR_b.ENDIAN = 0;
#endif
    ...
    R_MFWD->FWPC01_b.DDE = 0x1;
#if BSP_FEATURE_ETHER_NUM_CHANNELS > 1
    R_MFWD->FWPC11_b.DDE = 0x1;
#endif
    R_MFWD->FWPC21_b.DDE = 0x1;
    ...
    if (FSP_SUCCESS != phy_err)
    {
        ...
        R_MFWD->FWPC01_b.DDE = 0;
#if BSP_FEATURE_ETHER_NUM_CHANNELS > 1
        R_MFWD->FWPC11_b.DDE = 0;
#endif
        R_MFWD->FWPC21_b.DDE = 0;
    ...
    R_BSP_IrqCfgEnable(p_instance_ctrl->p_cfg->irq, (uint32_t) p_instance_ctrl->p_cfg->ipl, p_instance_ctrl);
    if (p_extend->etha_error_irq_port_0 >= 0)
    {
        R_BSP_IrqCfgEnable(p_extend->etha_error_irq_port_0, (uint32_t) p_extend->etha_error_ipl_port_0,
                           p_instance_ctrl);
    }

    if (p_extend->etha_error_irq_port_1 >= 0)
    {
        R_BSP_IrqCfgEnable(p_extend->etha_error_irq_port_1, (uint32_t) p_extend->etha_error_ipl_port_1,
                           p_instance_ctrl);
    }
                                                 ...
                                                 const layer3_switch_descriptor_queue_cfg_t * const p_queue_cfg)
{
    ...
    else
    {
        ...
        p_instance_ctrl->p_descriptor_queue_list[queue_index].ptr_h = (uint8_t)
                                                                      ((LAYER3_SWITCH_QUEUE_ADDRESS_UPPER_MASK &
                                                                        (uint64_t) (uintptr_t) p_queue_cfg->
                                                                        p_descriptor_array) >>
                                                                       LAYER3_SWITCH_QUEUE_ADDRESS_UPPER_POSITION);
#if defined(__BIG)

        /* Endian conversion must be applied to ptr_l before writing. */
        p_instance_ctrl->p_descriptor_queue_list[queue_index].ptr_l =
            LAYER3_SWITCH_BYTES_SWAP_32(
        ...
        *p_gwca_gwdcc_reg =
            ...
            (uint32_t) (p_queue_cfg->descriptor_format << R_GWCA0_GWDCC0_EDE_Pos) |
            (uint32_t) (p_queue_cfg->rx_timestamp_storage << R_GWCA0_GWDCC0_ETS_Pos) |
            (uint32_t) (p_queue_cfg->type << R_GWCA0_GWDCC0_DQT_Pos);
        ...
        *p_gwca_gwdie_reg = (*p_gwca_gwdie_reg) | (uint32_t) (1 << (queue_index % LAYER3_SWITCH_REGISTER_SIZE));
...
fsp_err_t R_LAYER3_SWITCH_StartDescriptorQueue (ether_switch_ctrl_t * const p_ctrl, uint32_t queue_index)
{
    ...
    *p_gwca_gwdcc_reg |= R_GWCA0_GWDCC0_BALR_Msk;
    ...
    if (LAYER3_SWITCH_QUEUE_TYPE_TX == p_instance_ctrl->p_queues_status[queue_index].p_queue_cfg->type)
    {
        ...
        *p_gwca_gwtrc_reg = (*p_gwca_gwtrc_reg) | (uint32_t) (1 << (queue_index % LAYER3_SWITCH_REGISTER_SIZE));
    }
    else
    {
        /* When RX queue. */
 ...
 * @retval  FSP_ERR_NOT_INITIALIZED      The forwarding table has not been initialized.
                                         ...
                                         layer3_switch_table_entry_cfg_t const * const p_entry_cfg)
{
    ...
    FSP_PARAMETER_NOT_USED(p_target_frame);
    FSP_PARAMETER_NOT_USED(p_entry_cfg);
                                                ...
                                                layer3_switch_psfp_error_status_bitmask_t bitmasks)
{
    ...
    if (0 != bitmasks.meter_filter_bitmask)
    {
        ...
        R_MFWD->FWEIS5 = write_value;
    }

#else
    FSP_PARAMETER_NOT_USED(p_ctrl);
    FSP_PARAMETER_NOT_USED(bitmasks);
    err = FSP_ERR_UNSUPPORTED;
#endif
                                          ...
                                          layer3_switch_table_cfg_t const * const p_table_cfg)
{
    ...
    {
        r_layer3_switch_enable_mac_table_aging(p_table_cfg->mac_entry_aging_time_sec);
    }
 ...
 * @retval  FSP_ERR_NOT_INITIALIZED      The forwarding table has not been initialized.
 * @retval  FSP_ERR_NOT_FOUND            The entry isn't found.
 * @retval  FSP_ERR_INVALID_ARGUMENT     Invalid configuration value.
 * @retval  FSP_ERR_UNSUPPORTED          Function is not supported.
 **********************************************************************************************************************/
...
#else
    FSP_PARAMETER_NOT_USED(p_ctrl);
    FSP_PARAMETER_NOT_USED(p_table);
    err = FSP_ERR_UNSUPPORTED;
#endif
                                        ...
                                        layer3_switch_tas_cfg_t   * p_tas_cfg)
{
...
#if LAYER3_SWITCH_CFG_TAS_ENABLE && BSP_FEATURE_ESWM_SUPPORT_TAS
    layer3_switch_instance_ctrl_t * p_instance_ctrl = (layer3_switch_instance_ctrl_t *) p_ctrl;
    ...
    tas_entry_addr = (uint8_t) p_etha_reg->EATASC_b.TASCA;
    for (uint8_t i = 0; i < BSP_FEATURE_ESWM_ETHA_IPV_QUEUE_NUM; i++)
    {
        p_eatasenc_reg  = &p_etha_reg->EATASENC0 + i;
        *p_eatasenc_reg =
            ((uint32_t) (p_tas_cfg->gate_cfg_list[i].tas_entry_num - 1) & R_ETHA0_EATASCTENC_TASCTAEN_Msk);
    ...
    for (uint8_t i = 0; i < BSP_FEATURE_ESWM_ETHA_IPV_QUEUE_NUM; i++)
    {
             ...
             learn_port_cnt++)
        {
            ...
            p_etha_reg->EATASGL0_b.TASGAL = (uint8_t) (tas_entry_addr + learn_count);
            p_etha_reg->EATASGL1          =
...
static void r_layer3_switch_reset_coma (void)
{
    ...
    R_COMA->RCEC = R_COMA_RCEC_RCE_Msk | R_COMA_RCEC_ACE0_Msk | R_COMA_RCEC_ACE1_Msk | R_COMA_RCEC_ACE2_Msk;
...
static void r_layer3_switch_initialize_linkfix_table (layer3_switch_instance_ctrl_t * p_instance_ctrl)
{
    ...
    p_instance_ctrl->p_gwca_reg->GWDCBAC0 = (uint32_t)
                                            ((LAYER3_SWITCH_QUEUE_ADDRESS_UPPER_MASK &
                                              (uint64_t) (uintptr_t) p_instance_ctrl->p_descriptor_queue_list) >>
                                             LAYER3_SWITCH_QUEUE_ADDRESS_UPPER_POSITION);
                                                                    ...
                                                                    uint32_t                        descriptor_index)
{
    ...
    /* Apply endian conversion before accessing descriptor pointer. */
    p_queue_head_descriptor =
        (layer3_switch_descriptor_t *) (LAYER3_SWITCH_BYTES_SWAP_32((uintptr_t) p_linkfix_descriptor->ptr_l));
#else
    p_queue_head_descriptor = (layer3_switch_descriptor_t *) ((uintptr_t) p_linkfix_descriptor->ptr_l);
#endif
    ...
    uint32_t                        queue_index)
{
    ...
    return (layer3_switch_descriptor_t *) ((uintptr_t) p_instance_ctrl->p_gwca_reg->GWAARSR1);
                                                        ...
                                                        uint32_t                        queue_index)
{
    ...
    if (LAYER3_SWITCH_QUEUE_TYPE_TX == p_instance_ctrl->p_queues_status[queue_index].p_queue_cfg->type)
    {
        ...
        if (*(&(p_instance_ctrl->p_gwca_reg->GWTRC0) + (uint32_t) (queue_index / LAYER3_SWITCH_REGISTER_SIZE)) &
            (uint32_t) (1 << (queue_index % LAYER3_SWITCH_REGISTER_SIZE)))
        {
            active = true;
        }
    }
    else                               /* When RX descriptor queue. */
    {
        ...
        else
        {
            ...
            {
                p_current_descriptor = (layer3_switch_descriptor_t *) ((uintptr_t) p_current_descriptor->basic.ptr_l);
            }
                                            ...
                                            layer3_switch_port_cfg_t const * const p_port_cfg)
{
    ...
    p_mfwd_fwpbfc_reg =
        (uint32_t *) ((uintptr_t) &(R_MFWD->FWPBFC0) + ((uint32_t) port * LAYER3_SWITCH_FWPBFC_REGISTER_OFFSET));
                                                       ...
                                                       uint8_t                                           port)
{
    uint32_t * p_mfwd_fwpc0_reg =
        (uint32_t *) ((uintptr_t) (&R_MFWD->FWPC00) + (uint32_t) (port * LAYER3_SWITCH_PORT_CONFIG_REGISTER_OFFSET));
 ...
 *
 * @param[in]   p_entry_cfg                 Pointer to the entry configuration
 * @param[in]   p_target_frame              Pointer to the target frame
 *
 * @retval      FSP_SUCCESS                 Learning completed successfully.
 * @retval      FSP_ERR_WRITE_FAILED        Entry learning failed.
 * @retval      FSP_ERR_OVERFLOW            Unsecure entry overflow.
 * @retval      FSP_ERR_INVALID_ARGUMENT    Destination or source MAC address isn't set.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_learn_mac_entry (layer3_switch_frame_filter_t const * const    p_target_frame,
                                                  layer3_switch_table_entry_cfg_t const * const p_entry_cfg)
{
 ...
 * @param[in]   p_entry_cfg                 Pointer to an entry configuration
 *
 * @retval      FSP_SUCCESS                 Searching completed successfully.
 * @retval      FSP_ERR_INVALID_POINTER     Pointer to a target frame is NULL.
 * @retval      FSP_ERR_NOT_FOUND           Destination or source MAC address isn't set.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_search_mac_entry (layer3_switch_frame_filter_t const * const p_target_frame,
                                                   layer3_switch_table_entry_cfg_t * const    p_entry_cfg)
{
    ...
    if (0 == R_MFWD->FWMACTSR0_b.MACSNF)
    {
        ...
        p_entry_cfg->security_enable                 = (bool) R_MFWD->FWMACTSR0_b.MACSLS;
        p_entry_cfg->mac.dinamic_entry               = (bool) R_MFWD->FWMACTSR0_b.MACDES;
 ...
 *
 * @retval      FSP_SUCCESS                 Reading completed successfully.
 * @retval      FSP_ERR_INVALID_ARGUMENT    MAC address isn't given.
 * @retval      FSP_ERR_NOT_FOUND           The entry isn't found.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_read_mac_entry (uint16_t offset, layer3_switch_table_entry_t * p_entry)
{
    ...
    {
        p_entry->entry_cfg.security_enable   = (bool) R_MFWD->FWMACTRR1_b.MACSLR;
        p_entry->entry_cfg.mac.dinamic_entry = (bool) R_MFWD->FWMACTRR1_b.MACDER;
 ...
 *
 * @retval      FSP_SUCCESS                 Learning completed successfully.
 * @retval      FSP_ERR_WRITE_FAILED        MAC address isn't given.
 * @retval      FSP_ERR_OVERFLOW            Unsecure entry overflow.
 * @retval      FSP_ERR_INVALID_ARGUMENT    S-TAG and C-TAG ID are invalid.
 * @retval      FSP_ERR_INVALID_MODE        Using VLAN table in non-VLAN mode.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_learn_vlan_entry (layer3_switch_frame_filter_t const * const    p_target_frame,
                                                   layer3_switch_table_entry_cfg_t const * const p_entry_cfg)
{
 ...
 *
 * @retval      FSP_SUCCESS                 Searching completed successfully
 * @retval      FSP_ERR_NOT_FOUND           No entry has been found
 * @retval      FSP_ERR_INVALID_ARGUMENT    S-TAG and C-TAG ID are invalid
 * @retval      FSP_ERR_INVALID_MODE        Using VLAN table in non-VLAN mode.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_search_vlan_entry (layer3_switch_frame_filter_t const * const p_target_frame,
                                                    layer3_switch_table_entry_cfg_t * const    p_entry_cfg)
{
    ...
    if (0 == R_MFWD->FWVLANTSR0_b.VLANSNF)
    {
        ...
        p_entry_cfg->security_enable         = (bool) R_MFWD->FWVLANTSR0_b.VLANSLS;
...
static fsp_err_t r_layer3_switch_read_vlan_entry (uint16_t offset, layer3_switch_table_entry_t * p_entry)
{
    ...
    p_entry->target_frame.vlan_c_tag.id = (uint16_t) (LAYER3_SWITCH_VLAN_ENTRY_MAX_NUM & offset);
 ...
 * @param[in]   p_entry_cfg                 Pointer to an entry configuration
 * @param[in]   p_target_frame              Pointer to the target frame
 * @param[in]   p_instance_ctrl             Pointer to LAYER3_SWITCH control block
 *
 * @retval      FSP_SUCCESS                 Learning completed successfully.
 * @retval      FSP_ERR_OVERFLOW            L3 rounting number overflow.
 * @retval      FSP_ERR_WRITE_FAILED        Entry learning failed.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_learn_l3_entry (layer3_switch_instance_ctrl_t               * p_instance_ctrl,
                                                 layer3_switch_frame_filter_t const * const    p_target_frame,
    ...
    if (err == FSP_SUCCESS)
    {
        ...
        if (NULL != p_entry_cfg->layer3.p_update_configs)
        {
            if (0 < p_entry_cfg->layer3.number_of_configs)
            {
                ...
                routing_number = (uint8_t) (p_instance_ctrl->l3_routing_number - 1);
            }
 ...
 *
 * @retval      FSP_SUCCESS                 Searching completed successfully.
 * @retval      FSP_ERR_NOT_FOUND           The entry for the stream ID is not found.
 **********************************************************************************************************************/
                                                  ...
                                                  layer3_switch_table_entry_cfg_t * const    p_entry_cfg)
{
    ...
    {
        /* Copy entry values. */
        routing_number                 = (uint8_t) R_MFWD->FWLTHTSR3_b.LTHRNS;
        ...
        p_entry_cfg->security_enable = (bool) R_MFWD->FWLTHTSR0_b.LTHSLS;
        l3_update_enable             = (bool) R_MFWD->FWLTHTSR3_b.LTHRVS;
    }
    else
    {
 ...
 *
 * @retval      FSP_SUCCESS             Learning update completed successfully.
 * @retval      FSP_ERR_WRITE_FAILED    Layer3 update learning fail.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_learn_l3_update (layer3_switch_instance_ctrl_t          * p_instance_ctrl,
                                                  layer3_switch_l3_update_config_t * const p_config)
{
    ...
    else
    {
        ...
        p_instance_ctrl->l3_routing_number = (uint8_t) (p_instance_ctrl->l3_routing_number + 1);
    }
 ...
 *
 * @retval      FSP_SUCCESS                 Searching update completed successfully.
 * @retval      FSP_ERR_NOT_FOUND           Layer3 update entry is not found.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_search_l3_update (uint8_t routing_number, layer3_switch_l3_update_config_t * p_config)
{
    fsp_err_t err = FSP_SUCCESS;
    ...
    if (0 == R_MFWD->FWL23URRR0_b.L23UREF)
    {
        ...
        p_config->vlan_c_tag.id     = (uint16_t) (R_MFWD->FWL23URRR3_b.L23UCVIDR);
        p_config->vlan_c_tag.pcp    = (uint16_t) (R_MFWD->FWL23URRR3_b.L23UCPCPR);
        p_config->vlan_c_tag.dei    = (uint16_t) (R_MFWD->FWL23URRR3_b.L23UCDEIR);
        p_config->vlan_s_tag.id     = (uint16_t) (R_MFWD->FWL23URRR3_b.L23USVIDR);
        p_config->vlan_s_tag.pcp    = (uint16_t) (R_MFWD->FWL23URRR3_b.L23USPCPR);
        p_config->vlan_s_tag.dei    = (uint16_t) (R_MFWD->FWL23URRR3_b.L23USDEIR);
 ...
 *
 * @retval      FSP_SUCCESS                 Enable aging completed successfully.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_enable_mac_table_aging (uint32_t aging_time)
{
 ...
 *
 * @retval      FSP_SUCCESS                 Extracting VLAN ID completed successfully.
 * @retval      FSP_ERR_INVALID_MODE        Using VLAN table in non-VLAN mode.
 * @retval      FSP_ERR_INVALID_ARGUMENT    S-TAG and C-TAG ID are invalid.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_extract_vlan_id (layer3_switch_frame_filter_t const * const p_target_frame,
                                                  ...
                                                  uint8_t                                           port)
{
    ...
    vcc_reg_value = (uint32_t) (p_port_cfg->vlan_ingress_mode << R_ETHA0_EAVCC_VIM_Pos) |
                    (uint32_t) (p_port_cfg->vlan_egress_mode << R_ETHA0_EAVCC_VEM_Pos);
 ...
 *
 * @param[in]   p_filter_cfg                Configure IPv4/IPv6/L2 stream filter completed successfully.
 *
 * @retval      FSP_SUCCESS                 The erase completed successfully.
 * @retval      FSP_ERR_INVALID_POINTER     Filter configuration isn't set.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_configure_stream_filter (layer3_switch_l3_stream_filter_cfg_t const * p_filter_cfg)
{
    ...
    R_MFWD->FWIP6OC =
        (uint32_t) ((R_MFWD_FWIP6OC_IP6IPO1_Msk &
                    ...
                    (R_MFWD_FWIP6OC_IP6IPO0_Msk &
                     (uint32_t) (p_filter_cfg->ipv6_address0.offset << R_MFWD_FWIP6OC_IP6IPO0_Pos)) |
                    (R_MFWD_FWIP6OC_IP6IPOM0_Msk &
                     (uint32_t) (p_filter_cfg->ipv6_address0.direction << R_MFWD_FWIP6OC_IP6IPOM0_Pos)));

    /* Set IPv4/IPv6 hash equation. */
    R_MFWD->FWSFHEC = (uint32_t) ((1 << R_MFWD_FWSFHEC_IP4HE0_Pos) | (1 << R_MFWD_FWSFHEC_IP6HE0_Pos));
...
uint16_t r_layer3_switch_calculate_l3_hash (layer3_switch_frame_filter_t const * p_frame)
{
    ...
    else
    {
        ...
        else
        {
            ...
            R_MFWD->FWSHCR4 = (uint32_t) (1 << R_MFWD_FWSHCR4_SHCFF_Pos) | p_frame->protocol;
        ...
        /* Read hash value. */
        hash_value = (uint16_t) (R_MFWD->FWSHCRR_b.SHCR);
    }
...
uint8_t r_layer3_switch_calculate_l3_format_code (layer3_switch_frame_filter_t const * p_frame)
{
    ...
    return (uint8_t) (ip_version_offset + ip_protocol_offset);
 ...
 *
 * @retval      FSP_SUCCESS         Calculating and updating a layer3 stream ID completed successfully.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_calculate_l3_stream_id (layer3_switch_frame_filter_t const * p_frame,
                                                         layer3_switch_stream_id_t          * p_stream_id)
{
 ...
 *
 * @param[in]   p_instance_ctrl         Pointer to an instance control
 * @param[out]  routing_number          Index of a target layer3 entry.
 * @param[in]   p_update_cfg            Pointer to the update configuration.
 *
 * @retval      FSP_SUCCESS             L3 update remapping configuration completed successfully.
 * @retval      FSP_ERR_WRITE_FAILED    Remapping configuration failed.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_remapping_l3_update (layer3_switch_instance_ctrl_t    * p_instance_ctrl,
                                                      uint32_t                           routing_number,
                                                      layer3_switch_l3_update_config_t * p_update_cfg)
{
    ...
    if (FSP_SUCCESS == err)
    {
        ...
        *p_mfwd_fwl23urmc_reg = (routing_number << R_MFWD_FWL23URMC0_RMRN_Pos) |
        ...
        /* Increment indexes. */
        p_instance_ctrl->l3_routing_number   = (uint8_t) (p_instance_ctrl->l3_routing_number + 1);
        p_instance_ctrl->l3_remapping_number = (uint8_t) (p_instance_ctrl->l3_remapping_number + 1);
    }
                                           ...
                                           layer3_switch_cbs_cfg_t const       * p_cbs_cfg)
{
    ...
    eswclk_frequency = R_BSP_SourceClockHzGet((fsp_priv_source_clock_t) (R_SYSTEM->ESWCKCR_b.ESWCKSEL)) /
                       R_FSP_ClockDividerGet((uint32_t) R_SYSTEM->ESWCKDIVCR_b.ESWCKDIV);
    ...
    for (uint8_t i = 0; i < BSP_FEATURE_ESWM_ETHA_IPV_QUEUE_NUM; i++)
    {
        if (0 != p_cbs_cfg->band_width_list[i])
        {
            ...
            civ_raw_value      = (double) (link_speed / LAYER3_SWITCH_CBS_BITS_PER_BYTE) * bandwidth_fraction /
                                 (double) eswclk_frequency;
            civ_int_value =
            ...
            *p_eacaulc_reg        = 0 *
            ...
            enable_cbs_bitmask |= (uint8_t) (1 << i);
        }
    }
                                                                 ...
                                                                 uint8_t const * p_max_burst_num_list)
{
    ...
    for (uint8_t i = (uint8_t) (queue_number + 1); i < BSP_FEATURE_ESWM_ETHA_IPV_QUEUE_NUM; i++)
    {
        ...
        {
            queue_interference_size_high +=
                ((p_max_burst_num_list[i] + (uint32_t) 1) * LAYER3_SWITCH_MAXIMUM_FRAME_SIZE +
 ...
 *
 * @param[in]   p_ctrl                          Pointer to a control structure
 * @param[in]   p_queue_cfg                     Pointer to the queue configuration
 * @param[out]  p_ts_descriptor_queue_index     Pointer to the index of TS descriptor queue
 *
 * @retval      FSP_SUCCESS                     Creating TS descriptor queue completed successfully.
 * @retval      FSP_ERR_OVERFLOW                Index of TS descriptor queue overflows maximum value.
 ***********************************************************************************************************************/
static fsp_err_t r_layer3_switch_create_tx_timestamp_queue (ether_switch_ctrl_t * const                        p_ctrl,
                                                            const layer3_switch_descriptor_queue_cfg_t * const p_queue_cfg,
                                                            uint32_t * const                                   p_ts_descriptor_queue_index)
{
    ...
    /* Endian conversion must be applied to ptr_l before writing. */
    p_ts_descriptor[(p_queue_cfg->array_length -
                     1)].ts_reception_descriptor_basic.ptr_l = LAYER3_SWITCH_BYTES_SWAP_32((uintptr_t) p_ts_descriptor);
#else
    p_ts_descriptor[(p_queue_cfg->array_length -
    ...
    p_gwtdcac1_reg = (volatile uint32_t *) ((uint8_t *) &p_instance_ctrl->p_gwca_reg->GWTDCAC01 +
                                            (LAYER3_SWITCH_TS_DESCRIPTOR_QUEUE_REGISTOR_OFFSET *
                                             ts_descriptor_queue_index));
 ...
 *
 * @param[in]     p_instance_ctrl      Pointer to a instance control
 * @param[in]     p_frer_cfg           Pointer to FRER configuration
 *
 * @retval      FSP_SUCCESS              FRER initialization completed successfully.
 * @retval      FSP_ERR_INVALID_POINTER  FRER configuration pointer is NULL.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_frer_init (layer3_switch_instance_ctrl_t  * p_instance_ctrl,
                                            layer3_switch_frer_cfg_t const * p_frer_cfg)
{
 ...
 * @param[in]     p_instance_ctrl       Pointer to a instance control
 * @param[in]     p_frer_entry          Pointer to a FRER entry.
 * @param[in]     p_sequence_recovery   Pointer to a sequence recovery entry.
 * @param[in]     sequence_recovery_id  ID of the target sequence recovery entry.
 *
 * @retval      FSP_SUCCESS             Learning FRER entry completed successfully.
 * @retval      FSP_ERR_WRITE_FAILED    Learning FRER entry failed.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_learn_frer_entry (layer3_switch_instance_ctrl_t    * p_instance_ctrl,
                                                   layer3_switch_frer_entry_t * const p_frer_entry,
                                                   layer3_switch_frer_entry_t * const p_sequence_recovery,
                                                   uint32_t                           sequence_recovery_id)
{
    ...
    else
    {
        ...
        {
            p_instance_ctrl->frer_sequence_recovery_status[sequence_recovery_id &
                                                           LAYER3_SWITCH_FRER_ENTRY_NUM_BITMASK].learned = true;
            p_instance_ctrl->frer_sequence_recovery_status[sequence_recovery_id &
                                                           LAYER3_SWITCH_FRER_ENTRY_NUM_BITMASK].frer_entry_index =
 ...
 * @param[in]     p_instance_ctrl       Pointer to a instance control
 * @param[in]     p_frer_entry_cfg      Pointer to a FRER entry configuration.
 *
 * @retval      FSP_SUCCESS             Learning FRER individual recovery entry completed successfully.
 * @retval      FSP_ERR_WRITE_FAILED    Learning FRER individual recovery entry failed.
 **********************************************************************************************************************/
static fsp_err_t r_layer3_switch_learn_frer_individual_recovery (layer3_switch_instance_ctrl_t        * p_instance_ctrl,
                                                                 layer3_switch_frer_entry_cfg_t * const p_frer_entry_cfg)
{
...
/*******************************************************************************************************************//**
 * Check whether a hardware MSDU filter is already assigned to one of the logical MSDU filter IDs.
 *
 * psfp_msdu_filter_info_list[] is indexed by the logical MSDU filter ID, so hardware occupancy
 * lives in the msdu_filter_hw_id field and not in the subscript.
 *
 * @param[in] p_instance_ctrl          Pointer to the instance control structure
 * @param[in] msdu_filter_hw_id        Hardware MSDU filter ID to check
 *
 * @return  true if the hardware MSDU filter ID is already assigned to a logical MSDU filter
 **********************************************************************************************************************/
static bool r_layer3_switch_psfp_is_msdu_filter_hw_id_used (layer3_switch_instance_ctrl_t const * p_instance_ctrl,
                                                            uint32_t                              msdu_filter_hw_id)
{
    for (uint8_t logical_id = 0;
         logical_id < BSP_FEATURE_ESWM_PSFP_MSDU_FILTER_MAX_NUM;
         logical_id++)
    {
        if ((LAYER3_SWITCH_PSFP_FILTER_STATUS_USED ==
             p_instance_ctrl->psfp_msdu_filter_info_list[logical_id].status) &&
            (msdu_filter_hw_id == p_instance_ctrl->psfp_msdu_filter_info_list[logical_id].msdu_filter_hw_id))
        {
            return true;
        }
    }
    ...
    return false;
}                                      /* End of function r_layer3_switch_psfp_is_msdu_filter_hw_id_used() */
                                                             ...
                                                             layer3_switch_psfp_msdu_filter_cfg_t * p_psfp_msdu_filter_cfg)
{
    ...
    /* The logical MSDU filter ID subscripts psfp_msdu_filter_info_list[], so it must be in range. */
    FSP_ERROR_RETURN(BSP_FEATURE_ESWM_PSFP_MSDU_FILTER_MAX_NUM > p_psfp_msdu_filter_cfg->msdu_filter_id,
                     FSP_ERR_INVALID_ARGUMENT);
    ...
    {
        /* Decide which hardware MSDU filter to use. This loop walks hardware filter numbers, so it has to
         * test hardware occupancy and not the status of the logical entry sharing the same subscript. */
        for (uint32_t unused_hw_id = 0;
             unused_hw_id < BSP_FEATURE_ESWM_PSFP_MSDU_FILTER_MAX_NUM;
             unused_hw_id++)
        {
            /* Use an unused hardware MSDU filter. */
            if (!r_layer3_switch_psfp_is_msdu_filter_hw_id_used(p_instance_ctrl, unused_hw_id))
            {
                *p_msdu_filter_id = unused_hw_id;
                err               = FSP_SUCCESS;
                break;
            }
        }
...
/*******************************************************************************************************************//**
 * Check whether a hardware Meter filter is already assigned to one of the logical Meter filter IDs.
 *
 * psfp_meter_filter_info_list[] is indexed by the logical Meter filter ID, so hardware occupancy
 * lives in the meter_filter_hw_id field and not in the subscript.
 *
 * @param[in] p_instance_ctrl          Pointer to the instance control structure
 * @param[in] meter_filter_hw_id       Hardware Meter filter ID to check
 *
 * @return  true if the hardware Meter filter ID is already assigned to a logical Meter filter
 **********************************************************************************************************************/
static bool r_layer3_switch_psfp_is_meter_filter_hw_id_used (layer3_switch_instance_ctrl_t const * p_instance_ctrl,
                                                             uint32_t                              meter_filter_hw_id)
{
    for (uint8_t logical_id = 0;
         logical_id < BSP_FEATURE_ESWM_PSFP_METER_FILTER_SINGLE_BUCKET_METERS_MAX_NUM;
         logical_id++)
    {
        if ((LAYER3_SWITCH_PSFP_FILTER_STATUS_USED ==
             p_instance_ctrl->psfp_meter_filter_info_list[logical_id].status) &&
            (meter_filter_hw_id == p_instance_ctrl->psfp_meter_filter_info_list[logical_id].meter_filter_hw_id))
        {
            return true;
        }
    }
    ...
    return false;
}                                      /* End of function r_layer3_switch_psfp_is_meter_filter_hw_id_used() */
                                                              ...
                                                              layer3_switch_psfp_meter_filter_cfg_t * p_psfp_meter_filter_cfg)
{
    ...
    /* The logical Meter filter ID subscripts psfp_meter_filter_info_list[], so it must be in range. */
    FSP_ERROR_RETURN(BSP_FEATURE_ESWM_PSFP_METER_FILTER_SINGLE_BUCKET_METERS_MAX_NUM >
                     p_psfp_meter_filter_cfg->meter_filter_id,
                     FSP_ERR_INVALID_ARGUMENT);
        ...
        p_instance_ctrl->psfp_meter_filter_info_list[p_psfp_meter_filter_cfg->meter_filter_id].status)
    {
        ...
        /* Decide which hardware Meter filter to use. This loop walks hardware meter numbers, so it has to
         * test hardware occupancy and not the status of the logical entry sharing the same subscript. */
        for (uint32_t unused_hw_id = minimum_id;
             unused_hw_id < maximum_id;
             unused_hw_id++)
        {
            /* Use an unused hardware Meter filter. */
            if (!r_layer3_switch_psfp_is_meter_filter_hw_id_used(p_instance_ctrl, unused_hw_id))
            {
                *p_meter_filter_id = unused_hw_id;
                err                = FSP_SUCCESS;
                break;
            }
        }
                                                        ...
                                                        layer3_switch_table_entry_cfg_t const * const p_table_entry_cfg)
{
    ...
    R_MFWD->FWEIE2 = (1U << msdu_filter_id);
    ...
    *p_fwpmfgc_reg = ((p_psfp_msdu_filter_cfg->maximum_frame_size << R_MFWD_FWPMFGC0_MSDUV_Pos) |
    ...
    if ((LAYER3_SWITCH_MSDU_FILTER_THROTTLE_MODE == p_psfp_msdu_filter_cfg->mode) &&
        ((R_MFWD->FWEIS2 & (1U << msdu_filter_id)) != 0))
    {
        /* Initialize FWEIS2_b.PMFS. */
        R_MFWD->FWEIS2 |= (1U << msdu_filter_id);
    }
                                                         ...
                                                         layer3_switch_table_entry_cfg_t const * const p_table_entry_cfg)
{
    ...
    *p_mfwd_fwpmtrfc_reg = 0;
    ...
    R_MFWD->FWEIS5 |= (1U << meter_filter_id);
    ...
    if (BSP_FEATURE_ESWM_PSFP_METER_FILTER_DOUBLE_BUCKET_METERS_MAX_NUM > meter_filter_id)
    {
        ...
        *p_mfwd_fwpmtrfc_reg |=
            (R_MFWD_FWPMTRFC0_MTRCM_Msk &
             ((uint32_t) yellow_only_bitmask << R_MFWD_FWPMTRFC0_MTRCM0_Pos));
    }
    ...
    *p_mfwd_fwpmtrfc_reg |= ((R_MFWD_FWPMTRFC0_MTRFE_Msk &
    ...
    R_MFWD->FWEIE5 |= (1 << meter_filter_id);
...
void layer3_switch_eaei_isr (void)
{
    ...
    for (uint32_t i = 0; i < BSP_FEATURE_ETHER_NUM_CHANNELS; i++)
    {
...
#if BSP_FEATURE_ESWM_SUPPORT_TAS
                           ...
                           R_ETHA0_EAEIS1_TASGES5_Msk | R_ETHA0_EAEIS1_TASGES6_Msk | R_ETHA0_EAEIS1_TASGES7_Msk))
        {
            ...
            callback_arg.ports |= (uint32_t) (1 << i);
