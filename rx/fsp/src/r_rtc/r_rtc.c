...
/* See Frequency Register (RFRH/RFRL)" of the relevant hardware manual */
...
#define RTC_PERFORM_READ_COUNT             (0x04U)
...
static inline void rtc_wait_count_register(void);
static inline void rtc_wait_alarm_register(void);
...
fsp_err_t R_RTC_Open (rtc_ctrl_t * const p_ctrl, rtc_cfg_t const * const p_cfg)
{
     ...
     * at power on. (see section RTC Control Register 4 (RCR4) of the relevant hardware manual)*/
...
fsp_err_t R_RTC_Close (rtc_ctrl_t * const p_ctrl)
{
     ...
     * (see section "RTC Control Register 1 (RCR1)" of the relevant hardware manual)*/
 ...
 *
 * @retval FSP_SUCCESS              Clock source set was successful.
 * @retval FSP_ERR_ASSERTION        Invalid p_ctrl or p_cfg pointer.
 ...
 *
 * @note This API starts the RTC by setting RCR2.START bit to 1. It must be called at least once to start
 * the RTC count operation.
 *
...
fsp_err_t R_RTC_CalendarTimeSet (rtc_ctrl_t * const p_ctrl, rtc_time_t * const p_time)
{
    ...
    /* See section "Setting the Time" of the relevant hardware manual for steps to set the time and error
     * adjustment value */
    ...
    R_RTC->RHRCNT  = (uint8_t) (rtc_dec_to_bcd((uint8_t) p_time->tm_hour) & RTC_RHRCNT_HOUR_MASK);
    ...
    /* Wait four read operations for the count registers */
    rtc_wait_count_register();
...
fsp_err_t R_RTC_CalendarTimeGet (rtc_ctrl_t * const p_ctrl, rtc_time_t * const p_time)
{
    ...
    bool carry_irq_status = (bool) R_BSP_IrqIsEnabled(p_instance_ctrl->p_cfg->carry_irq);

    if (false == carry_irq_status)
    {
     ...
     * therefore they must be read again. "Reading 64-Hz Counter and Time" of the relevant hardware manual)*/
    do
    {
        ...
        p_time->tm_hour = (int32_t) rtc_bcd_to_dec((uint8_t) (R_RTC->RHRCNT & RTC_RHRCNT_HOUR_MASK));
...
fsp_err_t R_RTC_CalendarAlarmSet (rtc_ctrl_t * const p_ctrl, rtc_alarm_time_t * const p_alarm)
{
    ...
    {
        field =
            (uint8_t) (rtc_dec_to_bcd((uint8_t) p_alarm->time.tm_sec) |
                       (uint8_t) (p_alarm->sec_match << RTC_COMPARE_ENB_BIT));
    }
    else
    {
        field = 0U;
    ...
    *p_reg = field;
    ...
    {
        field =
            (uint8_t) (rtc_dec_to_bcd((uint8_t) p_alarm->time.tm_min) |
                       (uint8_t) (p_alarm->min_match << RTC_COMPARE_ENB_BIT));
    }
    else
    {
        field = 0U;
    ...
    {
        field = (uint8_t) (rtc_dec_to_bcd((uint8_t) p_alarm->time.tm_hour) |
    ...
    {
        field =
            (uint8_t) ((uint8_t) p_alarm->time.tm_wday | (uint8_t) (p_alarm->dayofweek_match << RTC_COMPARE_ENB_BIT));
    }
    else
    {
        field = 0U;
    ...
    {
        field = (uint8_t) (rtc_dec_to_bcd((uint8_t) p_alarm->time.tm_mday) |
    ...
    if (p_alarm->mon_match)
    {
        ...
        field = (uint8_t) (rtc_dec_to_bcd((uint8_t) (p_alarm->time.tm_mon + 1)) |
    ...
    /* Wait four read operations for the alarm registers */
    rtc_wait_alarm_register();
...
fsp_err_t R_RTC_CalendarAlarmGet (rtc_ctrl_t * const p_ctrl, rtc_alarm_time_t * const p_alarm)
{
    ...
    p_reg                = &R_RTC->RSECAR + (RTC_ALARM_REG_SIZE * alarm_channel);
    reg_val              = *p_reg;
    p_alarm->time.tm_sec = rtc_bcd_to_dec((uint8_t) (reg_val & RTC_MASK_8TH_BIT));
    p_alarm->sec_match   = (bool) (reg_val & R_RTC_RSECAR_ENB_Msk);

    p_reg                = &R_RTC->RMINAR + (RTC_ALARM_REG_SIZE * alarm_channel);
    reg_val              = *p_reg;
    p_alarm->time.tm_min = rtc_bcd_to_dec((uint8_t) (reg_val & RTC_MASK_8TH_BIT));
    p_alarm->min_match   = (bool) (reg_val & R_RTC_RMINAR_ENB_Msk);

    p_reg                 = &R_RTC->RHRAR + (RTC_ALARM_REG_SIZE * alarm_channel);
    reg_val               = *p_reg;
    p_alarm->time.tm_hour = rtc_bcd_to_dec((uint8_t) (reg_val & RTC_MASK_8TH_BIT));
    p_alarm->hour_match   = (bool) (reg_val & R_RTC_RHRAR_ENB_Msk);

    p_reg                    = &R_RTC->RWKAR + (RTC_ALARM_REG_SIZE * alarm_channel);
    reg_val                  = *p_reg;
    p_alarm->time.tm_wday    = rtc_bcd_to_dec((uint8_t) (reg_val & RTC_MASK_8TH_BIT));
    p_alarm->dayofweek_match = (bool) (reg_val & R_RTC_RWKAR_ENB_Msk);

    p_reg                 = &R_RTC->RDAYAR + (RTC_ALARM_REG_SIZE * alarm_channel);
    reg_val               = *p_reg;
    p_alarm->time.tm_mday = rtc_bcd_to_dec((uint8_t) (reg_val & RTC_MASK_8TH_BIT));
    p_alarm->mday_match   = (bool) (reg_val & R_RTC_RDAYAR_ENB_Msk);

    /* Subtract one from month to match with C time.h standards */
    p_reg                = &R_RTC->RMONAR + (RTC_ALARM_REG_SIZE * alarm_channel);
    reg_val              = *p_reg;
    p_alarm->time.tm_mon = rtc_bcd_to_dec((uint8_t) (reg_val & RTC_MASK_8TH_BIT)) - (uint8_t) 1;
...
fsp_err_t R_RTC_ErrorAdjustmentSet (rtc_ctrl_t * const p_ctrl, rtc_error_adjustment_cfg_t const * const err_adj_cfg)
{
    ...
    /* Verify the frequecy comparison value for RFRL when using LOCO */
 ...
 * @retval FSP_ERR_INVALID_ARGUMENT    Invalid input noise filter mode.
 **********************************************************************************************************************/
fsp_err_t R_RTC_TimeCaptureSet (rtc_ctrl_t * const p_ctrl, rtc_time_capture_t * const p_time_capture)
{
#if BSP_FEATURE_RTC_HAS_ALARM1 || BSP_FEATURE_RTC_HAS_TCEN
 #if (RTC_CFG_PARAM_CHECKING_ENABLE)
    rtc_instance_ctrl_t * p_instance_ctrl = (rtc_instance_ctrl_t *) p_ctrl;
    FSP_ASSERT(NULL != p_instance_ctrl);
  ...
  #if !BSP_FEATURE_RTC_HAS_ALARM1
    FSP_ERROR_RETURN((RTC_TIME_CAPTURE_NOISE_FILTER_OFF == p_time_capture->noise_filter) ||
                     (RTC_TIME_CAPTURE_NOISE_FILTER_ON == p_time_capture->noise_filter) ||
                     (RTC_TIME_CAPTURE_NOISE_FILTER_ON_DIVIDER_32 == p_time_capture->noise_filter),
                     FSP_ERR_INVALID_ARGUMENT);
  #endif
 #else
    FSP_PARAMETER_NOT_USED(p_ctrl);
    ...
    /* When the time capture event input pin functions (RTCIC0, RTCIC1, and RTCIC2) are allocated to the same pins as
     * other multiplexed functions, make the general input port pin settings for the corresponding pins, set the corresponding
     * TAMPICR1.CHnEN bit (n = 0 to 2) to 1, and then set TCEN bit to 1 (see section "RTCCRn : Time Capture Control Register n
     * (n = 0 to 2)" of the relevant hardware manual) */
    R_BSP_RegisterProtectDisable(BSP_REG_PROTECT_OM_LPC_BATT_SWR);
    if (p_time_capture->channel <= 2)
    {
        R_SYSTEM->TAMPICR1 |= (uint8_t) (1U << p_time_capture->channel);
    }
    ...
    /* When RTCCRn is modified, check that all the bits except for the TCST bit have been updated before continuing with
     * further processing. (see section "RTCCRn : Time Capture Control Register n (n = 0 to 2)" of the
    ...
    /* When the noise filter is used, set the TCNF[1:0] bits, wait for 3 cycles of the specified sampling period (see
     * section "RTCCRn : Time Capture Control Register n (n = 0 to 2)" of the relevant hardware manual) */
    uint32_t noise_filter_delay_us = 0;

    if (p_time_capture->noise_filter != RTC_TIME_CAPTURE_NOISE_FILTER_OFF)
    {
        switch (p_time_capture->noise_filter)
        {
            ...
            {
                noise_filter_delay_us = BSP_DELAY_US_PER_SECOND *
                                        (NOISE_FILTER_CLOCK_DIVIDE_4096 / BSP_SUBCLOCK_FREQ_HZ);
                break;
            }
            ...
            {
                noise_filter_delay_us = BSP_DELAY_US_PER_SECOND *
                                        (NOISE_FILTER_CLOCK_DIVIDE_8192 / BSP_SUBCLOCK_FREQ_HZ);
                break;
            }
 #endif
     ...
     * additional processing. (see section"RTCCRn : Time Capture Control Register n (n = 0 to 2)" of the
     * relevant hardware manual) */
...
fsp_err_t R_RTC_TimeCaptureGet (rtc_ctrl_t * const p_ctrl, rtc_time_capture_t * const p_time_capture)
{
     ...
     * bits.(see section "RSECCPn : Second Capture Register n (n = 0 to 2) (in Calendar Count Mode)" of the
     * relevant hardware manual */
    uint8_t rtccr = R_RTC->RTCCR[p_time_capture->channel].RTCCR;
    R_RTC->RTCCR[p_time_capture->channel].RTCCR = (uint8_t) (rtccr & ((uint8_t) ~R_RTC_RTCCR_RTCCR_TCCT_Msk));
    ...
    p_time_capture->time.tm_sec =
        rtc_bcd_to_dec((uint8_t) (R_RTC->CP[p_time_capture->channel].RSEC & RTC_MASK_8TH_BIT));
    p_time_capture->time.tm_min =
        rtc_bcd_to_dec((uint8_t) (R_RTC->CP[p_time_capture->channel].RMIN & RTC_MASK_8TH_BIT));
    p_time_capture->time.tm_hour =
        rtc_bcd_to_dec((uint8_t) (R_RTC->CP[p_time_capture->channel].RHR & RTC_MASK_8TH_BIT));
    p_time_capture->time.tm_mday =
        rtc_bcd_to_dec((uint8_t) (R_RTC->CP[p_time_capture->channel].RDAY & RTC_MASK_8TH_BIT));

    /* Subtract one from month to match with C time.h standards */
    p_time_capture->time.tm_mon =
        rtc_bcd_to_dec((uint8_t) (R_RTC->CP[p_time_capture->channel].RMON & RTC_MASK_8TH_BIT)) -
        (uint8_t) 1;
...
static void r_rtc_start_bit_update (uint8_t value)
{
    ...
    R_RTC->RCR2_b.START = (uint8_t) (value & 1U);

    /* The START bit is updated in synchronization with the next cycle of the count source. Check if the bit is updated
     * before proceeding (see section "RTC Control Register 2 (RCR2)" of the relevant hardware manual)*/
...
static void r_rtc_software_reset (void)
{
     ...
     * (see section "RTC Control Register 2 (RCR2)" of the relevant hardware manual)*/
...
static void r_rtc_set_clock_source (rtc_instance_ctrl_t * const p_ctrl, rtc_cfg_t const * const p_cfg)
{
    /* Configure RCR4 only when LOCO is selected as the RTC clock source.
     * When sub-clock is selected, RCR4 is configured in BSP code, as the clock
     * source can only be set once before making the initial settings of
     * the RTC registers at power on.
     * (See section RTC Control Register 4 (RCR4) in the hardware manual) */
    if (RTC_CLOCK_SOURCE_LOCO == p_ctrl->p_cfg->clock_source)
    {
        R_RTC->RCR4 = (uint8_t) p_ctrl->p_cfg->clock_source;
    }

#if BSP_FEATURE_RTC_HAS_RCR3

    /* Start the sub-clock */
    R_RTC->RCR3_b.RTCEN = 1U;
    FSP_HARDWARE_REGISTER_WAIT(R_RTC->RCR3_b.RTCEN, 1U);
#endif

    /* Supply 6 clocks of the count source (LOCO/SOSC, 183us, 32kHZ).
     * See "Clock and Count Mode Setting Procedure" of the relevant hardware manual) */
    ...
    if (RTC_CLOCK_SOURCE_LOCO == p_ctrl->p_cfg->clock_source)
    {
         ...
         * Frequency Register (RFRH/RFRL)" of the relevant hardware manual) */
        R_RTC->RFRH = 0;
     ...
     * reset is completed (see section "RTC Control Register 2 (RCR2)" of the relevant hardware manual)*/
    ...
    /*
     * See "Notes on Writing to and Reading from Registers" of the relevant hardware manual.
...
static void r_rtc_config_rtc_interrupts (rtc_instance_ctrl_t * const p_ctrl, rtc_cfg_t const * const p_cfg)
{
    ...
    {
        R_BSP_IrqCfg(p_cfg->periodic_irq, (uint32_t) p_cfg->periodic_ipl, p_ctrl);
    }

    if (p_cfg->alarm_irq >= 0)
    {
        R_BSP_IrqCfg(p_cfg->alarm_irq, (uint32_t) p_cfg->alarm_ipl, p_ctrl);
    }
    ...
    {
        R_BSP_IrqCfg(p_cfg->carry_irq, (uint32_t) p_cfg->carry_ipl, p_ctrl);
    }
}
 ...
 *
 * As per HW manual, value of Year is between 0 to 99, the RTC has a 100 year calendar from 2000 to 2099.
 * (see section "Overview" of the relevant hardware manual)
 * But as per C standards, tm_year is years since 1900.
...
static void r_rtc_error_adjustment_set (rtc_error_adjustment_cfg_t const * const err_adj_cfg)
{
    ...
    if (mode != R_RTC->RCR2_b.AADJE)
    {
         ...
         * (see section "Procedure for changing the mode of adjustment" of the relevant hardware manual) */
        R_RTC->RADJ = 0U;

        /* When RADJ is modified, check that all the bits are updated before continuing with more processing.
         * (see section "Time Error Adjustment Register (RADJ)" of the relevant hardware manual) */
        FSP_HARDWARE_REGISTER_WAIT(R_RTC->RADJ, 0U);

        /* Set the error adjustment enable, 1: Enable Automatic adjustment 0: Disable Automatic adjustment */
        R_RTC->RCR2_b.AADJE = (uint8_t) ((uint8_t) mode & 1U);
...
/*******************************************************************************************************************//**
 * After writing to the count register, confirm its value has actually changed before
 * proceeding with further processing.
 * The values written to count registers are reflected when four read operations are performed after writing
 * (Refer "Notes on Writing to and Reading from Register" of the relevant hardware manual)
 **********************************************************************************************************************/
static inline void rtc_wait_count_register (void)
{
    uint32_t w_count;

    for (w_count = 0U; w_count < RTC_PERFORM_READ_COUNT; w_count++)
    {
        (void) R_RTC->RSECCNT;
        (void) R_RTC->RMINCNT;
        (void) R_RTC->RHRCNT;
...
/*******************************************************************************************************************//**
 * After writing to the alarm register, confirm its value has actually changed before
 * proceeding with further processing.
 * The values written to alarm registers are reflected when four read operations are performed after writing
 * (Refer "Notes on Writing to and Reading from Register" of the relevant hardware manual)
 **********************************************************************************************************************/
static inline void rtc_wait_alarm_register (void)
{
