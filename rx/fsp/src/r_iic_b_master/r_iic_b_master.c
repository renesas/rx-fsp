...
#define IIC_B_MASTER_DTC_RX_TRANSFER_SETTINGS         ((uint32_t) (TRANSFER_MODE_NORMAL <<                           \
                                                                   TRANSFER_SETTINGS_MODE_BITS) |                    \
                                                       (uint32_t) (TRANSFER_SIZE_1_BYTE <<                           \
...
#define IIC_B_MASTER_DTC_TX_TRANSFER_SETTINGS         ((uint32_t) (TRANSFER_MODE_NORMAL <<                           \
                                                                   TRANSFER_SETTINGS_MODE_BITS) |                    \
                                                       (uint32_t) (TRANSFER_SIZE_1_BYTE <<                           \
                                                                   TRANSFER_SETTINGS_SIZE_BITS) |                    \
                                                       (uint32_t) (TRANSFER_ADDR_MODE_INCREMENTED <<                 \
...
#define IIC_B_MASTER_PRV_ICSIER_INIT_MASK             (R_I3C0_ICSIER_NAKIE_Msk | R_I3C0_ICSIER_ALIE_Msk | \
                                                       R_I3C0_ICSIER_TMOIE_Msk)
#define IIC_B_MASTER_PRV_ICSER_INIT_MASK              (R_I3C0_ICSER_NAKDE_Msk | R_I3C0_ICSER_TEDE_Msk | \
                                                       R_I3C0_ICSER_ALE_Msk |                           \
                                                       R_I3C0_ICSER_TMOE_Msk)
#define IIC_B_MASTER_PRV_ICCSIER_INIT_MASK            (R_I3C0_ICCSIER_TIE_Msk | R_I3C0_ICCSIER_RIE_Msk)
#define IIC_B_MASTER_PRV_ICCSER_INIT_MASK             (R_I3C0_ICCSER_TDE_Msk | R_I3C0_ICCSER_RDE_Msk)

#define IIC_B_MASTER_PRV_ICSR2_ERR_STATUS_MASK        (R_I3C0_ICSR2_START_Msk | R_I3C0_ICSR2_STOP_Msk | \
                                                       R_I3C0_ICSR2_NACKF_Msk |                         \
                                                       R_I3C0_ICSR2_AL_Msk | R_I3C0_ICSR2_TMOF_Msk)

#define IIC_B_MASTER_ICBFTR_FASTPLUS_DELAY_CONST      (120U)
#define IIC_B_MASTER_ICBFTR_DELAY_CONST               (300U)
#define IIC_B_MASTER_BILLION                          (1000000000U)
#define IIC_B_MASTER_ROUNDING_OFF_COMP                (1U)

#if IIC_B_MASTER_CFG_PARAM_CHECKING_ENABLE
 #if BSP_FEATURE_IIC_B_CHECK_SCLI_BEFORE_MASTER_WRITE_TX_DATA && IIC_B_MASTER_CFG_DTC_ENABLE
 ...
 * Refer Table "Access cycles for non-GPT modules (1 of 2)" in the I/O Registers section of the relevant hardware manual)
 */
...
fsp_err_t R_IIC_B_MASTER_Open (i2c_master_ctrl_t * const p_api_ctrl, i2c_master_cfg_t const * const p_cfg)
{
    ...
    FSP_ERROR_RETURN(BSP_FEATURE_IIC_B_VALID_CHANNEL_MASK & ((uint32_t) (1 << p_cfg->channel)),
    ...
    {
        FSP_ASSERT((BSP_FEATURE_IIC_B_FAST_MODE_PLUS & ((uint32_t) (1 << p_cfg->channel))));
    }
#endif
#if IIC_B_MASTER_CFG_DTC_ENABLE
    ...
    p_ctrl->p_reg =
        (R_I3C0_Type *) (R_I3C0_BASE + (p_cfg->channel * (R_I3C1_BASE - R_I3C0_BASE)));
#else
    p_ctrl->p_reg = R_I3C0;
#endif
    ...
    if (((uint32_t) (1 << p_cfg->channel) & BSP_FEATURE_I3C_VALID_CHANNEL_MASK))
    {
     ...
     * 'Initial Settings' of the relevant hardware manual.). */
...
fsp_err_t R_IIC_B_MASTER_StatusGet (i2c_master_ctrl_t * const p_api_ctrl, i2c_master_status_t * p_status)
{
    ...
    p_status->open = (bool) (IIC_B_MASTER_OPEN == p_ctrl->open);
...
fsp_err_t R_IIC_B_MASTER_Close (i2c_master_ctrl_t * const p_api_ctrl)
{
    ...
    p_ctrl->p_reg->ICSIER  = 0x00;
    p_ctrl->p_reg->ICCSIER = 0x00;

    /* Clear ICCR.ICE to 0: SCL, SDA pins not driven */
    p_ctrl->p_reg->ICCR = 0;

    /* Abort an in-progress transfer with this device only
     * Set ICRCR.MRST to 1: Reset all registers and internal state; ICRCR.ISRST is already 0.
     */
    p_ctrl->p_reg->ICRCR = (uint32_t) (R_I3C0_ICRCR_MRST_Msk);

    /* Waiting for reset completion ICRCR.MRST = 0;  ICRCR.ISRST is already 0. */
    FSP_HARDWARE_REGISTER_WAIT(0U, p_ctrl->p_reg->ICRCR);
...
#if (1 == BSP_FEATURE_BSP_HAS_I3C_CLOCK)
    if (((uint32_t) (1 << p_ctrl->p_cfg->channel) & BSP_FEATURE_I3C_VALID_CHANNEL_MASK))
    {
                                          ...
                                          iic_b_master_transfer_dir_t direction)
{
    ...
    else
    {
         ...
         * See Section "Communication Data Format" of the relevant hardware manual.
         */
...
static void iic_b_master_abort_seq_master (iic_b_master_instance_ctrl_t * const p_ctrl, bool iic_reset)
{
     ...
     */
    p_ctrl->p_reg->ICSIER  = IIC_B_MASTER_PRV_ICSIER_INIT_MASK;
    p_ctrl->p_reg->ICCSIER = IIC_B_MASTER_PRV_ICCSIER_INIT_MASK;
}
                                         ...
                                         i2c_master_cfg_t const * const       p_cfg)
{
    ...
    p_ctrl->p_reg->ICCCR = (uint32_t) R_I3C0_ICCCR_CCE_Msk;
#endif
    ...
    /* In case of RX74M, ICMR.OMS = 1 and BFCFG.FIFOE = 0 after the above reset.
     * Refer 'ICMR : Protocol Selection Register' and
     * 'BFCFG : Buffer Configuration Selection Register' of the relevant hardware manual.
     *
     * Default value of ICSCR register is 0.
     * Default/Reset value is set after IIC reset above. Refer Table "Wake-up operation mode" of the relevant hardware manual.
    ...
    /* Clock settings - Set ICRCCR, ICFBR and ICEBR registers: Set transfer bit rate.
     * The values for these are set in the configuration structure by the tooling.
     */
    p_ctrl->p_reg->ICRCCR = (uint32_t) (p_extend->clock_settings.cks_value & R_I3C0_ICRCCR_CKS_Msk);
    p_ctrl->p_reg->ICFBR  = (uint32_t) ((p_extend->clock_settings.brl_value << R_I3C0_ICFBR_LOW_Pos) |
                                        (p_extend->clock_settings.brh_value << R_I3C0_ICFBR_HIGH_Pos));

    /* Set ICTOR, ICICR, ICTOR, ICACKR and ICCSCR registers.
     * (Default/Reset value is set after IIC reset above. Refer Table "Wake-up operation mode" of the relevant hardware manual.)
     * 1. ICTOR: Keep default settings in Open.
     *
     * 2. ICCSCR: Keep default settings in Open.
     *
     * 3. ICACKR: Keep default settings in Open.
     *
     * 4. ICICR: Keep default settings in Open:
     ...
     * (see Section 'ICICR : Input Control Register' of the relevant hardware manual.)
     *
     *  Set TMOH to enable timeouts when SCLn is high. (Non configurable)
     * Set TMOL based on user settings.
     * Set TMOS based on user settings. Only 14 (SHORT) and 16 (LONG) bit counters are allowed.
     * (see Section 'ICTOR : Timeout Control Register' of the relevant hardware manual.).
     */
    p_ctrl->p_reg->ICTOR = (uint32_t) ((uint32_t) R_I3C0_ICTOR_TMOH_Msk |
                                       ...
                                       (uint32_t) (((iic_b_master_extended_cfg_t *) p_ctrl->p_cfg->p_extend)->
                                                   timeout_scl_low << R_I3C0_ICTOR_TMOL_Pos));
    ...
    p_ctrl->p_reg->ICFER =
        (((uint32_t) (((iic_b_master_extended_cfg_t *) p_ctrl->p_cfg->p_extend)->smbus_operation <<
                      R_I3C0_ICFER_SMBS_Pos)) |
         ((uint32_t) ((I2C_MASTER_RATE_FASTPLUS == p_ctrl->p_cfg->rate) << R_I3C0_ICFER_FMPE_Pos)) |
         R_I3C0_ICFER_NALE_Msk | R_I3C0_ICFER_MALE_Msk | R_I3C0_ICFER_SCLE_Msk);

    /* If SMBus is used, update ICOCR to use SDA delay function */
    p_ctrl->p_reg->ICOCR = (((uint32_t) (p_extend->clock_settings.sdod_value << R_I3C0_ICOCR_SDAO_Pos)) |
                            ((uint32_t) (p_extend->clock_settings.sdodcs_value << R_I3C0_ICOCR_DLCS_Pos)));
     ...
     */
    p_ctrl->p_reg->ICSER = (uint32_t) IIC_B_MASTER_PRV_ICSER_INIT_MASK;

    /* Enable Interrupts: Timeout Detection, Arbitration Loss, NACK Detection.
     * Disable Interrupt: Transmit End, Start, Stop
     */
    p_ctrl->p_reg->ICSIER = (uint32_t) IIC_B_MASTER_PRV_ICSIER_INIT_MASK;

    /* Enable status for Transmit Data Buffer Empty and Receive Data Buffer Full */
    p_ctrl->p_reg->ICCSER = (uint32_t) IIC_B_MASTER_PRV_ICCSER_INIT_MASK;

    /* Enable Interrupts: Transmit Data Buffer Empty and Receive Data Buffer Full */
    p_ctrl->p_reg->ICCSIER = (uint32_t) IIC_B_MASTER_PRV_ICCSIER_INIT_MASK;

    /* Set valid interrupt contexts and user provided priority. Enable the interrupts at the NVIC  */
    R_BSP_IrqCfgEnable(p_cfg->eri_irq, (uint32_t) p_cfg->ipl, p_ctrl);
    R_BSP_IrqCfgEnable(p_cfg->txi_irq, (uint32_t) p_cfg->ipl, p_ctrl);
    R_BSP_IrqCfgEnable(p_cfg->tei_irq, (uint32_t) p_cfg->ipl, p_ctrl);
    R_BSP_IrqCfgEnable(p_cfg->rxi_irq, (uint32_t) p_cfg->ipl, p_ctrl);

    /* Set ICCR.ICE to 1: SCL, SDA pins in active state */
    p_ctrl->p_reg->ICCR = (uint32_t) R_I3C0_ICCR_ICE_Msk;

    /* Set FRECYC based on IIC reference clock.
     * (Not a part of init sequence in the HW manual)
     * See Frequency of section Electrical Characteristics. IIC Timing of the relevant hardware manual:
     * 1. tBUF (Standard Mode) = 5 × tIICcyc + 300 ns
     * 2. tBUF (Fast Mode) = 5 × tIICcyc + 300 ns
     * 3. tBUF (Fast Plus Mode) = 5 × tIICcyc + 120 ns
     *
     * See I3C Timing of section Electrical Characteristics. I3C Timing of the relevant hardware manual:
    ...
    uint32_t extra_ns =
        (I2C_MASTER_RATE_FASTPLUS ==
         p_ctrl->p_cfg->rate) ? IIC_B_MASTER_ICBFTR_FASTPLUS_DELAY_CONST : IIC_B_MASTER_ICBFTR_DELAY_CONST;
    uint32_t iic_clock_ns = (uint32_t) IIC_B_MASTER_BILLION / p_extend->iic_clock_freq;
    p_ctrl->p_reg->ICBFTR = (uint32_t) (BSP_FEATURE_IIC_BUS_FREE_TIME_MULTIPLIER + IIC_B_MASTER_ROUNDING_OFF_COMP +
                                        extra_ns / iic_clock_ns);
}

/*******************************************************************************************************************//**
 * Performs the data transfer described by the parameters when operating as a master.
 * See section "Master Transmit Operation" and section "Master Receive Operation"
...
static fsp_err_t iic_b_master_run_hw_master (iic_b_master_instance_ctrl_t * const p_ctrl)
{
    ...
    if (!p_ctrl->restarted)
    {
        ...
        FSP_ERROR_RETURN((R_I3C0_ICBSR_BFREE_Msk == (p_ctrl->p_reg->ICBSR & R_I3C0_ICBSR_BFREE_Msk)), FSP_ERR_IN_USE);
     ...
     * (see Section 'ICTOR : Timeout Control Register' of the relevant hardware manual.).
     */
    p_ctrl->p_reg->ICTOR = (uint32_t) (R_I3C0_ICTOR_TMOH_Msk |
                                       (uint32_t) (IIC_B_MASTER_TIMEOUT_MODE_SHORT ==
                                                   ((iic_b_master_extended_cfg_t *) p_ctrl->p_cfg->p_extend)->
                                                   timeout_mode) |
                                       (uint32_t) (((iic_b_master_extended_cfg_t *) p_ctrl->p_cfg->p_extend)->
     ...
     * Enable SPDE to detect unexpected STOP condition. This is disabled between communication events as it can lead
     * to undesired interrupts in multi-master setups.
     */
    p_ctrl->p_reg->ICSER = IIC_B_MASTER_PRV_ICSER_INIT_MASK | R_I3C0_ICSER_STDE_Msk | R_I3C0_ICSER_SPDE_Msk |
                           R_I3C0_ICSER_TMOE_Msk;
    p_ctrl->p_reg->ICSIER = IIC_B_MASTER_PRV_ICSIER_INIT_MASK | R_I3C0_ICSIER_STIE_Msk | R_I3C0_ICSIER_SPIE_Msk |
                            R_I3C0_ICSIER_TMOIE_Msk;
    ...
    if (!p_ctrl->restarted)
    {
        ...
        p_ctrl->p_reg->ICCGR = (uint32_t) R_I3C0_ICCGR_ST_Msk;
    }
    else
    {
        p_ctrl->restarted = false;

        /* Enable ICCSER.TDE. This is treated differently to support restart functionality.
         * The TXI status interrupt logging (ICCSER.TDE) had been disabled
         ...
         * ICMMR.ACF = 1 and ICMMR.TRS = 1 at this point.
         * With the above condition, Once ICCSER.TDE is set, ICCSR.TDRE will get set enabling the
         * pulse triggered IICn_TXI interrupt.
         * See Section 'Master Mode Operation', '(1) I2C Master Operation',
         * '(a) Data Write Transfer (Single Buffer transfer)' point #2.
         * AND
         * Table 'Interrupt Generation' of the relevant hardware manual..
         *
         */
        p_ctrl->p_reg->ICCSER = (uint32_t) IIC_B_MASTER_PRV_ICCSER_INIT_MASK;
    }
    ...
    /*
     * The Flowchart under "Master Mode Communication Flow"
     * of the relevant hardware manual. is covered in the interrupts:
     *
     * 1. NACKF processing is handled in the ERI interrupt.
     *    For receive, dummy reading ICDR is not required because the NACK processing in this driver resets the IIC peripheral.
     * 2. Data is written to ICDR in the transmit interrupt (TDRE is set when a transmit interrupt fires).
     ...
     * 5. For receive, remaining processing including reading ICDR happens in the receive interrupt (ACKTWE is set when a receive interrupt fires).
     */
    return FSP_SUCCESS;
}
...
static void iic_b_master_rxi_master (iic_b_master_instance_ctrl_t * p_ctrl)
{
    ...
    {
        /* Enable WAITRE for 1 or 2 byte read */
        if (2U >= p_ctrl->total)
        {
            p_ctrl->p_reg->ICCSCR = R_I3C0_ICCSCR_WAITRE_Msk;
        }
        ...
        if (1U == p_ctrl->remain)
        {
             ...
             * See Note 1 in Section 'ICACKR : Acknowledge Control Register'
             * of the relevant hardware manual.
             */
            p_ctrl->p_reg->ICACKR = (uint32_t) (R_I3C0_ICACKR_ACKWP_Msk | R_I3C0_ICACKR_ACKBT_Msk);
        }

#if IIC_B_MASTER_CFG_DTC_ENABLE
        uint32_t volatile const * p_iic_b_master_rx_buffer = &(p_ctrl->p_reg->ICDR);
        ...
        /* Do a dummy read to clock the data into the ICDR. */
        dummy_read = p_ctrl->p_reg->ICDR;
...
static void iic_b_master_txi_master (iic_b_master_instance_ctrl_t * p_ctrl)
{
    ...
    else if (!p_ctrl->read)
    {
...
#endif
        {
#if BSP_FEATURE_IIC_B_CHECK_SCLI_BEFORE_MASTER_WRITE_TX_DATA

            /* Confirm that ICIMR.SCLI = 0 (check the status of SCL) before writing the transmission data.
             * Please refer the relevant hardware manual I2C Master Transmission Flow (Single Buffer Transfer) for details.
             */
            FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->ICIMR_b.SCLI, 0);
#endif

            /* Write the data to ICDR register */
            p_ctrl->p_reg->ICDR = p_ctrl->p_buff[p_ctrl->loaded];
        ...
        /* We are done loading ICDR, wait for TEND to send a stop/restart */
        if (0U == p_ctrl->remain)
        {
            /* Disable Normal Transmit Data Buffer Empty Interrupt keeping other settings unchanged. */
            p_ctrl->p_reg->ICCSIER = (uint32_t) R_I3C0_ICCSIER_RIE_Msk;

            /* Wait for the value to reflect at the peripheral.
             * See 'Note' under Table "Interrupt sources" in the I3C section of the relevant hardware manual: I2C I3C unified IP (R-I3C v2) */
            IIC_B_MASTER_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->ICCSIER_b.TIE, 0U, timeout_count);
...
static void iic_b_master_tei_master (iic_b_master_instance_ctrl_t * p_ctrl)
{
    ...
    if ((p_ctrl->read) && (p_ctrl->addr_remain == 1U) && (false == p_ctrl->address_restarted))
    {
        ...
        p_ctrl->p_reg->ICCSER = (uint32_t) R_I3C0_ICCSER_RDE_Msk;

        /* Request IIC to issue the restart condition */
        p_ctrl->p_reg->ICCGR      = (uint32_t) R_I3C0_ICCGR_RS_Msk;
    ...
    else if (0U == p_ctrl->remain)
    {
        ...
        if (p_ctrl->restart)
        {
            /* ICCSIER.TIE would be disabled at this point from the previous TXI. Enable it */
            p_ctrl->p_reg->ICCSIER = IIC_B_MASTER_PRV_ICCSIER_INIT_MASK;

            /* Disable ICCSER.TDE
             * This diables the functionality of NTST.TDBEF0.
             * Once 'SR' is detected on the bus, ICMMR.CRMS and ICMMR.TRS will get set leaving NTST.TDBEF0 cleared.
             * Upon the next Read/Write API call, ICCSER.TDE will be enabled setting the NTST.TDBEF0 bit.
        ...
        else
        {
            ...
            p_ctrl->p_reg->ICSR2_b.STOP = 0U;

            /* Request IIC to issue the stop condition */
            p_ctrl->p_reg->ICCGR = (uint32_t) R_I3C0_ICCGR_SP_Msk;
        }
    }
    else
    {
    ...
    p_ctrl->p_reg->ICSR2_b.TEND  = 0U;
    p_ctrl->p_reg->ICSIER_b.TEIE = 0U;

    /* Wait for the value to reflect at the peripheral.
     * See 'Note' under Table "Interrupt sources" in the I3C section of the relevant hardware manual: I2C I3C unified IP (R-I3C v2) */
    IIC_B_MASTER_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->ICSIER_b.TEIE, 0U, timeout_count);
}
...
static void iic_b_master_err_master (iic_b_master_instance_ctrl_t * p_ctrl)
{
    ...
    uint32_t errs_events = IIC_B_MASTER_PRV_ICSR2_ERR_STATUS_MASK & p_ctrl->p_reg->ICSR2;
    p_ctrl->p_reg->ICSR2 &= (uint32_t) ~IIC_B_MASTER_PRV_ICSR2_ERR_STATUS_MASK;

    /* If the event was an error event, then handle it */
    if ((errs_events &
         (uint32_t) ((uint32_t) (R_I3C0_ICSR2_TMOF_Msk) | (uint32_t) (R_I3C0_ICSR2_AL_Msk))) |
        ((errs_events & (uint32_t) (R_I3C0_ICSR2_NACKF_Msk)) && (1U != p_ctrl->p_reg->ICMMR_b.ACF)))
    {
         ...
         *        Refer Section "ICMMR : Present State Register - Clearing conditions for CRMS"
         *        of the relevant hardware manual.
    ...
    else if ((errs_events & (uint32_t) (R_I3C0_ICSR2_NACKF_Msk)) && (1U == p_ctrl->p_reg->ICMMR_b.ACF))
    {
        /* CRMS bit must be set to issue a stop condition.
         * Refer Section "Issuing a Stop Condition" in the I3C of the relevant hardware manual: I2C I3C unified IP (R-I3C v2)
         */
         ...
         * See item '[4]' under 'Figure Example of I2C master transmission flowchart' of
         * the relevant hardware manual. */

        /* Request IIC to issue the stop condition */
        p_ctrl->p_reg->ICCGR = (uint32_t) R_I3C0_ICCGR_SP_Msk; /* It is safe to write 0's to other bits. */
        ...
        p_ctrl->p_reg->ICTOR = (uint32_t) (R_I3C0_ICTOR_TMOH_Msk |
                                           (uint32_t) (IIC_B_MASTER_TIMEOUT_MODE_SHORT ==
                                                       ...
                                                       timeout_mode)
                                           |
                                           (uint32_t) (1U << R_I3C0_ICTOR_TMOL_Pos));

        p_ctrl->p_reg->ICSIER_b.TMOIE = 1U;
    ...
    else if ((errs_events & (uint8_t) R_I3C0_ICSR2_STOP_Msk) ||
             ((p_ctrl->restarted) && (errs_events & (uint8_t) R_I3C0_ICSR2_START_Msk)))
    {
    ...
    {
        /* ICCSIER.TIE would be disabled at this point from the previous TXI
    ...
    else if (errs_events & (uint8_t) R_I3C0_ICSR2_START_Msk)
    {
        if (((iic_b_master_extended_cfg_t *) (p_ctrl->p_cfg->p_extend))->smbus_operation)
        {
...
static void iic_b_master_rxi_read_data (iic_b_master_instance_ctrl_t * const p_ctrl)
{
    ...
    else if (1U == p_ctrl->remain)
    {
        if (p_ctrl->restart)
        {
            /* NOTE: Only disable ICCSER.TDE (leaving ICCSIER.TIE enabled).
             * This will allow the interrupt to fire in the "restarted" (next)
             * transaction when ICCSER.TDE is re-enabled.
             * ICCSIER.TIE should be already enabled as this is a read transfer.
             */
            p_ctrl->p_reg->ICCSER = (uint32_t) R_I3C0_ICCSER_RDE_Msk;
             ...
             */
            p_ctrl->p_reg->ICACKR = (uint32_t) (R_I3C0_ICACKR_ACKWP_Msk);

            /* Request IIC to issue the restart condition */
            p_ctrl->p_reg->ICCGR = (uint32_t) R_I3C0_ICCGR_RS_Msk;

            /* Disable timeout function */
            p_ctrl->p_reg->ICSIER_b.TMOIE = 0U;
        ...
        else
        {
             ...
             * last byte from ICDR and clearing the RWE.
             * See Point #7 under Master Mode Operation, (1) I2C Master Operation,
             * (b) Data Read Transfer (Single Buffer transfer)' of the relevant hardware manual..
             */
        }
    }
    else
    ...
    volatile uint32_t read_data = p_ctrl->p_reg->ICDR;
...
static void iic_b_master_txi_send_address (iic_b_master_instance_ctrl_t * const p_ctrl)
{
    ...
    else
    {
        ...
        if (p_ctrl->addr_total > 1U)
        {
            ...
            {
                address_byte = (uint8_t) (p_ctrl->addr_high | (uint8_t) I2C_CODE_READ);
            }
            /* Low byte transfer */
            else
            {
...
#if IIC_B_MASTER_CFG_DTC_ENABLE
        uint32_t volatile const * p_iic_b_master_tx_buffer = &(p_ctrl->p_reg->ICDR);
        ...
        /* Confirm that ICIMR.SCLI = 0 (check the status of SCL) before writing the transmission data.
         * Please refer to the relevant hardware manual I2C Master Transmission Flow (Single Buffer Transfer) for details.
         */
        FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->ICIMR_b.SCLI, 0);
#endif

        /* Write the address byte */
        p_ctrl->p_reg->ICDR = address_byte;
                                                  ...
                                                  iic_b_master_transfer_dir_t direction)
{
    ...
    {
        p_cfg->transfer_settings_u.transfer_settings_word = IIC_B_MASTER_DTC_RX_TRANSFER_SETTINGS;
    }
    else
    {
        p_cfg->transfer_settings_u.transfer_settings_word = IIC_B_MASTER_DTC_TX_TRANSFER_SETTINGS;
    }
