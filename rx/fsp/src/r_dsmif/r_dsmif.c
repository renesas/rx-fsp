...
#define DSMIF_CHANNEL_STATUS_SHIFT    (4U)

#ifndef DSMIF_START_STOP_TIMEOUT
 #define DSMIF_START_STOP_TIMEOUT     (1000000U)
#endif
...
static fsp_err_t r_dsmif_start(dsmif_instance_ctrl_t * const p_instance_ctrl);
static fsp_err_t r_dsmif_stop(dsmif_instance_ctrl_t * const p_instance_ctrl);
static uint32_t  r_dsmif_channel_status_mask_get(uint32_t const channel_mask);
...
fsp_err_t R_DSMIF_Open (adc_ctrl_t * p_ctrl, adc_cfg_t const * const p_cfg)
{
    ...
    p_instance_ctrl->p_reg = (R_DSMIF0_Type *) (R_DSMIF0_BASE + address_offset * p_cfg->unit);

    /* Release from the module-stop state */
    R_BSP_MODULE_START(FSP_IP_DSMIF, p_cfg->unit);

    if (DSMIF_CLOCK_SELECTION_GPTCLK == p_extend->clock_selection)
    {
    ...
    p_instance_ctrl->p_reg->DSSEICR = p_extend->overcurrent_sum_error_u.overcurrent_sum_error_enable;
#endif
    ...
    for (uint32_t ch = 0U; ch < DSMIF_MAX_NUM_CHANNELS; ch++)
    {
        if (p_extend->p_channel_cfgs[ch] != NULL)
        {
            ...
            dsicr |= (uint32_t) p_error_detection_cfg->overcurrent_detection_u.overcurrent_detection_enable <<
                     R_DSMIF0_CH_DSICR_IOEL0_Pos;
            dsicr |=
                (uint32_t) p_error_detection_cfg->overcurrent_window_notification_enable_u.
                overcurrent_window_notification_enable
                    <<
            ...
            p_channel_reg->DSODWCR =
                (uint32_t) p_error_detection_cfg->overcurrent_window_notification_mode_u.
                overcurrent_window_notification_mode;
    ...
    {
        R_BSP_IrqCfgEnable(p_extend->common_current_data_irq,
                           (uint32_t) p_extend->common_current_data_ipl,
                           p_instance_ctrl);
    }
    ...
    {
        R_BSP_IrqCfgEnable(p_extend->common_capture_data_a_irq,
                           (uint32_t) p_extend->common_capture_data_a_ipl,
                           p_instance_ctrl);
    }
    ...
    {
        R_BSP_IrqCfgEnable(p_extend->common_capture_data_b_irq,
                           (uint32_t) p_extend->common_capture_data_b_ipl,
                           p_instance_ctrl);
    }
    ...
    {
        R_BSP_IrqCfgEnable(p_extend->common_error_irq, (uint32_t) p_extend->common_error_ipl, p_instance_ctrl);
    }
#endif
 ...
 * @retval  FSP_ERR_TIMEOUT            Timeout occurred while waiting for the channel to start running.
 **********************************************************************************************************************/
fsp_err_t R_DSMIF_ScanStart (adc_ctrl_t * p_ctrl)
{
    ...
    err = r_dsmif_start(p_instance_ctrl);
 ...
 * @retval  FSP_ERR_ASSERTION          An input pointer was NULL.
 * @retval  FSP_ERR_NOT_OPEN           Instance control block is not open.
 * @retval  FSP_ERR_TIMEOUT            Timeout occurred while waiting for the channel to stop.
 **********************************************************************************************************************/
...
fsp_err_t R_DSMIF_Close (adc_ctrl_t * p_ctrl)
{
    ...
    /* Stop processing before closing the module. */
    fsp_err_t err = r_dsmif_stop(p_instance_ctrl);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
...
fsp_err_t R_DSMIF_ErrorDetectionEnable (adc_ctrl_t * p_ctrl)
{
    ...
    for (uint32_t ch = 0U; ch < DSMIF_MAX_NUM_CHANNELS; ch++)
    {
        if (NULL != p_cfg_extend->p_channel_cfgs[ch])
        {
            ...
            p_channel_reg->DSODCR =
                ((uint32_t) p_error_detection_cfg->overcurrent_detection_u.overcurrent_detection_enable <<
                    R_DSMIF0_CH_DSODCR_ODEL_Pos) |
                ((uint32_t) p_error_detection_cfg->overcurrent_window_notification_enable_u.
                 overcurrent_window_notification_enable <<
    ...
    p_instance_ctrl->p_reg->DSSECR =
        (uint32_t) ((p_cfg_extend->overcurrent_sum_error_u.overcurrent_sum_error_enable_b.lower_limit_enable <<
                     R_DSMIF0_DSSECR_SEEL_Pos) |
                    (p_cfg_extend->overcurrent_sum_error_u.overcurrent_sum_error_enable_b.upper_limit_enable <<
 ...
 * @param[in]  p_instance_ctrl     Pointer to instance control block
 *
 * @retval  FSP_SUCCESS            Start processing succeeded.
 * @retval  FSP_ERR_TIMEOUT        Timeout occurred while waiting for the channel to start running.
 **********************************************************************************************************************/
static fsp_err_t r_dsmif_start (dsmif_instance_ctrl_t * const p_instance_ctrl)
{
    dsmif_extended_cfg_t const * p_extend    = (dsmif_extended_cfg_t const *) p_instance_ctrl->p_cfg->p_extend;
    uint32_t const               status_mask = r_dsmif_channel_status_mask_get(p_extend->channel_mask);
    ...
    uint32_t timeout = DSMIF_START_STOP_TIMEOUT;
    while (status_mask != (p_instance_ctrl->p_reg->DSCSSR & status_mask))
    {
...
static fsp_err_t r_dsmif_stop (dsmif_instance_ctrl_t * const p_instance_ctrl)
{
    ...
    /* Wait for all target DSMIF channels to stop (Refer to "DSMIF / Stop Flow" section in the MCU user manual). */
    uint32_t timeout = DSMIF_START_STOP_TIMEOUT;
    while (0U != (p_instance_ctrl->p_reg->DSCSSR & status_mask))
    {
        FSP_ERROR_RETURN(0U != timeout, FSP_ERR_TIMEOUT);
        timeout--;
    }
...
/*******************************************************************************************************************//**
 * Converts the configured channel trigger mask to the corresponding DSCSSR channel state bit mask.
 *
 * @param[in]  channel_mask        Configured DSMIF channel trigger mask
 *
 * @return DSCSSR channel state mask
 **********************************************************************************************************************/
static uint32_t r_dsmif_channel_status_mask_get (uint32_t const channel_mask)
{
    ...
    for (uint32_t ch = 0U; ch < DSMIF_MAX_NUM_CHANNELS; ch++)
    {
        ...
        {
            status_mask |= R_DSMIF0_DSCSSR_CHSTATE_Msk << (ch * DSMIF_CHANNEL_STATUS_SHIFT);
        }
    }
                                ...
                                dsmif_interrupt_cfg_t const * const p_irq_cfg)
{
    ...
    {
        R_BSP_IrqCfgEnable(p_irq_cfg->current_data_update_irq,
                           (uint32_t) p_irq_cfg->current_data_update_ipl,
                           p_instance_ctrl);
    }

    if (p_irq_cfg->capture_data_a_irq >= 0)
    {
        R_BSP_IrqCfgEnable(p_irq_cfg->capture_data_a_irq, (uint32_t) p_irq_cfg->capture_data_a_ipl, p_instance_ctrl);
    }

    if (p_irq_cfg->capture_data_b_irq >= 0)
    {
        R_BSP_IrqCfgEnable(p_irq_cfg->capture_data_b_irq, (uint32_t) p_irq_cfg->capture_data_b_ipl, p_instance_ctrl);
    }
}
...
void dsmif_cdupdn_isr (void)
{
    ...
    uint32_t ielsr_offset = (uint32_t) ICU_EVENT_DSMIF0_CDUPD0 +
                            (((uint32_t) ICU_EVENT_DSMIF1_CDUPD0 -
                              (uint32_t) ICU_EVENT_DSMIF0_CDUPD0) *
                             p_instance_ctrl->p_cfg->unit);
    uint32_t channel;

    if (irq >= (IRQn_Type) BSP_INTERRUPT_SLIR_ENTRY_NUM)
    {
        channel = R_ICU->SLIR[irq - (IRQn_Type) BSP_INTERRUPT_SLIR_ENTRY_NUM].SLIR - ielsr_offset;
    }
    else
    {
        channel = R_ICU->SLIXR[irq - (IRQn_Type) BSP_INTERRUPT_SLIXR_ENTRY_NUM].SLIXR - ielsr_offset;
    }

    /* Clear the channel status flag. */
    p_instance_ctrl->p_reg->DSCSCR = (uint32_t) (1 << channel);
...
void dsmif_cdaupdn_isr (void)
{
    ...
    /* Determine the channel associated with this interrupt request. */
    uint32_t ielsr_offset = (uint32_t) ICU_EVENT_DSMIF0_CDAUPD0 +
                            (((uint32_t) ICU_EVENT_DSMIF1_CDAUPD0 -
                              (uint32_t) ICU_EVENT_DSMIF0_CDAUPD0) *
                             p_instance_ctrl->p_cfg->unit);
    uint32_t channel;
...
void dsmif_cdbupdn_isr (void)
{
    ...
    /* Determine the channel associated with this interrupt request. */
    uint32_t ielsr_offset = (uint32_t) ICU_EVENT_DSMIF0_CDBUPD0 +
                            (((uint32_t) ICU_EVENT_DSMIF1_CDBUPD0 -
                              (uint32_t) ICU_EVENT_DSMIF0_CDBUPD0) *
                             p_instance_ctrl->p_cfg->unit);
    uint32_t channel;
