...
/* GCC compilier occur aggregate-return warning on FreeRTOS port. */
  ...
  #pragma GCC diagnostic ignored "-Waggregate-return"
            ...
            sys_thread_new("lwip_ether_input_thread",
                           ...
                           p_lwip_instance,
                           (int) p_lwip_instance->p_cfg->input_thread_stacksize,
                           (int) p_lwip_instance->p_cfg->input_thread_priority);
 #if defined(__GNUC__)
...
static err_t rm_lwip_ether_output (struct netif * p_netif, struct pbuf * p_pbuf)
{
    ...
    else
    {
        ...
        if (NULL != p_send_pbuf)
        {
            ...
            while (NULL != temp)
            {
                memcpy(p_current_buffer, (char *) temp->payload, (size_t) temp->len);
    ...
    if (NULL != p_send_pbuf)
    {
        fsp_err = p_ether_instance->p_api->write(p_ether_instance->p_ctrl, p_send_pbuf->payload, (uint32_t) len);
    }
    else
    {
