...
#endif
    R_SSI0_Type * p_reg = (R_SSI0_Type *) (uintptr_t) base_address;
...
static void r_ssi_interrupts_configure (ssi_instance_ctrl_t * const p_instance_ctrl, i2s_cfg_t const * const p_cfg)
{
    ...
    R_BSP_IrqCfgEnable(p_cfg->int_irq, (uint32_t) p_cfg->idle_err_ipl, p_instance_ctrl);
    ...
    if (p_cfg->rxi_irq >= 0)
    {
        R_BSP_IrqCfgEnable(p_cfg->rxi_irq, (uint32_t) p_cfg->rxi_ipl, p_instance_ctrl);
    }
    ...
    if (p_cfg->txi_irq >= 0)
    {
        R_BSP_IrqCfgEnable(p_cfg->txi_irq, (uint32_t) p_cfg->txi_ipl, p_instance_ctrl);
    }
}
                                                    ...
                                                    transfer_size_t         fifo_access_size)
{
    ...
    if (NULL != p_cfg->p_transfer_tx)
    {
        ...
        p_cfg->p_transfer_tx->p_cfg->p_info->transfer_settings_u.transfer_settings_word = transfer_settings_tx;
    ...
    if (NULL != p_cfg->p_transfer_rx)
    {
        ...
        p_cfg->p_transfer_rx->p_cfg->p_info->transfer_settings_u.transfer_settings_word = transfer_settings_rx;
...
fsp_err_t r_ssi_rx_unload_fifo (ssi_instance_ctrl_t * const p_instance_ctrl, void * const p_dest, uint32_t const bytes)
{
    ...
    if (NULL != p_transfer)
    {
        ...
        samples = 2U;
        p_data  = (void *) (uintptr_t) ((uint32_t) p_data + (bytes - (2U << p_instance_ctrl->fifo_access_size)));
    }
#endif
...
static void r_ssi_fifo_write (ssi_instance_ctrl_t * p_instance_ctrl)
{
    ...
    p_instance_ctrl->p_tx_src        = (void *) (uintptr_t) ((uint32_t) p_instance_ctrl->p_tx_src + bytes_written);
...
static void r_ssi_fifo_read (ssi_instance_ctrl_t * p_instance_ctrl)
{
    ...
    p_instance_ctrl->p_rx_dest        = (void *) (uintptr_t) ((uint32_t) p_instance_ctrl->p_rx_dest + bytes_read);
...
static void r_ssi_tx_fifo_write (ssi_instance_ctrl_t * p_instance_ctrl, uint32_t stages_to_write)
{
    /* Writes 32-bit data to the transmit FIFO, if FIFO access size is 4-bytes. */
    ...
    /* Writes 16-bit data to the transmit FIFO, if FIFO access size is 2-bytes. */
    ...
    /* Writes 8-bit data to the transmit FIFO, if FIFO access size is 1-bytes. */
...
static void r_ssi_rx_fifo_read (ssi_instance_ctrl_t * p_instance_ctrl, uint32_t stages_to_read)
{
    /* Reads 32-bit data from the receive FIFO, if FIFO access size is 4-bytes. */
    if (TRANSFER_SIZE_4_BYTE == p_instance_ctrl->fifo_access_size)
    {
    ...
    /* Reads 16-bit data from the receive FIFO, if FIFO access size is 2-bytes. */
    if (TRANSFER_SIZE_2_BYTE == p_instance_ctrl->fifo_access_size)
    {
    ...
    /* Reads 8-bit data from the receive FIFO, if FIFO access size is 1-bytes. */
    if (TRANSFER_SIZE_1_BYTE == p_instance_ctrl->fifo_access_size)
    {
