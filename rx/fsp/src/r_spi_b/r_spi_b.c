...
#define SPI_B_REG(channel)    ((R_SPI_B0_Type *) (R_SPI_B0_BASE + (R_SPI_B1_BASE - R_SPI_B0_BASE) * \
                                                  (uint32_t) (channel)))

#define SPI_B_DTC_RX_TRANSFER_SETTINGS    (((uint32_t) TRANSFER_MODE_NORMAL << TRANSFER_SETTINGS_MODE_BITS) |         \
                                           ((uint32_t) TRANSFER_SIZE_1_BYTE << TRANSFER_SETTINGS_SIZE_BITS) |         \
                                           ((uint32_t) TRANSFER_ADDR_MODE_FIXED << TRANSFER_SETTINGS_SRC_ADDR_BITS) | \
...
#define SPI_B_DTC_TX_TRANSFER_SETTINGS    (((uint32_t) TRANSFER_MODE_NORMAL << TRANSFER_SETTINGS_MODE_BITS) | \
                                           ((uint32_t) TRANSFER_SIZE_1_BYTE << TRANSFER_SETTINGS_SIZE_BITS) | \
                                           ((uint32_t) TRANSFER_ADDR_MODE_INCREMENTED <<                      \
                                            TRANSFER_SETTINGS_SRC_ADDR_BITS) |                                \
                                           ((uint32_t) TRANSFER_IRQ_END << TRANSFER_SETTINGS_IRQ_BITS) |      \
                                           ((uint32_t) TRANSFER_ADDR_MODE_FIXED << TRANSFER_SETTINGS_DEST_ADDR_BITS))
...
#define SPI_B_PRV_SPSCLR_ALL_CLEAR        (0xFD80U) // Clear all status flags
...
static void      r_spi_b_irq_config(spi_b_instance_ctrl_t * p_ctrl);
...
fsp_err_t R_SPI_B_Close (spi_ctrl_t * const p_api_ctrl)
{
    ...
    /* Disable interrupts in IRQ. When using DMA for data transfer, TXI and RXI interrupts
     ...
     * (See "SPI Control Register (SPSCLR)" description in the relevant hardware manual). */
    p_ctrl->p_regs->SPSCLR;
    p_ctrl->p_regs->SPSCLR = SPI_B_PRV_SPSCLR_ALL_CLEAR;
 ...
 * @retval FSP_ERR_INVALID_ARGUMENT    Bitrate is not achievable or selected clock source is not supported.
 **********************************************************************************************************************/
fsp_err_t R_SPI_B_CalculateBitrate (uint32_t bitrate, spi_b_clock_source_t clock_source, rspck_div_setting_t * spck_div)
{
#if SPI_B_CFG_PARAM_CHECKING_ENABLE
 #if BSP_FEATURE_BSP_HAS_SCISPI_CLOCK == 0 && BSP_FEATURE_BSP_HAS_SPI_CLOCK == 0

    /* Verify whether the MCU supports the TCLK request clock (SCISPICLOCK or SPICLOCK)
     * Reference the SPI_B Block Diagram figure */
    FSP_ERROR_RETURN(SPI_B_CLOCK_SOURCE_SCISPICLK != clock_source, FSP_ERR_INVALID_ARGUMENT);
 #endif
#endif

    /* desired_divider = Smallest integer greater than or equal to PCLK / bitrate. */
    ...
    else
    {
        ...
        /* If SCISPI clock source is not supported, then calculates with PCLK clock */
        desired_divider = R_FSP_SystemClockHzGet(BSP_FEATURE_SPI_CLK);
#endif
    }
    ...
    /*
     * Possible PCLK dividers for values of N:
     *   N = 0; div = [2,4,6,..,512]
     *   N = 1; div = [4,8,12,..,1024]
     ...
     *
     * spbr = PCLK_DIV / (2 * 2^i) - 1
     */
    uint32_t spbr_divisor = (2U * (1U << i));

    /* spbr = (Smallest integer greater than or equal to PCLK_DIV / (2 * 2^i)) - 1. */
...
static fsp_err_t r_spi_b_transfer_config (spi_cfg_t const * const p_cfg)
{
    ...
    if (p_transfer_tx)
    {
        p_transfer_tx->p_cfg->p_info->transfer_settings_u.transfer_settings_word = SPI_B_DTC_TX_TRANSFER_SETTINGS;
    ...
    if (p_transfer_rx)
    {
        p_transfer_rx->p_cfg->p_info->transfer_settings_u.transfer_settings_word = SPI_B_DTC_RX_TRANSFER_SETTINGS;
...
static void r_spi_b_hw_config (spi_b_instance_ctrl_t * p_ctrl)
{
    ...
    /* Set clock source */
    spcr |= (uint32_t) (p_extend->clock_source << R_SPI_B0_SPCR_SYNDIS_Pos);
    ...
    spcr |= (uint32_t) (p_extend->spi_comm << R_SPI_B0_SPCR_CMMD_Pos);

    /* Configure SSLn polarity setting. */
    sslp &= (uint8_t) ~0x0FU;
    sslp |= (uint8_t) (p_extend->ssl_polarity << p_extend->ssl_select);

    /* Configure bitrate */
    spbr |= (p_extend->spck_div.spbr) << R_SPI_B0_SPBR_SPBR_Pos;
    ...
    if (SPI_B_MOSI_IDLE_VALUE_FIXING_DISABLE != p_extend->mosi_idle)
    {
        ...
        sppcr |= R_SPI_B0_SPPCR_MOIFE_Msk;

        if (SPI_B_MOSI_IDLE_VALUE_FIXING_HIGH == p_extend->mosi_idle)
        {
            sppcr |= R_SPI_B0_SPPCR_MOIFV_Msk;
        }
    }
    ...
    spdcr |= (uint16_t) (p_extend->byte_swap);
    ...
    if (SPI_MODE_MASTER == p_ctrl->p_cfg->operating_mode)
    {
        ...
        spckd = (uint8_t) (p_extend->spck_delay << R_SPI_B0_SPCKD_SCKDL_Pos);
        sslnd = (uint8_t) (p_extend->ssl_negation_delay << R_SPI_B0_SSLND_SLNDL_Pos);
        spnd  = (uint8_t) (p_extend->next_access_delay << R_SPI_B0_SPND_SPNDL_Pos);
    }
    ...
    /* Write registers */
    p_ctrl->p_regs->SPBR   = spbr;
    p_ctrl->p_regs->SSLP   = sslp;
    p_ctrl->p_regs->SPCKD  = spckd;
    p_ctrl->p_regs->SPND   = spnd;
    p_ctrl->p_regs->SSLND  = sslnd;
    p_ctrl->p_regs->SPPCR  = sppcr;
    ...
    /* Read back SPCR to ensure 1 PCLK has passed
...
/*******************************************************************************************************************//**
 * Enable Receive Buffer Full, Transmit Buffer Empty, and Error Interrupts.
 *
 * @param[in]  p_ctrl          pointer to control structure.
 **********************************************************************************************************************/
static void r_spi_b_irq_config (spi_b_instance_ctrl_t * p_ctrl)
{
    if (p_ctrl->p_cfg->txi_irq >= 0)
    {
        R_BSP_IrqCfgEnable(p_ctrl->p_cfg->txi_irq, (uint32_t) p_ctrl->p_cfg->txi_ipl, p_ctrl);
    }

    if (p_ctrl->p_cfg->rxi_irq >= 0)
    {
        R_BSP_IrqCfgEnable(p_ctrl->p_cfg->rxi_irq, (uint32_t) p_ctrl->p_cfg->rxi_ipl, p_ctrl);
    }

    R_BSP_IrqCfgEnable(p_ctrl->p_cfg->eri_irq, (uint32_t) p_ctrl->p_cfg->eri_ipl, p_ctrl);

    R_BSP_IrqCfg(p_ctrl->p_cfg->tei_irq, (uint32_t) p_ctrl->p_cfg->tei_ipl, p_ctrl);
                                            ...
                                            spi_bit_width_t const bit_width)
{
    ...
    if (p_ctrl->p_cfg->p_transfer_rx)
    {
        ...
        p_info->transfer_settings_u.transfer_settings_word_b.size = size;
        p_info->length = (uint16_t) length;
        p_info->transfer_settings_u.transfer_settings_word_b.dest_addr_mode = TRANSFER_ADDR_MODE_INCREMENTED;
        p_info->p_dest = p_dest;

        if (NULL == p_dest)
        {
            static uint32_t dummy_rx;
            p_info->transfer_settings_u.transfer_settings_word_b.dest_addr_mode = TRANSFER_ADDR_MODE_FIXED;
    ...
    if (p_ctrl->p_cfg->p_transfer_tx)
    {
        ...
        p_info->transfer_settings_u.transfer_settings_word_b.size = size;
        p_info->length = (uint16_t) length;
        p_info->transfer_settings_u.transfer_settings_word_b.src_addr_mode = TRANSFER_ADDR_MODE_INCREMENTED;
        p_info->p_src = p_src;

        if (NULL == p_src)
        {
            static uint32_t dummy_tx = 0;
            p_info->transfer_settings_u.transfer_settings_word_b.src_addr_mode = TRANSFER_ADDR_MODE_FIXED;
...
static void r_spi_b_receive (spi_b_instance_ctrl_t * p_ctrl)
{
    ...
    p_ctrl->p_regs->SPSCLR = R_SPI_B0_SPSCLR_SPRFC_Msk;
...
static void r_spi_b_transmit (spi_b_instance_ctrl_t * p_ctrl)
{
    ...
    p_ctrl->p_regs->SPSCLR = R_SPI_B0_SPSCLR_SPTEFC_Msk;
...
static void r_spi_b_call_callback (spi_b_instance_ctrl_t * p_ctrl, spi_event_t event)
{
    ...
    /* If the project is not Secure, then it will never need to change security state in order to call the callback. */
