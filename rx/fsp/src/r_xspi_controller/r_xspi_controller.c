...
#include "r_xspi_controller.h"
 ...
 #if defined XSPI_CONTROLLER_CFG_DOTF_PROTECTED_MODE_SUPPORT_ENABLE
...
#define XSPI_CONTROLLER_PRV_OPEN                   (0x78535049U)

#define XSPI_CONTROLLER_PRV_CHANNELS_PER_UNIT      (2U)
#define XSPI_CONTROLLER_PRV_UNIT_CHANNELS_SHIFT    (XSPI_CONTROLLER_PRV_CHANNELS_PER_UNIT)
#define XSPI_CONTROLLER_PRV_UNIT_CHANNELS_MASK     ((1U << XSPI_CONTROLLER_PRV_UNIT_CHANNELS_SHIFT) - 1U)

/* If any of the BSP memory macros are not defined, give them a default value of 0 for use in this file. */
#ifndef BSP_XSPI0_CS0_START_ADDRESS
 #define BSP_XSPI0_CS0_START_ADDRESS               (0U)
#endif

#ifndef BSP_XSPI0_CS0_SIZE_BYTES
 #define BSP_XSPI0_CS0_SIZE_BYTES                  (0U)
#endif

#ifndef BSP_XSPI0_CS1_START_ADDRESS
 #define BSP_XSPI0_CS1_START_ADDRESS               (0U)
#endif

#ifndef BSP_XSPI0_CS1_SIZE_BYTES
 #define BSP_XSPI0_CS1_SIZE_BYTES                  (0U)
#endif

#ifndef BSP_XSPI1_CS0_START_ADDRESS
 #define BSP_XSPI1_CS0_START_ADDRESS               (0U)
#endif

#ifndef BSP_XSPI1_CS0_SIZE_BYTES
 #define BSP_XSPI1_CS0_SIZE_BYTES                  (0U)
#endif

#ifndef BSP_XSPI1_CS1_START_ADDRESS
 #define BSP_XSPI1_CS1_START_ADDRESS               (0U)
#endif

#ifndef BSP_XSPI1_CS1_SIZE_BYTES
 #define BSP_XSPI1_CS1_SIZE_BYTES                  (0U)
#endif

/**
 * Mask of all channels for a given XSPI_CONTROLLER unit.
 * @param p_ext_cfg Pointer to a XSPI_CONTROLLER extended config struct with the unit to mask.
 */
#define XSPI_CONTROLLER_PRV_UNIT_MASK(p_ext_cfg)                 (XSPI_CONTROLLER_PRV_UNIT_CHANNELS_MASK << \
                                                                  (((p_ext_cfg)->xspi_controller_unit) *    \
                                                                   XSPI_CONTROLLER_PRV_UNIT_CHANNELS_SHIFT))

/**
 * Individual bit mask for a single channel on a given XSPI_CONTROLLER unit.
 * @param p_ext_cfg Pointer to a XSPI_CONTROLLER extended config struct with the unit and channel to mask.
 */
#define XSPI_CONTROLLER_PRV_CH_MASK(p_ext_cfg)                   ((1U << ((p_ext_cfg)->channel)) <<      \
                                                                  (((p_ext_cfg)->xspi_controller_unit) * \
                                                                   XSPI_CONTROLLER_PRV_UNIT_CHANNELS_SHIFT))
 ...
 */
#define XSPI_CONTROLLER_PRV_EXTENDED_CFG(p_ctrl)                 ((xspi_controller_extended_cfg_t *)                    \
                                                                  ((xspi_controller_instance_ctrl_t *) (p_ctrl))->p_cfg \
                                                                  ->p_extend)

/** Indicates the provided protocol mode requires the Data-Strobe signal. */
#define XSPI_CONTROLLER_PRV_PROTOCOL_USES_DS_SIGNAL(protocol)    ((bool) (((uint32_t) (protocol)) & 0x200UL))

/** Number of bytes combined into a single transaction for memory-mapped writes. */
#define XSPI_CONTROLLER_PRV_COMBINATION_WRITE_LENGTH                  (2U *                                        \
                                                                       ((uint8_t)                                  \
                                                                        XSPI_CONTROLLER_CFG_COMBINATION_FUNCTION + \
                                                                        1U))

/** Converts @ref spi_flash_address_bytes_t to a register compatible length value. */
#define XSPI_CONTROLLER_PRV_ADDR_BYTES_TO_LENGTH(spi_flash_bytes)    ((uint8_t) ((spi_flash_bytes) + 1U))

#define XSPI_CONTROLLER_PRV_BMCTL_DEFAULT_VALUE                       (0x0C)

#define XSPI_CONTROLLER_PRV_CMCFG_1BYTE_VALUE_MASK                    (0xFF00U)
#define XSPI_CONTROLLER_PRV_CMCFG_2BYTE_VALUE_MASK                    (0xFFFFU)

#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_DATA_SIZE                 (0xFU)
#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_LATENCY_CYCLES            (0U)

#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_PREAMBLE_PATTERN_0        (0xFFFF0000U)
#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_PREAMBLE_PATTERN_1        (0x000800FFU)
#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_PREAMBLE_PATTERN_2        (0x00FFF700U)
#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_PREAMBLE_PATTERN_3        (0xF700F708U)

#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_RAM_PREAMBLE_PATTERN_0    (0xFFFF0000U)
#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_RAM_PREAMBLE_PATTERN_1    (0x0800FF00U)
#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_RAM_PREAMBLE_PATTERN_2    (0xFF0000F7U)
#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_RAM_PREAMBLE_PATTERN_3    (0x00F708F7U)

#define XSPI_CONTROLLER_PRV_ADDRESS_REPLACE_VALUE                     (0xF0U)
#define XSPI_CONTROLLER_PRV_ADDRESS_REPLACE_ENABLE_BITS               (XSPI_CONTROLLER_PRV_ADDRESS_REPLACE_VALUE << \
                                                                       R_XSPI0_CMCFGCS_CMCFG0_ADDRPEN_Pos)
#define XSPI_CONTROLLER_PRV_ADDRESS_REPLACE_MASK                      (~(XSPI_CONTROLLER_PRV_ADDRESS_REPLACE_VALUE << \
                                                                         24))

#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_FRAME_INTERVAL            (0x1FU)
#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_NO_WRITE_CMD              (0x1U)
#define XSPI_CONTROLLER_PRV_AUTOCALIBRATION_SHIFT_DS_END_VALUE        (0x1FU)

#define XSPI_CONTROLLER_PRV_WORD_ACCESS_SIZE                          (4U)
#define XSPI_CONTROLLER_PRV_HALF_WORD_ACCESS_SIZE                     (2U)

#define XSPI_CONTROLLER_PRV_DIRECT_ADDR_AND_DATA_MASK                 (7U)
#define XSPI_CONTROLLER_PRV_PAGE_SIZE_BYTES                           (256U)

#define XSPI_CONTROLLER_PRV_DIRECT_CMD_SIZE_MASK                      (0x3U)

#define XSPI_CONTROLLER_PRV_CDTBUF_CMD_OFFSET                         (16U)
#define XSPI_CONTROLLER_PRV_CDTBUF_CMD_UPPER_OFFSET                   (24U)
#define XSPI_CONTROLLER_PRV_CDTBUF_CMD_1B_VALUE_MASK                  (0xFFU)
#define XSPI_CONTROLLER_PRV_CDTBUF_CMD_1B_VALUE_SHIFT                 (8U)
#define XSPI_CONTROLLER_PRV_CDTBUF_CMD_2B_VALUE_MASK                  (0xFFFFU)

#if BSP_FEATURE_XSPI_CONTROLLER_HAS_CHANNEL_1
 #define XSPI_CONTROLLER_PRV_BMCTL0_RESERVED_MASK                     (0x00) // Reserved bit mask for BMCTL0 register
 #define XSPI_CONTROLLER_PRV_ACCESS_BITFIELD                          (0x03) // Access bitfield width for BMCTL1 and COMSTT register

#else
 #define XSPI_CONTROLLER_PRV_BMCTL0_RESERVED_MASK                     (0xF0) // Reserved bit mask for BMCTL0 register
 #define XSPI_CONTROLLER_PRV_ACCESS_BITFIELD                          (0x01) // Access bitfield width for BMCTL1 and COMSTT register

#endif

/* When XSPI supports only channel 0, BMCTL0 register bit [4..7] should set 0b1111 */
#define XSPI_CONTROLLER_PRV_BMCTL0_DISABLED_VALUE                     (XSPI_CONTROLLER_PRV_BMCTL0_RESERVED_MASK | 0x00) // 0b0000'0000
#define XSPI_CONTROLLER_PRV_BMCTL0_READ_ONLY_VALUE                    (XSPI_CONTROLLER_PRV_BMCTL0_RESERVED_MASK | 0x55) // 0b0101'0101
#define XSPI_CONTROLLER_PRV_BMCTL0_WRITE_ONLY_VALUE                   (XSPI_CONTROLLER_PRV_BMCTL0_RESERVED_MASK | 0xAA) // 0b1010'1010
#define XSPI_CONTROLLER_PRV_BMCTL0_READ_WRITE_VALUE                   (XSPI_CONTROLLER_PRV_BMCTL0_RESERVED_MASK | 0xFF) // 0b1111'1111

/* When XSPI supports only channel 0, BMCTL1 register should access bitfield with a width of 1 bit. */
#define XSPI_CONTROLLER_PRV_BMCTL1_CLEAR_PREFETCH_MASK                (XSPI_CONTROLLER_PRV_ACCESS_BITFIELD << \
                                                                       R_XSPI0_BMCTL1_PBUFCLRCH0_Pos)
#define XSPI_CONTROLLER_PRV_BMCTL1_PUSH_COMBINATION_WRITE_MASK        (XSPI_CONTROLLER_PRV_ACCESS_BITFIELD << \
                                                                       R_XSPI0_BMCTL1_MWRPUSHCH0_Pos)

/* When XSPI supports only channel 0, COMSTT register should access a bitfield with a width of 1 bit. */
#define XSPI_CONTROLLER_PRV_COMSTT_MEMACCCH_MASK                      (XSPI_CONTROLLER_PRV_ACCESS_BITFIELD << \
                                                                       R_XSPI0_COMSTT_MEMACCCH0_Pos)
#define XSPI_CONTROLLER_PRV_COMSTT_WRBUFNECH_MASK                     (XSPI_CONTROLLER_PRV_ACCESS_BITFIELD << \
                                                                       R_XSPI0_COMSTT_WRBUFNECH0_Pos)

#define XSPI_CONTROLLER_PRV_COMSTT_PENDING_ACTION_MASK                (XSPI_CONTROLLER_PRV_COMSTT_MEMACCCH_MASK | \
                                                                       XSPI_CONTROLLER_PRV_COMSTT_WRBUFNECH_MASK)

#define XSPI_CONTROLLER_SOFTWARE_DELAY                                (50U)

#define XSPI_CONTROLLER_PRV_DOTF_REG00_RESET_VALUE                    (0x22000000)
#define XSPI_CONTROLLER_PRV_CONVAREAST_RESET_VALUE                    (0x0)
#define XSPI_CONTROLLER_PRV_CONVAREAD_RESET_VALUE                     (0x0)
#define XSPI_CONTROLLER_PRV_DOTF_SEED_1_SEED_MASK                     (0x3)
#define XSPI_CONTROLLER_PRV_DOTF_SEED_1_ENABLE_MASK                   (0xC)

/* These are used as modulus checking, make sure they are powers of 2. */
#define XSPI_CONTROLLER_PRV_CPU_ACCESS_LENGTH                         (8U)
#define XSPI_CONTROLLER_PRV_CPU_ACCESS_ALIGNMENT                      (8U)

#define XSPI_CONTROLLER_PRV_PROTOCOL_USES_DS_MASK                     (0x200U)

#define XSPI_CONTROLLER_PRV_UINT32_BITS                               (32)

#ifndef XSPI_CONTROLLER_MAX_WRITE_ENABLE_LOOPS
 #define XSPI_CONTROLLER_MAX_WRITE_ENABLE_LOOPS                       (5)
#endif

/* Number of address bytes in 4 byte address mode. */
#define XSPI_CONTROLLER_4_BYTE_ADDRESS                                (4U)
 ...
 **********************************************************************************************************************/
static bool r_xspi_controller_status_sub(
    xspi_controller_instance_ctrl_t * p_instance_ctrl,
    uint8_t                           bit_pos);
static fsp_err_t r_xspi_controller_protocol_specific_settings(
    xspi_controller_instance_ctrl_t * p_instance_ctrl);
static fsp_err_t r_xspi_controller_write_enable(
    xspi_controller_instance_ctrl_t * p_instance_ctrl);
static void r_xspi_controller_direct_transfer(
    xspi_controller_instance_ctrl_t   * p_instance_ctrl,
    spi_flash_direct_transfer_t * const p_transfer,
    spi_flash_direct_transfer_dir_t     direction);
static xspi_controller_command_set_t const * r_xspi_controller_command_set_get(
    xspi_controller_instance_ctrl_t * p_instance_ctrl);

#if XSPI_CONTROLLER_CFG_AUTOCALIBRATION_SUPPORT_ENABLE
static fsp_err_t r_xspi_controller_automatic_calibration_seq(xspi_controller_instance_ctrl_t * p_instance_ctrl);

#endif

#if XSPI_CONTROLLER_CFG_ROW_ADDRESSING_SUPPORT_ENABLE
static void r_xspi_controller_row_load_store(xspi_controller_instance_ctrl_t * const p_ctrl,
                                             uint32_t                                row_index,
                                             bool                                    store_row);

#endif

#if XSPI_CONTROLLER_CFG_XIP_SUPPORT_ENABLE
static void r_xspi_controller_dummy_read(uint32_t * p_read_address);
static void r_xspi_controller_xip(xspi_controller_instance_ctrl_t * p_instance_ctrl, bool is_entering);

#endif

#if XSPI_CONTROLLER_CFG_DOTF_SUPPORT_ENABLE
static fsp_err_t r_xspi_controller_dotf_setup(uint8_t xspi_controller_unit, xspi_controller_dotf_cfg_t * p_dotf_cfg);
...
static uint32_t g_xspi_controller_channels_open_flags = 0;
...
const spi_flash_api_t g_xspi_controller_on_spi_flash =
{
    .open           = R_XSPI_CONTROLLER_Open,
    .directWrite    = R_XSPI_CONTROLLER_DirectWrite,
    .directRead     = R_XSPI_CONTROLLER_DirectRead,
    .directTransfer = R_XSPI_CONTROLLER_DirectTransfer,
    .spiProtocolSet = R_XSPI_CONTROLLER_SpiProtocolSet,
    .write          = R_XSPI_CONTROLLER_Write,
    .erase          = R_XSPI_CONTROLLER_Erase,
    .statusGet      = R_XSPI_CONTROLLER_StatusGet,
    .xipEnter       = R_XSPI_CONTROLLER_XipEnter,
    .xipExit        = R_XSPI_CONTROLLER_XipExit,
    .bankSet        = R_XSPI_CONTROLLER_BankSet,
    .close          = R_XSPI_CONTROLLER_Close,
    .autoCalibrate  = R_XSPI_CONTROLLER_AutoCalibrate,
};
...
{
    xspi_controller_instance_ctrl_t * p_instance_ctrl = (xspi_controller_instance_ctrl_t *) p_ctrl;
    fsp_err_t ret = FSP_SUCCESS;

#if XSPI_CONTROLLER_CFG_PARAM_CHECKING_ENABLE
    FSP_ASSERT(NULL != p_instance_ctrl);
    FSP_ASSERT(NULL != p_cfg);
    FSP_ASSERT(NULL != p_cfg->p_extend);
    FSP_ERROR_RETURN(XSPI_CONTROLLER_PRV_OPEN != p_instance_ctrl->open, FSP_ERR_ALREADY_OPEN);
    if (((xspi_controller_extended_cfg_t *) p_cfg->p_extend)->memory_type ==
        XSPI_CONTROLLER_DEVICE_TYPE_RAM)
    {
        FSP_ERROR_RETURN(SPI_FLASH_PROTOCOL_8D_8D_8D == p_cfg->spi_protocol, FSP_ERR_INVALID_ARGUMENT);
    }
#endif

    const xspi_controller_extended_cfg_t * const p_cfg_extend = (xspi_controller_extended_cfg_t *) (p_cfg->p_extend);

#if XSPI_CONTROLLER_CFG_PARAM_CHECKING_ENABLE
    FSP_ASSERT(BSP_PERIPHERAL_XSPI_CONTROLLER_CHANNEL_MASK & (1U << p_cfg_extend->xspi_controller_unit));
    FSP_ERROR_RETURN(0 == (g_xspi_controller_channels_open_flags & XSPI_CONTROLLER_PRV_CH_MASK(p_cfg_extend)),
                     FSP_ERR_ALREADY_OPEN);
#endif

    R_XSPI0_Type * p_reg = (R_XSPI0_Type *) (R_XSPI0_BASE +
                                             p_cfg_extend->xspi_controller_unit * (R_XSPI1_BASE - R_XSPI0_BASE));

    /* Enable clock to the xSPI block */
    R_BSP_MODULE_START(FSP_IP_XSPI, p_cfg_extend->xspi_controller_unit);
    ...
    p_instance_ctrl->xspi_controller_unit = p_cfg_extend->xspi_controller_unit;

#if XSPI_CONTROLLER_CFG_DOTF_SUPPORT_ENABLE
    if (NULL != p_cfg_extend->p_dotf_cfg)
    {
        ret = r_xspi_controller_dotf_setup(p_cfg_extend->xspi_controller_unit, p_cfg_extend->p_dotf_cfg);
        if (FSP_SUCCESS != ret)
        {
            /* If the DOTF initialization fails, stop the module if no other channels are active. */
            if (0 == (g_xspi_controller_channels_open_flags & XSPI_CONTROLLER_PRV_UNIT_MASK(p_cfg_extend)))
            {
                R_BSP_MODULE_STOP(FSP_IP_XSPI, p_cfg_extend->xspi_controller_unit);
            }
        }
    ...
    if (XSPI_CONTROLLER_DEVICE_NUMBER_0 == p_instance_ctrl->channel)
    {
#if BSP_FEATURE_XSPI_CONTROLLER_HAS_CHANNEL_1
        p_reg->BMCTL0 &= ~(R_XSPI0_BMCTL0_CH0CS0ACC_Msk | R_XSPI0_BMCTL0_CH1CS0ACC_Msk);
#else
        p_reg->BMCTL0 &= ~(R_XSPI0_BMCTL0_CH0CS0ACC_Msk);
#endif
    }
    else
    {
...
#else
        p_reg->BMCTL0 &= ~(R_XSPI0_BMCTL0_CH0CS1ACC_Msk);
#endif
    }

    /* Perform xSPI Initial configuration as described in hardware manual (see
     * "Flows of Operations" in the xSPI section of the relevant hardware manual). */
    ...
    /* Set output enable assertion extension in cycles. */
    uint32_t comcfg = ((uint32_t) p_cfg_extend->p_timing_settings->output_assert_delay << R_XSPI0_COMCFG_OEASTEX_Pos) &
                      R_XSPI0_COMCFG_OEASTEX_Msk;
    p_reg->COMCFG = comcfg;

    /* Set xSPI memory-mapping operation. */
    ret = r_xspi_controller_protocol_specific_settings(p_instance_ctrl);
    ...
    const uint32_t bmcfgch = (0 << R_XSPI0_BMCFGCH_WRMD_Pos) |
                             ((XSPI_CONTROLLER_CFG_COMBINATION_FUNCTION << R_XSPI0_BMCFGCH_MWRCOMB_Pos) &
                              (R_XSPI0_BMCFGCH_MWRCOMB_Msk | R_XSPI0_BMCFGCH_MWRSIZE_Msk)) |
                             ((XSPI_CONTROLLER_CFG_PREFETCH_FUNCTION << R_XSPI0_BMCFGCH_PREEN_Pos) &
                              R_XSPI0_BMCFGCH_PREEN_Msk);

    /* Both of these should have the same configuration and it affects all XSPI slave channels. */
    ...
    if (0 == p_instance_ctrl->channel)
    {
        ...
        p_reg->BMCTL0 |= R_XSPI0_BMCTL0_CH0CS0ACC_Msk | R_XSPI0_BMCTL0_CH1CS0ACC_Msk;
#else
        p_reg->BMCTL0 |= R_XSPI0_BMCTL0_CH0CS0ACC_Msk | XSPI_CONTROLLER_PRV_BMCTL0_RESERVED_MASK;
#endif
    }
    else
    {
        ...
        p_reg->BMCTL0 |= R_XSPI0_BMCTL0_CH0CS1ACC_Msk | R_XSPI0_BMCTL0_CH1CS1ACC_Msk;
#else
        p_reg->BMCTL0 |= R_XSPI0_BMCTL0_CH0CS1ACC_Msk | XSPI_CONTROLLER_PRV_BMCTL0_RESERVED_MASK;
#endif
    }

    if (FSP_SUCCESS == ret)
    {
        p_instance_ctrl->open                  = XSPI_CONTROLLER_PRV_OPEN;
        g_xspi_controller_channels_open_flags |= XSPI_CONTROLLER_PRV_CH_MASK(p_cfg_extend);
    }
    else if (0 == (g_xspi_controller_channels_open_flags & XSPI_CONTROLLER_PRV_UNIT_MASK(p_cfg_extend)))
    {
...
/*******************************************************************************************************************//**
 * Writes raw data directly to the OctaFlash. API not supported. Use R_XSPI_CONTROLLER_DirectTransfer
 *
 * Implements @ref spi_flash_api_t::directWrite.
 *
 * @retval FSP_ERR_UNSUPPORTED         API not supported by XSPI.
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_DirectWrite (spi_flash_ctrl_t    * p_ctrl,
...
/*******************************************************************************************************************//**
 * Reads raw data directly from the OctaFlash. API not supported. Use R_XSPI_CONTROLLER_DirectTransfer.
 *
 * Implements @ref spi_flash_api_t::directRead.
 *
 * @retval FSP_ERR_UNSUPPORTED         API not supported by XSPI.
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_DirectRead (spi_flash_ctrl_t * p_ctrl, uint8_t * const p_dest, uint32_t const bytes)
{
    FSP_PARAMETER_NOT_USED(p_ctrl);
 ...
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_DirectTransfer (spi_flash_ctrl_t                  * p_ctrl,
                                            spi_flash_direct_transfer_t * const p_transfer,
                                            spi_flash_direct_transfer_dir_t     direction)
{
    xspi_controller_instance_ctrl_t * p_instance_ctrl = (xspi_controller_instance_ctrl_t *) p_ctrl;
    ...
    FSP_ERROR_RETURN(XSPI_CONTROLLER_PRV_OPEN == p_instance_ctrl->open, FSP_ERR_NOT_OPEN);
#endif

    r_xspi_controller_direct_transfer(p_instance_ctrl, p_transfer, direction);
 ...
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_XipEnter (spi_flash_ctrl_t * p_ctrl)
{
#if XSPI_CONTROLLER_CFG_XIP_SUPPORT_ENABLE
    xspi_controller_instance_ctrl_t * p_instance_ctrl = (xspi_controller_instance_ctrl_t *) p_ctrl;
    ...
    FSP_ERROR_RETURN(XSPI_CONTROLLER_PRV_OPEN == p_instance_ctrl->open, FSP_ERR_NOT_OPEN);
    if (((xspi_controller_extended_cfg_t *) p_instance_ctrl->p_cfg->p_extend)->memory_type ==
        XSPI_CONTROLLER_DEVICE_TYPE_RAM)
    {
    ...
    r_xspi_controller_xip(p_instance_ctrl, true);
 ...
 * @retval FSP_ERR_UNSUPPORTED         XiP support is not enabled.
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_XipExit (spi_flash_ctrl_t * p_ctrl)
{
#if XSPI_CONTROLLER_CFG_XIP_SUPPORT_ENABLE
    xspi_controller_instance_ctrl_t * p_instance_ctrl = (xspi_controller_instance_ctrl_t *) p_ctrl;
    ...
    r_xspi_controller_xip(p_instance_ctrl, false);
 ...
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_Write (spi_flash_ctrl_t    * p_ctrl,
                                   uint8_t const * const p_src,
                                   ...
                                   uint32_t              byte_count)
{
 ...
 #if !XSPI_CONTROLLER_CFG_DMAC_SUPPORT_ENABLE
    FSP_ERROR_RETURN((byte_count & (XSPI_CONTROLLER_PRV_CPU_ACCESS_LENGTH - 1)) == 0, FSP_ERR_INVALID_SIZE);
    FSP_ERROR_RETURN(((uint32_t) p_dest & (XSPI_CONTROLLER_PRV_CPU_ACCESS_ALIGNMENT - 1)) == 0,
    ...
    FSP_ERROR_RETURN(false == r_xspi_controller_status_sub(p_instance_ctrl, p_instance_ctrl->p_cfg->write_status_bit),
                     FSP_ERR_DEVICE_BUSY);

#if XSPI_CONTROLLER_CFG_DMAC_SUPPORT_ENABLE
    xspi_controller_extended_cfg_t * p_cfg_extend = XSPI_CONTROLLER_PRV_EXTENDED_CFG(p_instance_ctrl);
    ...
    p_transfer->p_cfg->p_info->transfer_settings_u.transfer_settings_word_b.size = TRANSFER_SIZE_1_BYTE;
    p_transfer->p_cfg->p_info->transfer_settings_u.transfer_settings_word_b.mode = TRANSFER_MODE_NORMAL;
    ...
    r_xspi_controller_write_enable(p_instance_ctrl);
    ...
    if (XSPI_CONTROLLER_CFG_COMBINATION_FUNCTION != XSPI_CONTROLLER_COMBINATION_FUNCTION_DISABLE)
    {
        uint8_t combo_bytes = (uint8_t) (2U * ((uint8_t) XSPI_CONTROLLER_CFG_COMBINATION_FUNCTION + 1U));
        if (byte_count < combo_bytes)
        {
            p_reg->BMCTL1 = XSPI_CONTROLLER_PRV_BMCTL1_PUSH_COMBINATION_WRITE_MASK;
        }
    }

#else
    uint32_t * p_dest32 =
        (uint32_t *) (uintptr_t) ((uint32_t) p_dest & ~(XSPI_CONTROLLER_PRV_CPU_ACCESS_ALIGNMENT - 1));
    uint32_t * p_src32 = (uint32_t *) p_src;

    err = r_xspi_controller_write_enable(p_instance_ctrl);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
    ...
    while (sizeof(uint32_t) <= byte_count)
    {
         ...
         * "Combination Function" in the xSPI Operation section of the relevant hardware manual.
 ...
 * @retval FSP_ERR_DEVICE_BUSY         The device is busy.
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_Erase (spi_flash_ctrl_t * p_ctrl, uint8_t * const p_device_address, uint32_t byte_count)
{
    xspi_controller_instance_ctrl_t * p_instance_ctrl = (xspi_controller_instance_ctrl_t *) p_ctrl;
    ...
    xspi_controller_command_set_t const * p_cmd_set = p_instance_ctrl->p_cmd_set;
    ...
    /* Start with a full 32-bit mask with the address MSB mask in the lower byte. */
    uint32_t addr_mask = (uint32_t) (~UINT8_MAX | p_cmd_set->address_msb_mask);

    /* Shift the MSB to the correct position. */
    addr_mask <<= 8U * (XSPI_CONTROLLER_PRV_ADDR_BYTES_TO_LENGTH(p_cmd_set->address_bytes) - 1U);

    /* Mask off the address bits that aren't needed. */
    uint32_t chip_address = (uint32_t) ((uint32_t) p_device_address & ~addr_mask);

    FSP_ERROR_RETURN(false == r_xspi_controller_status_sub(p_instance_ctrl, p_cfg->write_status_bit),
                     FSP_ERR_DEVICE_BUSY);
    ...
    fsp_err_t err = r_xspi_controller_write_enable(p_instance_ctrl);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
    ...
    spi_flash_direct_transfer_t direct_command =
    {
        ...
        .address_length =
            (uint8_t) ((send_address) ? XSPI_CONTROLLER_PRV_ADDR_BYTES_TO_LENGTH(p_cmd_set->address_bytes) : 0U),
        .data_length    = 0,
    };

    r_xspi_controller_direct_transfer(p_instance_ctrl, &direct_command, SPI_FLASH_DIRECT_TRANSFER_DIR_WRITE);

    /* If prefetch is enabled, make sure the banks aren't being used and flush the prefetch caches after an erase. */
#if XSPI_CONTROLLER_CFG_PREFETCH_FUNCTION
    R_XSPI0_Type * const p_reg = p_instance_ctrl->p_reg;
    FSP_HARDWARE_REGISTER_WAIT((p_reg->COMSTT & XSPI_CONTROLLER_PRV_COMSTT_MEMACCCH_MASK), 0);
    p_reg->BMCTL1 = XSPI_CONTROLLER_PRV_BMCTL1_CLEAR_PREFETCH_MASK;
#endif
 ...
 * @retval FSP_ERR_NOT_OPEN            Driver is not opened.
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_StatusGet (spi_flash_ctrl_t * p_ctrl, spi_flash_status_t * const p_status)
{
    xspi_controller_instance_ctrl_t * p_instance_ctrl = (xspi_controller_instance_ctrl_t *) p_ctrl;
    ...
    /* Read device status. */
    p_status->write_in_progress =
        r_xspi_controller_status_sub(p_instance_ctrl, p_instance_ctrl->p_cfg->write_status_bit);

    return FSP_SUCCESS;
}

/*******************************************************************************************************************//**
 * Selects the bank to access. Use XSPI_CONTROLLER_bank_select_t as the bank value.
 *
 * Implements @ref spi_flash_api_t::bankSet.
 *
 * @retval FSP_ERR_UNSUPPORTED           This function is unsupported.
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_BankSet (spi_flash_ctrl_t * p_ctrl, uint32_t bank)
{
    FSP_PARAMETER_NOT_USED(p_ctrl);
 ...
 * @retval FSP_ERR_CALIBRATE_FAILED   Failed to perform auto-calibrate.
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_SpiProtocolSet (spi_flash_ctrl_t * p_ctrl, spi_flash_protocol_t spi_protocol)
{
    xspi_controller_instance_ctrl_t * p_instance_ctrl = (xspi_controller_instance_ctrl_t *) p_ctrl;
    ...
    FSP_ASSERT(NULL != p_ctrl);
    FSP_ERROR_RETURN(XSPI_CONTROLLER_PRV_OPEN == p_instance_ctrl->open, FSP_ERR_NOT_OPEN);
    xspi_controller_extended_cfg_t * p_cfg_extend = (xspi_controller_extended_cfg_t *) p_instance_ctrl->p_cfg->p_extend;
    if (p_cfg_extend->memory_type == XSPI_CONTROLLER_DEVICE_TYPE_RAM)
    {
    ...
    fsp_err_t err = r_xspi_controller_protocol_specific_settings(p_instance_ctrl);
 ...
 * @retval FSP_ERR_NOT_OPEN        Driver is not opened.
 **********************************************************************************************************************/
fsp_err_t R_XSPI_CONTROLLER_Close (spi_flash_ctrl_t * p_ctrl)
{
    xspi_controller_instance_ctrl_t * p_instance_ctrl = (xspi_controller_instance_ctrl_t *) p_ctrl;
    fsp_err_t err = FSP_SUCCESS;
...
/*******************************************************************************************************************//**
 * AutoCalibrate the XSPI_CONTROLLER DS signal.
 *
...
fsp_err_t R_XSPI_CONTROLLER_AutoCalibrate (spi_flash_ctrl_t * const p_ctrl)
{
    ...
    if (XSPI_CONTROLLER_PRV_PROTOCOL_USES_DS_SIGNAL(p_instance_ctrl->spi_protocol))
    {
        return r_xspi_controller_automatic_calibration_seq(p_instance_ctrl);
    }
 ...
 * @param     row_index Index of the row in the XSPI device.
 *
...
fsp_err_t R_XSPI_CONTROLLER_RowLoad (spi_flash_ctrl_t * const p_ctrl, uint32_t row_index)
{
 ...
 #if XSPI_CONTROLLER_CFG_PARAM_CHECKING_ENABLE
    FSP_ASSERT(NULL != p_ctrl);
    FSP_ERROR_RETURN(XSPI_CONTROLLER_PRV_OPEN == ((xspi_controller_instance_ctrl_t *) p_ctrl)->open, FSP_ERR_NOT_OPEN);

    xspi_controller_extended_cfg_t * const p_extend_cfg = XSPI_CONTROLLER_PRV_EXTENDED_CFG(p_ctrl);
    FSP_ERROR_RETURN(row_index < (1ULL << (8U * XSPI_CONTROLLER_PRV_ADDR_BYTES_TO_LENGTH(
                                               p_extend_cfg->row_index_bytes))),
                     FSP_ERR_INVALID_ARGUMENT);
 #endif

    r_xspi_controller_row_load_store((xspi_controller_instance_ctrl_t *) p_ctrl, row_index, false);
...
fsp_err_t R_XSPI_CONTROLLER_RowStore (spi_flash_ctrl_t * const p_ctrl, uint32_t row_index)
{
    ...
    r_xspi_controller_row_load_store((xspi_controller_instance_ctrl_t *) p_ctrl, row_index, true);
 ...
 * Configure DOTF
 *
 * @param[in]   p_ctrl                          Pointer to XSPI specific control structure
                                            ...
                                            xspi_controller_dotf_cfg_t * const p_dotf_cfg)
{
    ...
    xspi_controller_extended_cfg_t * p_cfg_extend = XSPI_CONTROLLER_PRV_EXTENDED_CFG(p_ctrl);

    if (NULL != p_dotf_cfg)
    {
        ret = r_xspi_controller_dotf_setup(p_cfg_extend->xspi_controller_unit, p_dotf_cfg);
        if (FSP_SUCCESS != ret)
        {
            /* If the DOTF initialization fails, stop the module if no other channels are active. */
            if (0 == (g_xspi_controller_channels_open_flags & XSPI_CONTROLLER_PRV_UNIT_MASK(p_cfg_extend)))
 ...
 * @retval      FSP_ERR_INVALID_MODE       Invalid configuration for p_cmd_set.
 **********************************************************************************************************************/
static fsp_err_t r_xspi_controller_protocol_specific_settings (xspi_controller_instance_ctrl_t * p_instance_ctrl)
{
    R_XSPI0_Type * const p_reg = p_instance_ctrl->p_reg;
    fsp_err_t            ret   = FSP_SUCCESS;

    /* Get the command set for the configured protocol and save it to the control struct. */
    xspi_controller_command_set_t const * p_cmd_set = r_xspi_controller_command_set_get(p_instance_ctrl);
    ...
    liocfg |=
        (((uint32_t) p_cmd_set->write_data_mask << R_XSPI0_LIOCFGCS_WRMSKMD_Pos) & R_XSPI0_LIOCFGCS_WRMSKMD_Msk);
    ...
    uint32_t cmcfg0 = ((uint32_t) (p_cmd_set->address_msb_mask << R_XSPI0_CMCFGCS_CMCFG0_ADDRPEN_Pos)) |
                      ...
                      ((uint32_t) p_cmd_set->address_mode << R_XSPI0_CMCFGCS_CMCFG0_ARYAMD_Pos);
    ...
    {
        cmcfg0 |= XSPI_CONTROLLER_PRV_ADDRESS_REPLACE_ENABLE_BITS;
    }
    ...
    if (XSPI_CONTROLLER_COMMAND_BYTES_1 == p_cmd_set->command_bytes)
    {
        read_command =
            (uint16_t) ((read_command & XSPI_CONTROLLER_PRV_CDTBUF_CMD_1B_VALUE_MASK) <<
                        XSPI_CONTROLLER_PRV_CDTBUF_CMD_1B_VALUE_SHIFT);
        write_command =
            (uint16_t) ((write_command & XSPI_CONTROLLER_PRV_CDTBUF_CMD_1B_VALUE_MASK) <<
                        XSPI_CONTROLLER_PRV_CDTBUF_CMD_1B_VALUE_SHIFT);
    }
    ...
    if ((0 == p_cfg_extend->data_latch_delay_clocks) &&
        XSPI_CONTROLLER_PRV_PROTOCOL_USES_DS_SIGNAL(p_instance_ctrl->spi_protocol))
    {
        ret = r_xspi_controller_automatic_calibration_seq(p_instance_ctrl);
    }
#endif
 ...
 **********************************************************************************************************************/
static bool r_xspi_controller_status_sub (xspi_controller_instance_ctrl_t * p_instance_ctrl, uint8_t bit_pos)
{
    xspi_controller_command_set_t const * p_cmd_set = p_instance_ctrl->p_cmd_set;
    ...
    spi_flash_direct_transfer_t direct_command =
    {
        ...
        .address_length = (uint8_t) (p_cmd_set->status_needs_address ?
                                     XSPI_CONTROLLER_PRV_ADDR_BYTES_TO_LENGTH(p_cmd_set->status_address_bytes) : 0U),
    ...
    {
        direct_command.address_length = XSPI_CONTROLLER_PRV_ADDR_BYTES_TO_LENGTH(p_cmd_set->address_bytes);
    }

    r_xspi_controller_direct_transfer(p_instance_ctrl, &direct_command, SPI_FLASH_DIRECT_TRANSFER_DIR_READ);

    return (bool) ((direct_command.data_u.data >> bit_pos) & 1U);
}
 ...
 * @retval      FSP_ERR_NOT_ENABLED Write enable failed.
 **********************************************************************************************************************/
static fsp_err_t r_xspi_controller_write_enable (xspi_controller_instance_ctrl_t * p_instance_ctrl)
{
    xspi_controller_command_set_t const * const p_cmd_set = p_instance_ctrl->p_cmd_set;
...
#if XSPI_CONTROLLER_MAX_WRITE_ENABLE_LOOPS > 0U

    /* Verify write is enabled. */
    for (uint32_t i = 0U; i < XSPI_CONTROLLER_MAX_WRITE_ENABLE_LOOPS; i++)
    {
        write_enabled = r_xspi_controller_status_sub(p_instance_ctrl, p_instance_ctrl->p_cfg->write_enable_bit);
        if (write_enabled)
        {
            break;
        }
...
static fsp_err_t r_xspi_controller_automatic_calibration_seq (xspi_controller_instance_ctrl_t * p_instance_ctrl)
{
    ...
    xspi_controller_device_number_t channel = p_instance_ctrl->channel;
    ...
    uint16_t      read_command      = p_cmd_set->read_command;
    const uint8_t read_dummy_cycles = p_cmd_set->read_dummy_cycles;
    const uint8_t address_bytes     = XSPI_CONTROLLER_PRV_ADDR_BYTES_TO_LENGTH(p_cmd_set->address_bytes);
    ...
    p_reg->CCCTLCS[channel].CCCTL1 =
        ...
        (XSPI_CONTROLLER_PRV_AUTOCALIBRATION_DATA_SIZE << R_XSPI0_CCCTLCS_CCCTL1_CADATASIZE_Pos) |
        (XSPI_CONTROLLER_PRV_AUTOCALIBRATION_LATENCY_CYCLES << R_XSPI0_CCCTLCS_CCCTL1_CAWRLATE_Pos) |
    ...
    p_reg->CCCTLCS[channel].CCCTL3 = (uint32_t) p_cfg_extend->p_autocalibration_preamble_pattern_addr &
                                     XSPI_CONTROLLER_PRV_ADDRESS_REPLACE_MASK;

    if (XSPI_CONTROLLER_DEVICE_TYPE_FLASH == p_cfg_extend->memory_type)
    {
        p_reg->CCCTLCS[channel].CCCTL4 = XSPI_CONTROLLER_PRV_AUTOCALIBRATION_PREAMBLE_PATTERN_0;
        p_reg->CCCTLCS[channel].CCCTL5 = XSPI_CONTROLLER_PRV_AUTOCALIBRATION_PREAMBLE_PATTERN_1;
        p_reg->CCCTLCS[channel].CCCTL6 = XSPI_CONTROLLER_PRV_AUTOCALIBRATION_PREAMBLE_PATTERN_2;
        p_reg->CCCTLCS[channel].CCCTL7 = XSPI_CONTROLLER_PRV_AUTOCALIBRATION_PREAMBLE_PATTERN_3;
    }
    else
    {
        p_reg->CCCTLCS[channel].CCCTL4 = XSPI_CONTROLLER_PRV_AUTOCALIBRATION_RAM_PREAMBLE_PATTERN_0;
        p_reg->CCCTLCS[channel].CCCTL5 = XSPI_CONTROLLER_PRV_AUTOCALIBRATION_RAM_PREAMBLE_PATTERN_1;
        p_reg->CCCTLCS[channel].CCCTL6 = XSPI_CONTROLLER_PRV_AUTOCALIBRATION_RAM_PREAMBLE_PATTERN_2;
        p_reg->CCCTLCS[channel].CCCTL7 = XSPI_CONTROLLER_PRV_AUTOCALIBRATION_RAM_PREAMBLE_PATTERN_3;
    }

    p_reg->CCCTLCS[channel].CCCTL0 =
        (XSPI_CONTROLLER_PRV_AUTOCALIBRATION_NO_WRITE_CMD << R_XSPI0_CCCTLCS_CCCTL0_CANOWR_Pos) |
        (XSPI_CONTROLLER_PRV_AUTOCALIBRATION_FRAME_INTERVAL <<
            R_XSPI0_CCCTLCS_CCCTL0_CAITV_Pos) |
        (XSPI_CONTROLLER_PRV_AUTOCALIBRATION_SHIFT_DS_END_VALUE <<
    ...
    while ((0 == ((p_reg->INTS >> (R_XSPI0_INTS_CASUCCS0_Pos + channel)) & 0x01)) &&
           (0 == ((p_reg->INTS >> (R_XSPI0_INTS_CAFAILCS0_Pos + channel)) & 0x01)))
    {
        /* Do nothing. */
    }
    ...
    if (1 == ((p_reg->INTS >> (R_XSPI0_INTS_CASUCCS0_Pos + channel)) & 0x01))
    {
        /* Clear automatic calibration success status */
        p_reg->INTC = (uint32_t) 1 << (R_XSPI0_INTS_CASUCCS0_Pos + channel);
    }
    else if (1 == ((p_reg->INTS >> (R_XSPI0_INTS_CAFAILCS0_Pos + channel)) & 0x01))
    {
        ret = FSP_ERR_CALIBRATE_FAILED;

        /* Clear automatic calibration failure status */
        p_reg->INTC = (uint32_t) 1 << (R_XSPI0_INTS_CAFAILCS0_Pos + channel);
    }
    else
    {
        /* Do nothing. */
 ...
 **********************************************************************************************************************/
static void r_xspi_controller_direct_transfer (xspi_controller_instance_ctrl_t   * p_instance_ctrl,
                                               spi_flash_direct_transfer_t * const p_transfer,
                                               spi_flash_direct_transfer_dir_t     direction)
{
    R_XSPI0_Type * const p_reg = p_instance_ctrl->p_reg;
    const xspi_controller_device_number_t channel = p_instance_ctrl->channel;
    ...
    cdtbuf0 |= (1 == p_transfer->command_length) ?
               ((p_transfer->command & XSPI_CONTROLLER_PRV_CDTBUF_CMD_1B_VALUE_MASK) <<
                XSPI_CONTROLLER_PRV_CDTBUF_CMD_UPPER_OFFSET) :
               ((p_transfer->command & XSPI_CONTROLLER_PRV_CDTBUF_CMD_2B_VALUE_MASK) <<
                XSPI_CONTROLLER_PRV_CDTBUF_CMD_OFFSET);
     ...
     * (see "Flow of Manual-command Procedure" in the xSPI section of the relevant hardware manual). */
    ...
    {
        if (0U == p_transfer->data_length)
        {
            p_reg->CDBUF[0].CDD0 = 0U;
        }
        else if (p_transfer->data_length > sizeof(uint32_t))
        {
            p_reg->CDBUF[0].CDD0 = (uint32_t) (p_transfer->data_u.data_u64 & UINT32_MAX);
            p_reg->CDBUF[0].CDD1 = (uint32_t) (p_transfer->data_u.data_u64 >> XSPI_CONTROLLER_PRV_UINT32_BITS);
        }
        else
        {
            p_reg->CDBUF[0].CDD0 = p_transfer->data_u.data;
        }
    }
    ...
    if (SPI_FLASH_DIRECT_TRANSFER_DIR_READ == direction)
    {
        ...
        {
            p_transfer->data_u.data_u64  = p_reg->CDBUF[0].CDD0;
            p_transfer->data_u.data_u64 |= (((uint64_t) p_reg->CDBUF[0].CDD1) << XSPI_CONTROLLER_PRV_UINT32_BITS);
        }
        else
        {
            p_transfer->data_u.data = p_reg->CDBUF[0].CDD0;
        }
    }
 ...
 * toggling of b3 ensures the next dummy read will occur at an address that is not currently available in
 * the XSPI_CONTROLLER peripheral's 1-line (8-byte) data read buffer (this is not the same as the prefetch buffer).
 * If the same address is read twice in a row, the peripheral may return the buffered value instead of issuing
...
static void r_xspi_controller_dummy_read (uint32_t * p_read_address)
{
    ...
    {
        dummy_value     = *((volatile uint64_t *) (uintptr_t) *p_read_address);
...
static void r_xspi_controller_xip (xspi_controller_instance_ctrl_t * p_instance_ctrl, bool is_entering)
{
    ...
    if (0U == p_instance_ctrl->xspi_controller_unit)
    {
        dummy_read_address = ((XSPI_CONTROLLER_DEVICE_NUMBER_0 == p_instance_ctrl->channel) ?
                              BSP_XSPI0_CS0_START_ADDRESS :
                              BSP_XSPI0_CS1_START_ADDRESS);
    }
    else if (1U == p_instance_ctrl->xspi_controller_unit)
    {
        dummy_read_address = ((XSPI_CONTROLLER_DEVICE_NUMBER_0 == p_instance_ctrl->channel) ?
                              BSP_XSPI1_CS0_START_ADDRESS :
                              BSP_XSPI1_CS1_START_ADDRESS);
    }
    else
    {
        /* Not a valid unit. */
    ...
    if (XSPI_CONTROLLER_COMBINATION_FUNCTION_DISABLE != XSPI_CONTROLLER_CFG_COMBINATION_FUNCTION)
    {
    ...
    r_xspi_controller_dummy_read(&dummy_read_address);
    ...
    FSP_HARDWARE_REGISTER_WAIT((p_reg->COMSTT & XSPI_CONTROLLER_PRV_COMSTT_PENDING_ACTION_MASK), 0);

    if (is_entering)
    {
        /* Change memory-mapping to read-only mode. */
        p_reg->BMCTL0 = XSPI_CONTROLLER_PRV_BMCTL0_READ_ONLY_VALUE;

        /* XiP enter/exit codes are configured only for memory mapped operations and affects both XSPI slave channels. */
    ...
    else
    {
        ...
        p_reg->BMCTL0 = XSPI_CONTROLLER_PRV_BMCTL0_READ_WRITE_VALUE;
    }
    ...
    xspi_controller_instance_ctrl_t * p_instance_ctrl)
{
    ...
    xspi_controller_command_set_t * p_cmd_set;
    for (uint32_t i = 0; i < p_cfg_extend->p_xspi_command_set->length; i++)
    {
        p_cmd_set = &((xspi_controller_command_set_t *) p_cfg_extend->p_xspi_command_set->p_table)[i];
...
/*******************************************************************************************************************//**
 * Handles row load/stores for row addressable XSPI devices.
 * @param[in] p_ctrl    Pointer to the instance constrol struct.
 * @param     row_index Index of the page in the XSPI device.
...
static void r_xspi_controller_row_load_store (xspi_controller_instance_ctrl_t * const p_ctrl,
                                              ...
                                              bool                                    store_row)
{
    xspi_controller_extended_cfg_t * const p_extend_cfg =
        (xspi_controller_extended_cfg_t *) p_ctrl->p_cfg->p_extend;
    xspi_controller_command_set_t const * const p_cmd_set = p_ctrl->p_cmd_set;
    spi_flash_direct_transfer_t                 transfer  =
    {
        .command_length = (uint8_t) p_cmd_set->command_bytes,
        .address        = 0U,
        .address_length = 0U,
        .data_u.data    = row_index,
        .data_length    = XSPI_CONTROLLER_PRV_ADDR_BYTES_TO_LENGTH(p_extend_cfg->row_index_bytes)
    };
    ...
    r_xspi_controller_direct_transfer(p_ctrl, &transfer, SPI_FLASH_DIRECT_TRANSFER_DIR_WRITE);
}
 ...
 **********************************************************************************************************************/
static fsp_err_t r_xspi_controller_dotf_side_channal_seed_init (uint32_t p_seed[])
{
    ...
    if (FSP_SUCCESS == err)
    {
        ...
        p_seed[1] = (random_number[1] & XSPI_CONTROLLER_PRV_DOTF_SEED_1_SEED_MASK) |
                    XSPI_CONTROLLER_PRV_DOTF_SEED_1_ENABLE_MASK;
    }
 ...
 *
 * @param[in]   xspi_controller_unit                       Which XSPI unit
...
static fsp_err_t r_xspi_controller_dotf_setup (uint8_t xspi_controller_unit, xspi_controller_dotf_cfg_t * p_dotf_cfg)
{
    ...
    uint32_t       seed[2] = {0};
    R_DOTF0_Type * p_reg   = (R_DOTF0_Type *) (R_DOTF0_BASE + (xspi_controller_unit * (R_DOTF1_BASE - R_DOTF0_BASE)));

    if (XSPI_CONTROLLER_DOTF_KEY_FORMAT_WRAPPED != p_dotf_cfg->format)
    {
    ...
    if (XSPI_CONTROLLER_DOTF_AES_KEY_TYPE_128 == p_dotf_cfg->key_type)
    {
        p_wrapped_key->type = RSIP_KEY_TYPE_AES_128;
        memcpy(p_wrapped_key->p_value, p_dotf_cfg->p_key, RSIP_BYTE_SIZE_WRAPPED_KEY_AES_128);
    }
    else if (XSPI_CONTROLLER_DOTF_AES_KEY_TYPE_192 == p_dotf_cfg->key_type)
    {
        return FSP_ERR_INVALID_ARGUMENT;
    }
    else if (XSPI_CONTROLLER_DOTF_AES_KEY_TYPE_256 == p_dotf_cfg->key_type)
    {
    ...
    sce_ret = r_xspi_controller_dotf_side_channal_seed_init(seed);
    if (FSP_SUCCESS != sce_ret)
    {
        return sce_ret;
    }
    ...
    sce_ret =
        gp_rsip_instance->p_api->otfInit(gp_rsip_instance->p_ctrl,
                                         (rsip_otf_channel_t) (xspi_controller_unit),
                                         p_wrapped_key,
                                         (uint8_t *) &seed[0]);
    ...
    if (sce_ret == FSP_SUCCESS)
    {
        ...
        p_reg->REG00 = (XSPI_CONTROLLER_PRV_DOTF_REG00_RESET_VALUE | R_DOTF_REG00_B09_Msk);

        /* Load the IV. */
        p_reg->REG03 = bswap_32big(p_dotf_cfg->p_iv[0]);
        p_reg->REG03 = bswap_32big(p_dotf_cfg->p_iv[1]);
        p_reg->REG03 = bswap_32big(p_dotf_cfg->p_iv[2]);
        p_reg->REG03 = bswap_32big(p_dotf_cfg->p_iv[3]);
    }
    ...
    p_reg->CONVAREAED = (uint32_t) p_dotf_cfg->p_end_addr;
    p_reg->CONVAREAST = (uint32_t) p_dotf_cfg->p_start_addr;
...
{
    fsp_err_t      sce_ret = FSP_SUCCESS;
    uint32_t       unit    = xspi_controller_unit;
    uint32_t       seed[2] = {0};
    ...
    if (XSPI_CONTROLLER_DOTF_KEY_FORMAT_PLAINTEXT == p_dotf_cfg->format)
    {
    ...
    if (sce_ret == FSP_SUCCESS)
    {
        ...
        /* Load the IV. */
        p_reg->REG03 = change_endian_long(p_dotf_cfg->p_iv[0]);
        p_reg->REG03 = change_endian_long(p_dotf_cfg->p_iv[1]);
        p_reg->REG03 = change_endian_long(p_dotf_cfg->p_iv[2]);
        p_reg->REG03 = change_endian_long(p_dotf_cfg->p_iv[3]);
    }
