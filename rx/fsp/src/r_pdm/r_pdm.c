...
/* Mask for Clear flag: Sound detection, Short circuit, Overvoltage lower limit exceeded, Overvoltage upper limit exceeded, Buffer overwriting*/
...
/* Mask for Error detection Short circuit, Overvoltage lower limit exceeded, Overvoltage upper limit exceeded, Buffer overwriting*/
...
fsp_err_t R_PDM_Open (pdm_ctrl_t * const p_ctrl, pdm_cfg_t const * const p_cfg)
{
    ...
    /* Enable PDM module */
    R_BSP_MODULE_START(FSP_IP_PDMIF, p_cfg->unit);
    ...
    /* Set Channel's Data Buffer Trigger Interrupt Threshold*/
    p_reg->PDDBCR = (uint32_t) p_extend->interrupt_threshold;

    /* Set Channel's Short Circuit Error Detection Control */
                       ...
                       uint32_t const     number_of_data_to_callback)
{
    ...
    if ((p_cfg->err_irq >= 0) && (0 == p_reg->PDICR_b.IEDE))
    {
        ...
        R_BSP_IrqCfgEnable(p_cfg->err_irq, (uint32_t) p_cfg->err_ipl, p_instance_ctrl);
    }
    ...
    if (p_cfg->dat_irq >= 0)
    {
        R_BSP_IrqCfgEnable(p_cfg->dat_irq, (uint32_t) p_cfg->dat_ipl, p_instance_ctrl);
    }
...
fsp_err_t R_PDM_SoundDetectionEnable (pdm_ctrl_t * const p_ctrl, pdm_sound_detection_setting_t sound_detection_setting)
{
    ...
    if (p_cfg->sdet_irq >= 0)
    {
        ...
        R_BSP_IrqCfgEnable(p_cfg->sdet_irq, (uint32_t) p_cfg->sdet_ipl, p_instance_ctrl);
    }
...
fsp_err_t R_PDM_StatusGet (pdm_ctrl_t * const p_ctrl, pdm_status_t * const p_status)
{
    ...
    p_status->sound_detection_enabled = (bool) (0 != p_reg->PDICR_b.ISDE);
...
fsp_err_t R_PDM_Close (pdm_ctrl_t * const p_ctrl)
{
    ...
    /* Stop PDM according to "Stop flow" from the "Setting Flow" section of the user manual. */
...
static fsp_err_t r_pdm_dependent_drivers_configure (pdm_instance_ctrl_t * p_instance_ctrl)
{
    ...
    if (NULL != p_transfer_rx)
    {
        ...
        p_transfer_info->transfer_settings_u.transfer_settings_word_b.size = TRANSFER_SIZE_4_BYTE;
        p_transfer_info->transfer_settings_u.transfer_settings_word_b.mode = TRANSFER_MODE_BLOCK;
        p_transfer_info->transfer_settings_u.transfer_settings_word_b.irq  = TRANSFER_IRQ_END;
