...
fsp_err_t RM_BLOCK_MEDIA_RAM_MediaInit (rm_block_media_ctrl_t * const p_ctrl)
{
    ...
    p_start_address = &g_block_media_ram_area[0];

    /* Clear the media area */
    memset(p_start_address, 0, RM_BLOCK_MEDIA_RAM_CFG_MEDIA_SIZE);

    p_adr = p_start_address;

    /* Copy the boot sector. */
    memcpy(p_adr, &g_block_media_ram_boot_sector, RM_BLOCK_MEDIA_RAM_SECTOR_SIZE);
    p_adr += (RM_BLOCK_MEDIA_RAM_SECTOR_SIZE * RM_BLOCK_MEDIA_RAM_RESERVED_SECTORS);

    /* Copy the fat table to the first fat copy. */
    memcpy(p_adr, &g_block_media_ram_tablefat, sizeof(g_block_media_ram_tablefat));
    p_adr += (RM_BLOCK_MEDIA_RAM_SECTOR_SIZE * RM_BLOCK_MEDIA_RAM_SECTORS_PER_FAT);
                                   ...
                                   uint32_t const                num_blocks)
{
    ...
    memcpy(p_dest_address, p_start_address + (block_address * RM_BLOCK_MEDIA_RAM_SECTOR_SIZE),
                                    ...
                                    uint32_t const                num_blocks)
{
    ...
    memcpy(p_start_address + (block_address * RM_BLOCK_MEDIA_RAM_SECTOR_SIZE), p_src_address,
           (RM_BLOCK_MEDIA_RAM_SECTOR_SIZE * num_blocks));
                                    ...
                                    uint32_t const                num_blocks)
{
    ...
    memset(p_start_address + (block_address * RM_BLOCK_MEDIA_RAM_SECTOR_SIZE), 0,
           (RM_BLOCK_MEDIA_RAM_SECTOR_SIZE * num_blocks));
