...
#define RMAC_REG_SIZE                          (0x2000UL)
#define ETHA_REG_SIZE                          (0x2000UL)
...
#define RMAC_PHY_ETHA_CONFIG_MODE              (0x02)
#define RMAC_PHY_ETHA_OPERATION_MODE           (0x03)
#define RMAC_PHY_ETHA_DISABLE_MODE             (0x01)
...
extern void rmac_phy_target_ksz8081mnx_initialize(rmac_phy_instance_ctrl_t * p_instance_ctrl);
extern bool rmac_phy_target_ksz8081mnx_is_support_link_partner_ability(rmac_phy_instance_ctrl_t * p_instance_ctrl,
                                                                       uint32_t                   line_speed_duplex);
                                            ...
                                            uint32_t * const         p_partner_pause)
{
    ...
    if (mpic != p_reg_rmac->MPIC)
    {
        ...
        r_rmac_phy_set_operation_mode(p_instance_ctrl,
                                      (uint32_t) p_instance_ctrl->phy_lsi_cfg_index,
                                      RMAC_PHY_ETHA_DISABLE_MODE);
        r_rmac_phy_set_operation_mode(p_instance_ctrl,
                                      (uint32_t) p_instance_ctrl->phy_lsi_cfg_index,
                                      RMAC_PHY_ETHA_CONFIG_MODE);
...
#else
        FSP_PARAMETER_NOT_USED(p_reg_rmac);
...
static void rmac_phy_targets_initialize (rmac_phy_instance_ctrl_t * p_instance_ctrl)
{
    ...
    switch (p_extend->p_phy_lsi_cfg_list[p_instance_ctrl->phy_lsi_cfg_index]->type)
    {
        ...
        {
            rmac_phy_target_ksz8081mnx_initialize(p_instance_ctrl);
            break;
        }
#endif
                                                              ...
                                                              uint32_t                   line_speed_duplex)
{
    ...
    switch (p_extend->p_phy_lsi_cfg_list[p_instance_ctrl->phy_lsi_cfg_index]->type)
    {
        ...
        case ETHER_PHY_LSI_TYPE_KSZ8081MNX:
        {
            result = rmac_phy_target_ksz8081mnx_is_support_link_partner_ability(p_instance_ctrl, line_speed_duplex);
            break;
        }
#endif
...
static uint8_t r_rmac_phy_get_operation_mode (rmac_phy_instance_ctrl_t * p_instance_ctrl)
{
    ...
    return (uint8_t) p_etha_reg->EAMS_b.OPS;
}
    ...
    switch (p_instance_ctrl->p_ether_phy_cfg->mii_type)
    {
        case ETHER_PHY_MII_TYPE_RMII:
        {
            *p_miicr_register = 2;
            ...
            R_ESWM->MIIRR = R_ESWM->MIIRR | (uint32_t) (1 << (R_ESWM_MIIRR_RMRST0_Pos + port));
            break;
        }
        ...
        case ETHER_PHY_MII_TYPE_RGMII:
        {
            ...
            *p_miicr_register = (R_ESWM_MIICR0_TXCIDE_Msk | 1);
            ...
            R_ESWM->MIIRR = R_ESWM->MIIRR | (uint32_t) (1 << (R_ESWM_MIIRR_RGRST0_Pos + port));
            break;
        }
