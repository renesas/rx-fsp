...
fsp_err_t RM_BLOCK_MEDIA_SPI_Open (rm_block_media_ctrl_t * const p_ctrl, rm_block_media_cfg_t const * const p_cfg)
{
    ...
    /* A nonzero write frame size must not exceed the lower driver's page size and must divide it evenly. */
    uint32_t write_frame_size = p_extended_cfg->write_frame_size_bytes;
    uint32_t page_size        = p_spi_flash->p_cfg->page_size_bytes;
    FSP_ERROR_RETURN((0U < write_frame_size) &&
                     ((write_frame_size <= page_size) && (0U == (page_size % write_frame_size))),
...
fsp_err_t RM_BLOCK_MEDIA_SPI_StatusGet (rm_block_media_ctrl_t * const p_ctrl, rm_block_media_status_t * const p_status)
{
    ...
    bool block_driver_busy = (bool) (p_instance_ctrl->read_in_progress || p_instance_ctrl->write_in_progress ||
                                     p_instance_ctrl->erase_in_progress);

    p_status->media_inserted = true;
    p_status->initialized    = p_instance_ctrl->initialized;
    p_status->busy           = (bool) (spi_driver_busy || block_driver_busy);
                                    ...
                                    uint32_t const                num_blocks)
{
    ...
    err = rm_block_media_spi_program(p_instance_ctrl, (uint8_t *) (uintptr_t) rom_address, p_src, num_blocks);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
                                    ...
                                    uint32_t const                num_blocks)
{
    ...
    p_spi_flash = (spi_flash_instance_t *) p_extended_cfg->p_spi;
    rom_address = (uint8_t *) (uintptr_t) p_extended_cfg->base_address + (start_block * block_size);
                                             ...
                                             uint32_t const                       num_blocks)
{
    ...
    /* Get the data size for each spi_flash_api_t::write call*/
    uint32_t frame_data_size = p_extended_cfg->write_frame_size_bytes;

    /* Write data into the block media SPI flash */
    p_instance_ctrl->write_in_progress = true;
    uint32_t page_size_flash;
    uint32_t data_offset;
    while ((iteration > 0U) && (FSP_SUCCESS == err))
    {
        page_size_flash = page_size;
        data_offset     = 0;
        while (0 < page_size_flash)
        {
            err =
                p_spi_flash->p_api->write(p_spi_flash->p_ctrl,
                                          (p_buffer + data_offset),
                                          (p_device_address + data_offset),
                                          frame_data_size);
            FSP_ERROR_RETURN(FSP_SUCCESS == err, err);

            /* Update the remaining data byte for write data */
            page_size_flash = page_size_flash - frame_data_size;

            /* Increment to move to next position for write data */
            data_offset = data_offset + frame_data_size;
                                              ...
                                              uint32_t const                       num_blocks)
{
    ...
    memcpy(p_dest, (uint8_t *) (uintptr_t) rom_address, read_bytes);
