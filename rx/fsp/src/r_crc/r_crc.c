                                      ...
                                      uint32_t                  * calculatedValue)
{
    ...
    switch (p_instance_ctrl->p_cfg->polynomial)
    {
        ...
        case CRC_POLYNOMIAL_CRC_CCITT:
        {
            const uint8_t * p_data = (uint8_t *) inputBuffer;
        ...
        default:
        {
            const uint32_t * p_data = (uint32_t *) inputBuffer;
...
static fsp_err_t r_crc_open_cfg_check (crc_cfg_t const * const p_cfg)
{
    ...
    FSP_ERROR_RETURN((uint32_t) (1 << p_cfg->polynomial) & BSP_FEATURE_CRC_POLYNOMIAL_MASK, FSP_ERR_UNSUPPORTED);
