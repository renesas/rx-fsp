                                     ...
                                     ethercat_ssc_port_cfg_t const * const p_cfg)
{
    ...
    p_reg_esc->PHYOFF = p_instance_ctrl->p_cfg->address_offset;

    /* Set EEPROM size */
    p_reg_esc->ESCMR = (uint32_t) p_extend->eeprom_size;

    /* Set TX signal delay time */
    ecatdbgc =
        (uint32_t) ((p_extend->tx_signal_delay_port1 << 2) | (p_extend->tx_signal_delay_port0 << 0));
    p_reg_esc->TXSFTR = ecatdbgc;

    /* Set PHY link polarity */
    p_reg_esc->PHYLINK_b.LINKPOL0 = ETHERCAT_SSC_PORT_CFG_CAT0_LINKSTA_POL;
    p_reg_esc->PHYLINK_b.LINKPOL1 = ETHERCAT_SSC_PORT_CFG_CAT1_LINKSTA_POL;

    /* ESC Reset */
    p_reg_esc->ESCRST_b.PHYRST = ETHERCAT_SSC_PORT_RESET;
    p_reg_esc->ESCRST_b.ESCRST = ETHERCAT_SSC_PORT_RESET;

    /* Reset hold time */
    R_BSP_SoftwareDelay(p_instance_ctrl->p_cfg->reset_hold_time, BSP_DELAY_UNITS_MILLISECONDS);

    /* ESC Reset release*/
    p_reg_esc->ESCRST_b.ESCRST = ETHERCAT_SSC_PORT_RESET_RELEASE;
    ...
    p_reg_esc->ESCRST_b.PHYRST = ETHERCAT_SSC_PORT_RESET_RELEASE;
    ...
    for (i = 0; BSP_FEATURE_ESC_NUM_PORTS > i; i++)
    {
        ...
        if (NULL != p_ether_phy_instance)
        {
            ...
            err = p_ether_phy_instance->p_api->startAutoNegotiate(p_ether_phy_instance->p_ctrl);
        }
    }
...
{
    ethercat_ssc_port_instance_ctrl_t * p_instance_ctrl = (ethercat_ssc_port_instance_ctrl_t *) p_ctrl;
    R_ESC_Type * p_reg_esc = p_instance_ctrl->p_reg_esc;

    /* Configure the EtherCAT interrupt. */
    R_BSP_IrqCfgEnable(p_instance_ctrl->p_cfg->common_irq,
                       (uint32_t) p_instance_ctrl->p_cfg->common_ipl,
                       p_instance_ctrl);

    /* Set Detect type */
    p_reg_esc->ESCICR_b.SYNC0C = ETHERCAT_SSC_PORT_IRQ_TYPE_EDGE_RISE;

    /* Configure the EtherCAT SYNC0 interrupt. */
    R_BSP_IrqCfgEnable(p_instance_ctrl->p_cfg->sync0_irq, (uint32_t) p_instance_ctrl->p_cfg->sync0_ipl,
                       p_instance_ctrl);

    /* Set Detect type */
    p_reg_esc->ESCICR_b.SYNC1C = ETHERCAT_SSC_PORT_IRQ_TYPE_EDGE_RISE;

    /* Configure the EtherCAT SYNC1 interrupt. */
    R_BSP_IrqCfgEnable(p_instance_ctrl->p_cfg->sync1_irq, (uint32_t) p_instance_ctrl->p_cfg->sync1_ipl,
                       p_instance_ctrl);
...
static fsp_err_t ethercat_ssc_port_loading_eeprom (void * const p_reg_base)
{
    ...
    while (1)
    {
        if (1 == p_reg_esc->ESCDLSTATUS_b.PDIOPE)
        {
            err = FSP_SUCCESS;
            break;                                                        /* Loading successful, PDI operations */
        }

        eeprom_error_status = (uint16_t) (p_reg_esc->EEPCONTSTAT & ETHERCAT_SSC_PORT_EEPROM_MASK_STATE);
...
{
    ethercat_ssc_port_instance_ctrl_t * p_instance_ctrl = (ethercat_ssc_port_instance_ctrl_t *) p_ctrl;
    uint32_t increment_time = (uint32_t) p_instance_ctrl->sys_time + ECAT_TIMER_INC_P_MS;
