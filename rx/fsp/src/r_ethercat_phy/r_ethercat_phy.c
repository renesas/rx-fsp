...
extern void ethercat_phy_target_vsc8541_initialize(ethercat_phy_instance_ctrl_t * p_instance_ctrl);
extern bool ethercat_phy_target_vsc8541_is_support_link_partner_ability(ethercat_phy_instance_ctrl_t * p_instance_ctrl,
                                                                        uint32_t                       line_speed_duplex);

#endif

#if (ETHER_PHY_CFG_TARGET_KSZ8081MNX_ENABLE)
extern void ethercat_phy_target_ksz8081mnx_initialize(ethercat_phy_instance_ctrl_t * p_instance_ctrl);
extern bool ethercat_phy_target_ksz8081mnx_is_support_link_partner_ability(
...
fsp_err_t R_ETHERCAT_PHY_Read (ether_phy_ctrl_t * const p_ctrl, uint32_t reg_addr, uint32_t * const p_data)
{
    ...
    FSP_HARDWARE_REGISTER_WAIT(p_reg_esc->MIICONTSTAT_b.BUSY, 0U)

    if (FSP_SUCCESS == err)
    {
        /* Get PDI access right */
        p_reg_esc->MIIPDIACSSTAT_b.ACSMII = 1;

        /* Confirm that access rights switch to PDI. */
        FSP_HARDWARE_REGISTER_WAIT(p_reg_esc->MIIECATACSSTAT_b.ACSMII, 0U)
    }

    if (FSP_SUCCESS == err)
    {
        /* Read PHY register */
        p_reg_esc->PHYADR = (uint8_t) ((uint8_t) R_ESC_PHYADR_PHYADDR_Msk &
                                       (p_instance_ctrl->p_ether_phy_cfg->phy_lsi_address << R_ESC_PHYADR_PHYADDR_Pos));
        p_reg_esc->PHYREGADR = (uint8_t) (R_ESC_PHYREGADR_PHYREGADDR_Msk &
                                          (reg_addr << R_ESC_PHYREGADR_PHYREGADDR_Pos));
        p_reg_esc->MIICONTSTAT = ETHERCAT_PHY_MII_CONT_STAT_READ_COMMAND << R_ESC_MIICONTSTAT_COMMAND_Pos; // < Read command
    ...
    {
        /* Check the error bit */
        if ((p_reg_esc->MIICONTSTAT &
             (R_ESC_MIICONTSTAT_CMDERR_Msk | R_ESC_MIICONTSTAT_READERR_Msk)))
        {
            /*  Error operation */
            err = FSP_ERR_INVALID_MODE;

            /* Clear error bit */
            p_reg_esc->MIICONTSTAT_b.COMMAND = 0x0;
        }
    }
    ...
    {
        /* Get read data */
        *p_data = p_reg_esc->PHYDATA;

        /* Give the access right to ECAT */
        p_reg_esc->MIIPDIACSSTAT_b.ACSMII = 0;
    }
    else
    {
...
fsp_err_t R_ETHERCAT_PHY_Write (ether_phy_ctrl_t * const p_ctrl, uint32_t reg_addr, uint32_t data)
{
    ...
    if (FSP_SUCCESS == err)
    {
        ...
        p_reg_esc->PHYREGADR = (uint8_t) (R_ESC_PHYREGADR_PHYREGADDR_Msk &
                                          (reg_addr << R_ESC_PHYREGADR_PHYREGADDR_Pos));
        p_reg_esc->PHYDATA = (uint16_t) (R_ESC_PHYDATA_PHYREGDATA_Msk &
                                         (data << R_ESC_PHYDATA_PHYREGDATA_Pos));
        p_reg_esc->MIICONTSTAT = ETHERCAT_PHY_MII_CONT_STAT_WRITE_COMMAND << R_ESC_MIICONTSTAT_COMMAND_Pos; // < Write command
...
static void ethercat_phy_targets_initialize (ethercat_phy_instance_ctrl_t * p_instance_ctrl)
{
    switch (p_instance_ctrl->p_ether_phy_cfg->phy_lsi_type)
    {
        ...
        {
            ethercat_phy_target_vsc8541_initialize(p_instance_ctrl);
            break;
        }
#endif
        ...
        /* Use KSZ8081MNX */
#if (ETHER_PHY_CFG_TARGET_KSZ8081MNX_ENABLE)
        case ETHER_PHY_LSI_TYPE_KSZ8081MNX:
        {
            ethercat_phy_target_ksz8081mnx_initialize(p_instance_ctrl);
            break;
        }
#endif
                                                                  ...
                                                                  uint32_t                       line_speed_duplex)
{
    ...
    switch (p_instance_ctrl->p_ether_phy_cfg->phy_lsi_type)
    {
        ...
        case ETHER_PHY_LSI_TYPE_VSC8541:
        {
            result = ethercat_phy_target_vsc8541_is_support_link_partner_ability(p_instance_ctrl, line_speed_duplex);
            break;
        }
#endif
        ...
        case ETHER_PHY_LSI_TYPE_KSZ8081MNX:
        {
            result = ethercat_phy_target_ksz8081mnx_is_support_link_partner_ability(p_instance_ctrl, line_speed_duplex);
            break;
        }
#endif
