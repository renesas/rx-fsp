...
#if BSP_FEATURE_SYSC_HAS_TAMPICR1 || BSP_FEATURE_RTC_HAS_TCEN
...
{
    BSP_FEATURE_IOPORT_VBATT_PIN_INPUT_CH0, ///< Associated with TAMPICR1->CH0EN
    BSP_FEATURE_IOPORT_VBATT_PIN_INPUT_CH1, ///< Associated with TAMPICR1->CH1EN
    BSP_FEATURE_IOPORT_VBATT_PIN_INPUT_CH2  ///< Associated with TAMPICR1->CH2EN
};
#endif
 ...
 * by the Pins tab of the RX Configuration editor or manually by the developer. Different pin configurations can be
...
fsp_err_t R_IOPORT_PortRead (ioport_ctrl_t * const p_ctrl, bsp_io_port_t port, ioport_size_t * p_port_value)
{
    ...
    *p_port_value = (ioport_size_t) (p_ioport_regs->PCNTR2 & IOPORT_PRV_16BIT_MASK);
...
fsp_err_t R_IOPORT_PortWrite (ioport_ctrl_t * const p_ctrl, bsp_io_port_t port, ioport_size_t value, ioport_size_t mask)
{
    ...
    /* High bits */
    setbits = (ioport_size_t) (value & mask);
 ...
 * @retval FSP_ERR_NOT_OPEN             The module has not been open
 * @retval FSP_ERR_ASSERTION            NULL pointer
 *
...
fsp_err_t R_IOPORT_PinWrite (ioport_ctrl_t * const p_ctrl, bsp_io_port_pin_t pin, bsp_io_level_t level)
{
    ...
    ioport_size_t shift    = (ioport_size_t) (IOPORT_PRV_PIN_BITS & (ioport_size_t) pin);
...
fsp_err_t R_IOPORT_PortEventInputRead (ioport_ctrl_t * const p_ctrl, bsp_io_port_t port, ioport_size_t * p_event_data)
{
    ...
    FSP_ERROR_RETURN((BSP_FEATURE_IOPORT_ELC_PORTS_MASK & (uint32_t) (1 << port_number)), FSP_ERR_INVALID_ARGUMENT);
 #else
    FSP_PARAMETER_NOT_USED(p_ctrl);
 #endif
    ...
    *p_event_data = (ioport_size_t) p_ioport_regs->PCNTR2_b.EIDR;
...
fsp_err_t R_IOPORT_PinEventInputRead (ioport_ctrl_t * const p_ctrl, bsp_io_port_pin_t pin, bsp_io_level_t * p_pin_event)
{
    ...
    /* Read current value of EIDR value from PCNTR2 register for the specified port */
    portvalue = (ioport_size_t) p_ioport_regs->PCNTR2_b.EIDR;
                                         ...
                                         ioport_size_t         mask_value)
{
    ...
    set_bits = (ioport_size_t) (event_data & mask_value);
...
static void r_ioport_pfs_write (bsp_io_port_pin_t pin, uint32_t value)
{
    /* PMR bits should be cleared before specifying PSEL. Reference section "22.6.1 Procedure for Specifying the Pin Functions"
     * in the RX74M manual R01UH1055JJ. */
...
/*******************************************************************************************************************//**
 * @brief Initializes TAMPICR1 register based on pin configuration.
 *
 * The TAMPICR1 register may need to be modified based on the project's pin configuration. There is a set of pins that
 * needs to be checked. If one of these pins is found in the pin configuration table then it will be tested to see if
 * the appropriate TAMPICR1 bit needs to be set or cleared. If one of the pins that is being searched for is not found
 * then the accompanying TAMPICR1 bit is left as-is.
 **********************************************************************************************************************/
static void bsp_vbatt_init (ioport_cfg_t const * const p_pin_cfg)
{
    ...
    for (pin_index = 0U; pin_index < p_pin_cfg->number_of_pins; pin_index++)
    {
             ...
             vbatt_index++)
        {
            if (p_pin_cfg->p_pin_cfg_data[pin_index].pin == g_vbatt_pins_input[vbatt_index])
            {
                ...
                /* Check if pin is being used for RTC use. */
                if (IOPORT_PERIPHERAL_RTC == pfs_psel_value)
                {
                    /* Bit should be set to 1. */
 #if BSP_FEATURE_SYSC_HAS_TAMPICR1
                    if (0 == (p_system->TAMPICR1 & (uint8_t) (1U << vbatt_index)))
                    {
                        R_BSP_RegisterProtectDisable(BSP_REG_PROTECT_OM_LPC_BATT_SWR);
                        p_system->TAMPICR1 |= (uint8_t) (1U << vbatt_index);
 ...
 #if BSP_FEATURE_SYSC_HAS_TAMPICR1
                    if ((p_system->TAMPICR1 & (uint8_t) (1U << vbatt_index)) > 0)
                    {
                        R_BSP_RegisterProtectDisable(BSP_REG_PROTECT_OM_LPC_BATT_SWR);
                        p_system->TAMPICR1 &= (uint8_t) ~(1U << vbatt_index);
                        R_BSP_RegisterProtectEnable(BSP_REG_PROTECT_OM_LPC_BATT_SWR);
