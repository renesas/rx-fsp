...
#define ADC_B_FIFO_STAGE_COUNT                    (16)
...
#define R_ADC_B_ADCALSTR_ADC_2_CAL_Msk            (0x10000UL)
#define R_ADC_B_ADCALSTR_GAIN_OFFSET_2_CAL_Msk    (0x20000UL)
#define R_ADC_B_ADGSPCR_GRP_PRI_CTRL_123_Msk      (0x7)
#define R_ADC_B_ADGSPCR_GRP_PRI_CTRL_4_Msk        (0x8)

#define ADC_B_DATA_BITS_LENGTH                    (32U)
#define ADC_B_PRV_BIT_MASK_VALUE                  (0x80000000U)
...
static uint32_t adc_b_reverse_bit_order(uint32_t val);
static uint8_t  adc_b_count_leading_zeros(uint32_t val);
...
void adc_b_adi9_isr(void);
void adc_b_adi10_isr(void);
void adc_b_adi11_isr(void);
...
void adc_b_fiforeq9_isr(void);
void adc_b_fiforeq10_isr(void);
void adc_b_fiforeq11_isr(void);
...
fsp_err_t R_ADC_B_Open (adc_ctrl_t * p_ctrl, adc_cfg_t const * const p_cfg)
{
    ...
    if (ADC_B_CLOCK_SOURCE_GPT == p_extend->clock_control_u.clock_control_bits.source_selection)
    {
        R_BSP_MODULE_START(FSP_IP_GPT, 0);
    }

    /*  Set synchronous operation period and Enable/Disable for ADC 0/1 */
    R_ADC_B->ADSYCR = p_extend->sync_operation_control_u.sync_operation_control;

    /* Configure the ADC clock. See Table "Initial setup procedure"  in the "Procedure for Setting up and Changing" section of the relevant hardware manual */
    R_ADC_B->ADCLKENR = 0x00;
    FSP_HARDWARE_REGISTER_WAIT(R_ADC_B->ADCLKSR, 0x00);
    R_ADC_B->ADCLKCR  = p_extend->clock_control_u.clock_control_data;
    R_ADC_B->ADCLKENR = 0x01;
    FSP_HARDWARE_REGISTER_WAIT(R_ADC_B->ADCLKSR, 0x01);

    /* Set ADC unit scan modes and configure Group Scan Priority for units 0, 1 and 2. */
    for (uint8_t i = 0U; i < BSP_FEATURE_ADC_B_UNIT_COUNT; i++)
    {
#if ADC_B_CFG_PARAM_CHECKING_ENABLE

        /* ADC Converter Background Continuous Mode is only valid for Hybrid mode. */
        FSP_ASSERT(p_extend->adc_b_converter_mode[i].mode != ADC_B_CONVERTER_MODE_BACKGROUND_SCAN ||
                   p_extend->adc_b_converter_mode[i].method == ADC_B_CONVERSION_METHOD_HYBRID);
#endif
        admdr_value |= ((((uint32_t) p_extend->adc_b_converter_mode[i].method << 2U) |
                         (uint32_t) p_extend->adc_b_converter_mode[i].mode) & R_ADC_B0_ADMDR_ADMD0_Msk) <<
                       (i * R_ADC_B0_ADMDR_ADMD1_Pos);

        if (ADC_B_CONVERSION_METHOD_SAR == p_extend->adc_b_converter_mode[i].method)
        {
            /* PGS, RSCN and LGRRS bit are set 1 in SAR mode. */
            adgspcr_value |= (uint32_t) R_ADC_B_ADGSPCR_GRP_PRI_CTRL_123_Msk <<
                             (i * R_ADC_B0_ADGSPCR_PGS1_Pos);
            if (ADC_B_CONVERTER_MODE_CONTINUOUS_SCAN == p_extend->adc_b_converter_mode[i].mode)
            {
                /* GRP bit is set 1 in Continuous Scan mode. */
                adgspcr_value |= (uint32_t) R_ADC_B_ADGSPCR_GRP_PRI_CTRL_4_Msk <<
                                 (i * R_ADC_B0_ADGSPCR_PGS1_Pos);
            }
        }
    }

    R_ADC_B->ADMDR   = admdr_value;
...
#if BSP_FEATURE_ADC_B_SAMPLE_AND_HOLD_UNIT456_SUPPORTED
    ...
    R_ADC_B->ADDFSR2 = p_extend->adc_filter_selection[2].bits;
    ...
    R_ADC_B->ADFIFOCR    = p_extend->fifo_enable_mask_u.fifo_enable_mask;
    R_ADC_B->ADFIFOINTCR = p_extend->fifo_interrupt_enable_mask;
    for (uint8_t i = 0U; i < BSP_FEATURE_ADC_B_GROUP_PAIR_REGISTER_COUNT; i++)
    {
        uint32_t group                = (uint32_t) i * 2U;
        uint32_t fifo_interrupt_level =
            ((uint32_t) p_extend->fifo_interrupt_level[group] & R_ADC_B0_ADFIFOINTLR0_FIFOILV0_Msk) <<
                R_ADC_B0_ADFIFOINTLR0_FIFOILV0_Pos;
        fifo_interrupt_level |=
            ((uint32_t) p_extend->fifo_interrupt_level[group + 1U] & R_ADC_B0_ADFIFOINTLR0_FIFOILV0_Msk) <<
                R_ADC_B0_ADFIFOINTLR0_FIFOILV1_Pos;

        *ADC_B_REG_ADDRESS(R_ADC_B->ADFIFOINTLR0, i, sizeof(R_ADC_B->ADFIFOINTLR0)) = fifo_interrupt_level;
    }
    ...
    R_ADC_B->ADCALINTCR = R_ADC_B0_ADCALINTCR_CALENDIE0_Msk | R_ADC_B0_ADCALINTCR_CALENDIE1_Msk |
                          R_ADC_B0_ADCALINTCR_CALENDIE2_Msk;

    /* Enable converter error interrupts */
    R_ADC_B->ADERINTCR = R_ADC_B0_ADERINTCR_ADEIE0_Msk | R_ADC_B0_ADERINTCR_ADEIE1_Msk | R_ADC_B0_ADERINTCR_ADEIE2_Msk;

    /* Enable overflow Interrupts */
    R_ADC_B->ADOVFINTCR = R_ADC_B0_ADOVFINTCR_ADOVFIE0_Msk | R_ADC_B0_ADOVFINTCR_ADOVFIE1_Msk |
                          R_ADC_B0_ADOVFINTCR_ADOVFIE2_Msk;
...
fsp_err_t R_ADC_B_ScanCfg (adc_ctrl_t * p_ctrl, void const * const p_scan_cfg)
{
    ...
    for (uint8_t i = 0U; i < BSP_FEATURE_ADC_B_START_GROUP_CONTROL_REGISTER_COUNT; i++)
    {
        uint32_t group               = (uint32_t) i * 4U;
        uint32_t converter_selection =
            ((uint32_t) p_extend->converter_selection[group] & R_ADC_B0_ADSGCR0_SGADS0_Msk) <<
                R_ADC_B0_ADSGCR0_SGADS0_Pos;
        converter_selection |=
            ((uint32_t) p_extend->converter_selection[group + 1U] & R_ADC_B0_ADSGCR0_SGADS0_Msk) <<
                R_ADC_B0_ADSGCR0_SGADS1_Pos;
        converter_selection |=
            ((uint32_t) p_extend->converter_selection[group + 2U] & R_ADC_B0_ADSGCR0_SGADS0_Msk) <<
                R_ADC_B0_ADSGCR0_SGADS2_Pos;
        converter_selection |=
            ((uint32_t) p_extend->converter_selection[group + 3U] & R_ADC_B0_ADSGCR0_SGADS0_Msk) <<
                R_ADC_B0_ADSGCR0_SGADS3_Pos;

        *ADC_B_REG_ADDRESS(R_ADC_B->ADSGCR0, i, sizeof(R_ADC_B->ADSGCR0)) = converter_selection;
    }
    ...
    {
        uint32_t group               = (uint32_t) i * 2U;
        uint32_t start_trigger_delay =
            ((uint32_t) p_extend->start_trigger_delay_table[group] & R_ADC_B0_ADTRGDLR0_TRGDLY0_Msk) <<
                R_ADC_B0_ADTRGDLR0_TRGDLY0_Pos;
        start_trigger_delay |=
            ((uint32_t) p_extend->start_trigger_delay_table[group + 1U] & R_ADC_B0_ADTRGDLR0_TRGDLY0_Msk) <<
                R_ADC_B0_ADTRGDLR0_TRGDLY1_Pos;

        *ADC_B_REG_ADDRESS(R_ADC_B->ADTRGDLR0, i, sizeof(R_ADC_B->ADTRGDLR0)) = start_trigger_delay;
    }

    /* Configure sampling state tables */
    for (uint8_t i = 0; i < 8; i++)
    {
        uint32_t table_index    = (uint32_t) i * 2U;
        uint32_t sampling_state =
            ((uint32_t) p_extend->sampling_state_table[table_index] & R_ADC_B0_ADSSTR0_SST0_Msk) <<
                R_ADC_B0_ADSSTR0_SST0_Pos;
        sampling_state |=
            ((uint32_t) p_extend->sampling_state_table[table_index + 1U] & R_ADC_B0_ADSSTR0_SST0_Msk) <<
                R_ADC_B0_ADSSTR0_SST1_Pos;

        *ADC_B_REG_ADDRESS(R_ADC_B->ADSSTR0, i, sizeof(R_ADC_B->ADSSTR0)) = sampling_state;
    }

#if ADC_B_CFG_PARAM_CHECKING_ENABLE
    uint32_t adsycyc = (p_extend->sync_operation_control_u.sync_operation_control & R_ADC_B0_ADSYCR_ADSYCYC_Msk) >>
    ...
    uint32_t cst2     = (adcnvstr & R_ADC_B0_ADCNVSTR_CST2_Msk) >> R_ADC_B0_ADCNVSTR_CST2_Pos;

    /*================================================================================================================================
     *  Constraints to check while Synchronous Mode is enabled
     *   - See section Restrictions on Synchronous Operation, in the Synchronous Operation section of the relevant hardware manual.
     *================================================================================================================================*/
    bool sync_operation_enabled =
        (bool) (!p_extend->sync_operation_control_u.sync_operation_control_bits.adc_0_disable_sync ||
                !p_extend->sync_operation_control_u.sync_operation_control_bits.
                adc_1_disable_sync ||
                !p_extend->sync_operation_control_u.sync_operation_control_bits.
                adc_2_disable_sync);
    ...
    for (uint8_t group = 0; group < p_scan_data->group_count; group++)
    {
        ...
        uint32_t        cst                 = ((converter_selection == 0) ? cst0 :
                                               (converter_selection == 1) ? cst1 : cst2);
        if (sync_operation_enabled)
        {
            /* Set the Synchronous Operation Period to a value larger than successive approximation time of ADCm (m = 0, 1, 2) */
        ...
        *ADC_B_REG_ADDRESS(R_ADC_B->ADSGDCR0, p_adc_group->scan_group_id, 0x04)  = p_adc_group->self_diagnosis_mask;
        ...
        for (uint8_t channel = 0; channel < p_adc_group->virtual_channel_count; channel++)
        {
            ...
            adc_channel_t channel_id =
                (adc_channel_t) p_virtual_channel_cfg->channel_cfg_u.channel_cfg_bits.channel;
            adc_b_channel_mask_t extended_mask = 1ULL <<
                                                 (channel_id - ADC_CHANNEL_FIRST_INTERNAL +
                                                  ADC_B_CHANNEL_MASK_EXT_OFFSET);
            adc_b_channel_mask_t physical_mask = 1ULL << channel_id;
            adc_b_channel_mask_t channel_mask  =
            ...
            if (sync_operation_enabled)
            {
                /* Set so that the sum of the sampling times of the analog channels and successive approximation time of ADCm (m = 0, 1, 2) is an integral multiple of the Synchronous Operation Period */
                ...
                if (adc_channel_is_sample_hold_enabled(an_channel_id, p_extend->sample_and_hold_enable_mask))
                {
                    ...
                    uint8_t  calshhst  = (uint8_t) ((adcalshcr & R_ADC_B0_ADCALSHCR_CALSHHST_Msk) >>
                    ...
                    uint8_t calshsst = (uint8_t) ((adcalshcr & R_ADC_B0_ADCALSHCR_CALSHSST_Msk) >>
            ...
            *ADC_B_REG_ADDRESS(R_ADC_B->ADCHCR0, virtual_channel_id,
                               0x10) = p_virtual_channel_cfg->channel_cfg_u.channel_cfg;
            *ADC_B_REG_ADDRESS(R_ADC_B->ADDOPCRA0, virtual_channel_id,
                               0x10) = p_virtual_channel_cfg->channel_control_a_u.channel_control_a;
            *ADC_B_REG_ADDRESS(R_ADC_B->ADDOPCRB0, virtual_channel_id,
                               0x10) = p_virtual_channel_cfg->channel_control_b_u.channel_control_b;
            *ADC_B_REG_ADDRESS(R_ADC_B->ADDOPCRC0, virtual_channel_id,
                               0x10) = p_virtual_channel_cfg->channel_control_c_u.channel_control_c;

            /* Enable temperature sensor if configured. */
            uint32_t channel_selection =
                (p_virtual_channel_cfg->channel_cfg_u.channel_cfg & R_ADC_B0_ADCHCR0_CNVCS_Msk) >>
            ...
            switch (channel_selection)
            {
                case ADC_CHANNEL_TEMPERATURE:
                {
                    if (!R_TSN_CTRL->TSCR_b.TSEN)
                    {
                        ...
                        /* Follow procedure listed in in Figure "Setting procedure for temperature monitor sensor" in the Resets section of the relevant hardware manual
                        ...
                        {
                            R_BSP_RegisterProtectDisable(BSP_REG_PROTECT_RST); // Enable TEMPRLR and TEMPRCR
                            R_SYSTEM->TEMPRLR_b.LOCK    = 0;                   // Unlock TEMPRCR
                            R_SYSTEM->TEMPRCR_b.TSNKEEP = 1;                   // Enable TSN
                            R_SYSTEM->TEMPRLR_b.LOCK    = 1;                   // Lock TEMPRCR
                            R_BSP_RegisterProtectEnable(BSP_REG_PROTECT_RST);  // Disable TEMPRLR and TEMPRCR
                        }
#endif
                        R_TSN_CTRL->TSCR_b.TSEN = 1;
                        R_BSP_SoftwareDelay(30, BSP_DELAY_UNITS_MICROSECONDS); // Wait for stabilization. See Figure "Procedure example for using the TSN" in the TSN section of the relevant hardware manual.
...
fsp_err_t R_ADC_B_StatusGet (adc_ctrl_t * p_ctrl, adc_status_t * p_status)
{
    ...
    bool calibration_in_progress = (bool) ((p_instance_ctrl->adc_state > ADC_B_CONVERTER_STATE_NONE) &&
...
fsp_err_t R_ADC_B_Read (adc_ctrl_t * p_ctrl, adc_channel_t const channel_id, uint16_t * const p_data)
{
    ...
    bool data_err = (bool) (R_ADC_B0_ADDR_ERR_Msk & register_data);
...
{
    uint8_t group_id = adc_b_count_leading_zeros(adc_b_reverse_bit_order((uint32_t) group_mask));
#if ADC_B_CFG_PARAM_CHECKING_ENABLE
    adc_b_instance_ctrl_t * p_instance_ctrl = (adc_b_instance_ctrl_t *) p_ctrl;
    FSP_ASSERT(NULL != p_instance_ctrl);
    FSP_ASSERT(NULL != p_data);
    ...
    FSP_ERROR_RETURN((group_id < ADC_B_DATA_BITS_LENGTH) && (group_mask == (adc_group_mask_t) (1U << group_id)),
                     FSP_ERR_INVALID_ARGUMENT);
#else
    FSP_PARAMETER_NOT_USED(p_ctrl);
#endif

    volatile uint32_t * p_fifo_data   = ADC_B_REG_ADDRESS(R_ADC_B->ADFIFODR0, group_id, sizeof(R_ADC_B->ADFIFODR0));
    uint32_t          * p_fifo_status = ADC_B_REG_ADDRESS(R_ADC_B->ADFIFOSR0,
                                                          group_id / 2U,
                                                          sizeof(R_ADC_B->ADFIFOSR0));
    uint32_t remaining_count = *p_fifo_status &
                               (R_ADC_B0_ADFIFOSR0_FIFOST0_Msk | R_ADC_B0_ADFIFOSR0_FIFOST1_Msk);

    remaining_count >>= ((group_id % 2) ? R_ADC_B0_ADFIFOSR0_FIFOST1_Pos : R_ADC_B0_ADFIFOSR0_FIFOST0_Pos);
    uint8_t count = (uint8_t) (ADC_B_FIFO_STAGE_COUNT - (uint8_t) remaining_count);
    p_data->count = count;
    volatile adc_b_fifo_data_t * p_data_dest = &p_data->fifo_data[0];
    for (uint8_t i = 0; i < count; i++)
    {
        // Read 32-bit data from FIFO
        uint32_t fifo_value = *p_fifo_data;

        // Extract 32-bit data into the adc_b_fifo_data_t structure
        p_data_dest[i].data             = (uint16_t) (fifo_value & R_ADC_B0_ADFIFODR0_DATA_Msk);     // Extract 16-bit data for ADFIFODRn.DATA[15:0]
        p_data_dest[i].physical_channel =
            (adc_channel_t) ((fifo_value & R_ADC_B0_ADFIFODR0_CH_Msk) >> R_ADC_B0_ADFIFODR0_CH_Pos); // Extract 7-bit data for ADFIFODRn.CH[6:0]
        p_data_dest[i].err =
            (bool) ((fifo_value & R_ADC_B0_ADFIFODR0_ERR_Msk) >> R_ADC_B0_ADFIFODR0_ERR_Pos);        // Extract 1-bit data for ADFIFODRn.ERR
    }
...
fsp_err_t R_ADC_B_Close (adc_ctrl_t * p_ctrl)
{
    ...
    /* Disable temperature sensor if configured. See Figure "Procedure example for using the TSN" in the "Procedures for Using the Temperature Sensor" section of the relevant hardware manual. */
    if (R_TSN_CTRL->TSCR_b.TSEN)
    {
...
static void adc_b_calculate_wait_time (adc_b_instance_ctrl_t * p_instance_ctrl, adc_b_scan_cfg_t * p_scan_data)
{
    ...
    {
        (bool) (!p_extend->sync_operation_control_u.sync_operation_control_bits.adc_0_disable_sync),
        (bool) (!p_extend->sync_operation_control_u.sync_operation_control_bits.adc_1_disable_sync),
        (bool) (!p_extend->sync_operation_control_u.sync_operation_control_bits.adc_2_disable_sync)
    };
    uint16_t sync_cycles = (uint16_t) (p_extend->sync_operation_control_u.sync_operation_control_bits.period_cycle);
    ...
    for (uint8_t group = 0; (group < p_scan_data->group_count) && p_instance_ctrl->cached_adtrgenr; group++)
    {
        ...
        if (sync_enabled)
        {
            calculated_cycles = trigger_delay + sync_cycles * 2U;
        }
        else
        {
...
/******************************************************************************************************************************************************************************
 * @brief Helper function - Perform ADC calibration according to Table "Self-calibration procedure" in the "Self-calibration Procedure" section of the relevant hardware manual
 * @param p_instance_ctrl - Pointer to adc_b instance configuration
    ...
    switch (p_instance_ctrl->adc_state)
    {
        ...
        case ADC_B_CONVERTER_STATE_ADC_0_CALIBRATING:
        {
            if (R_ADC_B->ADSHCR0 & ADC_B_SAMPLE_AND_HOLD_MASK_UNIT_012)
            {
                 ...
                 * - See table "Self-calibration procedure" Note 2 in the "Self-calibration Procedure" section of the relevant hardware manual */
        ...
        case ADC_B_CONVERTER_STATE_ADC_1_CALIBRATING:
        {
             ...
             * - See table "Self-calibration procedure" Note 1 in the "Self-calibration Procedure" section of the relevant hardware manual */
...
#else
            p_instance_ctrl->adc_state = ADC_B_CONVERTER_STATE_ADC_2_CALIBRATING;
            adcalstr = (uint32_t) (R_ADC_B_ADCALSTR_ADC_2_CAL_Msk | R_ADC_B_ADCALSTR_GAIN_OFFSET_2_CAL_Msk);
#endif
...
#else
        case ADC_B_CONVERTER_STATE_ADC_2_CALIBRATING:
#endif
...
static void adc_b_force_stop (adc_b_instance_ctrl_t * p_instance_ctrl)
{
    ...
    /* Perform delay according to section "Waiting Time after Disabling Trigger Input for Forced Stop Processing" in the "Start and Stop Control of A/D Conversion" section of the relevant hardware manual */
    ...
    if (R_ADC_B->ADSR)
    {
        R_ADC_B->ADSTOPR = R_ADC_B0_ADSTOPR_ADSTOP0_Msk | R_ADC_B0_ADSTOPR_ADSTOP1_Msk | R_ADC_B0_ADSTOPR_ADSTOP2_Msk;
...
static void adc_b_isr_enable (adc_b_instance_ctrl_t * p_instance_ctrl, IRQn_Type irq, uint8_t ipl)
{
    if (irq >= 0)
    {
        R_BSP_IrqCfgEnable(irq, (uint32_t) ipl, p_instance_ctrl);
    }
}
...
static bool adc_channel_is_sample_hold_enabled (adc_channel_t channel, uint8_t sample_and_hold_enable_mask)
{
    ...
    bool sh_available = (bool) (channel <= ADC_CHANNEL_11);
    bool sh_enabled   = (bool) (1 << (channel / 2) & (sample_and_hold_enable_mask >> (channel / 6)));
 #else

    /* sample and hold connected to channels in pairs from 0/1 to 4/5, corresponding to Sample and Hold IDs 0-2 */
    bool sh_available = (bool) (channel <= ADC_CHANNEL_5);
    bool sh_enabled   = (bool) (1 << (channel / 2) & sample_and_hold_enable_mask);
 #endif

    return (bool) (sh_available && sh_enabled);
}
...
void adc_b_calend0_isr (void)
{
    ...
    if (R_ADC_B->ADOVFERSR_b.ADOVFEF0)
    {
        ...
        uint32_t physical_channels_0 = R_ADC_B->ADOVFCHSR0;
        uint32_t physical_channels_1 = R_ADC_B->ADOVFCHSR1;
        uint64_t physical_mask       = ((uint64_t) physical_channels_1 << ADC_B_DATA_BITS_LENGTH) | physical_channels_0;
        ...
        channel_mask |= physical_mask | extended_mask;
    }
     ...
     * - See Register "ADCALSTR : A/D Converter Self-calibration Start Register" in the Register Descriptions section of the relevant hardware manual */
...
/***********************************************************************************************************************
 * @brief Calibration end ISR for A/D converter unit 2
 * @param  None
 * @return None
 **********************************************************************************************************************/
void adc_b_calend2_isr (void)
{
    ...
    if (R_ADC_B->ADERSR_b.ADERF2)
    {
        event |= ADC_EVENT_CONVERSION_ERROR;
    }

    if (R_ADC_B->ADOVFERSR_b.ADOVFEF2)
    {
        event |= ADC_EVENT_OVERFLOW;
        uint32_t physical_channels_0 = R_ADC_B->ADOVFCHSR0;
        uint32_t physical_channels_1 = R_ADC_B->ADOVFCHSR1;
    ...
    R_ADC_B->ADCALENDSCR = R_ADC_B0_ADCALENDSCR_CALENDC2_Msk;
    R_BSP_IrqStatusClear(irq);
...
void adc_b_limclpi_isr (void)
{
    ...
    uint32_t physical_channels_0 = R_ADC_B->ADLIMCHSR0;
    uint32_t physical_channels_1 = R_ADC_B->ADLIMCHSR1;
    uint64_t physical_mask       = ((uint64_t) physical_channels_1 << ADC_B_DATA_BITS_LENGTH) |
                                   physical_channels_0;
    uint32_t             extended_channels = R_ADC_B->ADLIMEXSR;
    uint64_t             extended_mask     = ((uint64_t) extended_channels) << ADC_B_CHANNEL_MASK_EXT_OFFSET;
    adc_b_channel_mask_t channel_mask      = physical_mask | extended_mask;
    ...
    R_ADC_B->ADLIMCHSCR0 = physical_channels_0;
    R_ADC_B->ADLIMCHSCR1 = physical_channels_1;
    R_ADC_B->ADLIMEXSCR  = extended_channels;
    R_BSP_IrqStatusClear(irq);
...
/***********************************************************************************************************************
 * @brief A/D converter error ISR for unit 2
 * @param  None
 * @return None
 **********************************************************************************************************************/
void adc_b_err2_isr (void)
{
    ...
    R_ADC_B->ADERSCR = R_ADC_B0_ADERSCR_ADERCLR2_Msk;
    R_BSP_IrqStatusClear(irq);
...
void adc_b_resovf0_isr (void)
{
    ...
    R_ADC_B->ADOVFCHSCR0 = physical_channels_0;
    R_ADC_B->ADOVFCHSCR1 = physical_channels_1;
...
/***********************************************************************************************************************
 * @brief A/D converter overflow ISR for unit 2
 * @param  None
 * @return None
 **********************************************************************************************************************/
void adc_b_resovf2_isr (void)
{
    ...
    IRQn_Type irq = adc_b_isr_handler(ADC_EVENT_OVERFLOW, ADC_GROUP_MASK_NONE, channel_mask, ADC_B_UNIT_MASK_2);

    R_ADC_B->ADOVFCHSCR0 = physical_channels_0;
    R_ADC_B->ADOVFCHSCR1 = physical_channels_1;
    R_ADC_B->ADOVFEXSCR  = extended_channels;
    R_ADC_B->ADOVFERSCR  = R_ADC_B0_ADOVFERSCR_ADOVFEC2_Msk;
    R_BSP_IrqStatusClear(irq);
...
/***********************************************************************************************************************
 * @brief A/D converter complete ISR for groups 9
 * @param  None
 * @return None
 **********************************************************************************************************************/
void adc_b_adi9_isr (void)
{
    ...
    R_ADC_B->ADSCANENDSCR = ADC_GROUP_MASK_9;
    R_BSP_IrqStatusClear(irq);
...
/***********************************************************************************************************************
 * @brief A/D converter complete ISR for groups 10
 * @param  None
 * @return None
 **********************************************************************************************************************/
void adc_b_adi10_isr (void)
{
    ...
    R_ADC_B->ADSCANENDSCR = ADC_GROUP_MASK_10;
    R_BSP_IrqStatusClear(irq);
...
/***********************************************************************************************************************
 * @brief A/D converter complete ISR for groups 11
 * @param  None
 * @return None
 **********************************************************************************************************************/
void adc_b_adi11_isr (void)
{
    ...
    R_ADC_B->ADSCANENDSCR = ADC_GROUP_MASK_11;
    R_BSP_IrqStatusClear(irq);
...
void adc_b_adi5678_isr (void)
{
    ...
    uint8_t group_id =
        adc_b_count_leading_zeros(adc_b_reverse_bit_order(ADC_B_GROUP_MASK_5678 & R_ADC_B->ADSCANENDSR));
    adc_group_mask_t group_mask =
...
/***********************************************************************************************************************
 * @brief FIFO data threshold read ISR for group 9
 * @param  None
 * @return None
 **********************************************************************************************************************/
void adc_b_fiforeq9_isr (void)
...
/***********************************************************************************************************************
 * @brief FIFO data threshold read ISR for group 10
 * @param  None
 * @return None
 **********************************************************************************************************************/
void adc_b_fiforeq10_isr (void)
...
/***********************************************************************************************************************
 * @brief FIFO data threshold read ISR for group 11
 * @param  None
 * @return None
 **********************************************************************************************************************/
void adc_b_fiforeq11_isr (void)
...
static uint32_t adc_b_reverse_bit_order (uint32_t val)
{
    ...
    for (index = 0U; index < ADC_B_DATA_BITS_LENGTH; index++)
    {
...
static uint8_t adc_b_count_leading_zeros (uint32_t val)
{
    ...
    if (val != 0)
    {
        while ((val & ADC_B_PRV_BIT_MASK_VALUE) == 0)
        {
            leading_zeros_count++;
            val <<= 1;
        }
    ...
    {
        leading_zeros_count = ADC_B_DATA_BITS_LENGTH;
    }
