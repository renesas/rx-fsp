...
/* Convert a 32-bit */
#define RM_BLOCK_MEDIA_USB_MASK_32BIT_L              (0x000000ff)
#define RM_BLOCK_MEDIA_USB_MASK_32BIT_ML             (0x0000ff00)
#define RM_BLOCK_MEDIA_USB_MASK_32BIT_MH             (0x00ff0000)
#define RM_BLOCK_MEDIA_USB_MASK_32BIT_H              (0xff000000)
...
#endif
static uint32_t rm_block_media_usb_convert_endian(uint32_t input);
 ...
 **********************************************************************************************************************/
const uint8_t g_block_media_usb_erase_data[512] BSP_ALIGN_VARIABLE(4) = {0};
...
fsp_err_t RM_BLOCK_MEDIA_USB_MediaInit (rm_block_media_ctrl_t * const p_ctrl)
{
    ...
    fsp_err_t        err   = R_USB_HMSC_StorageCommand(p_usb->p_ctrl,
                                                       (uint8_t *) p_instance_ctrl->p_read_buffer,
    ...
    p_instance_ctrl->sector_count      = rm_block_media_usb_convert_endian(p_instance_ctrl->p_read_buffer[0]) + 1;
    p_instance_ctrl->sector_size_bytes = rm_block_media_usb_convert_endian(p_instance_ctrl->p_read_buffer[1]);
                                   ...
                                   uint32_t const                num_blocks)
{
    ...
    err = R_USB_HMSC_StorageReadSector((uint16_t) p_drive, p_dest_address, block_address, (uint16_t) num_blocks);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
    ...
    {
        /* Check Detach */
        data = usb_hmsc_get_dev_sts((uint16_t) p_drive);
                                    ...
                                    uint32_t const                num_blocks)
{
    ...
    err = R_USB_HMSC_StorageWriteSector((uint16_t) p_drive, p_src_address, block_address, (uint16_t) num_blocks);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
                                    ...
                                    uint32_t const                num_blocks)
{
    ...
    {
        err =
            R_USB_HMSC_StorageWriteSector((uint16_t) p_drive, &g_block_media_usb_erase_data[0], block_address + i, 1U);
        FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
 ...
 *
 * @retval  FSP_SUCCESS          Successful CallbackSet.
...
{
    rm_block_media_usb_instance_ctrl_t * p_api_ctrl = (rm_block_media_usb_instance_ctrl_t *) p_ctrl;

#if RM_BLOCK_MEDIA_USB_CFG_PARAM_CHECKING_ENABLE
    FSP_ASSERT(p_ctrl);
    FSP_ASSERT(p_callback);
    FSP_ERROR_RETURN(RM_BLOCK_MEDIA_USB_OPEN == p_api_ctrl->open, FSP_ERR_NOT_OPEN);
#endif

    /* Store callback and context */
    p_api_ctrl->p_callback        = p_callback;
    p_api_ctrl->p_context         = p_context;
    p_api_ctrl->p_callback_memory = p_callback_memory;
...
fsp_err_t RM_BLOCK_MEDIA_USB_StatusGet (rm_block_media_ctrl_t * const p_ctrl, rm_block_media_status_t * const p_status)
{
    ...
    if (USB_STATUS_DETACH != status.device_status)
    {
        p_status->media_inserted = true;
    }
    else
    {
        p_status->media_inserted = false;
    }
...
/******************************************************************************
 * Function Name   : rm_block_media_usb_convert_endian
 * Description     : Convert a 32-bit big-endian data value into a little-endian data value, or a 32-bit
 *                   little-endian data value into a big-endian data value.
...
static uint32_t rm_block_media_usb_convert_endian (uint32_t input)
{
    ...
    output  = (input & RM_BLOCK_MEDIA_USB_MASK_32BIT_L) << 24;
    output |= (input & RM_BLOCK_MEDIA_USB_MASK_32BIT_ML) << 8;
    output |= (input & RM_BLOCK_MEDIA_USB_MASK_32BIT_MH) >> 8;
    output |= (input & RM_BLOCK_MEDIA_USB_MASK_32BIT_H) >> 24;
