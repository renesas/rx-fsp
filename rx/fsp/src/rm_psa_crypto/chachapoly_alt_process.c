                                       ...
                                       const unsigned char key[HW_SCE_CHACHA20_KEY_BYTE_SIZE])
{
    ...
    if (0 == ret)
    {
        ...
        {
#if defined(RM_PSA_CRYPTO_PLATFORM_LITTLE_ENDIAN)
            ctx->internal_key[i] = MBEDTLS_GET_UINT32_LE(p_internal_key, i << 2);
#else
            ctx->internal_key[i] = MBEDTLS_GET_UINT32_BE(p_internal_key, i << 2);
#endif
        }
    }
                                       ...
                                       size_t                     size)
{
    ...
    uint32_t indata_aad_len[1] = {(uint32_t) ctx->aad_len};  /* Fix: proper uint32_t */
