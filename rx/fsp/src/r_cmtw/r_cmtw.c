...
#define CMTW_OPEN                          (('C' << 24U) | ('M' << 16U) | ('T' << 8U) | ('W' << 0U))
...
fsp_err_t R_CMTW_Open (timer_ctrl_t * const p_ctrl, timer_cfg_t const * const p_cfg)
{
    ...
    uint32_t address_gap = (uint32_t) R_CMTW1_BASE - (uint32_t) R_CMTW0_BASE;
    p_instance_ctrl->p_reg = (R_CMTW0_Type *) (R_CMTW0_BASE + (address_gap * p_cfg->channel));
...
fsp_err_t R_CMTW_Start (timer_ctrl_t * const p_ctrl)
{
    ...
    /* Start timer */
    p_instance_ctrl->period_count_passed = false;
 ...
 *
 * @retval FSP_ERR_UNSUPPORTED      CMTW compare match is not supported.
 **********************************************************************************************************************/
...
static fsp_err_t r_cmtw_open_param_checking (cmtw_instance_ctrl_t * p_instance_ctrl, timer_cfg_t const * const p_cfg)
{
    ...
    /* Allowed dividers for PCLK are 8, 32, 128, and 512. */
...
static void r_cmtw_hardware_cfg (cmtw_instance_ctrl_t * const p_instance_ctrl, timer_cfg_t const * const p_cfg)
{
    ...
    r_cmtw_enable_irq(p_extend->compare_oc0_irq, (uint32_t) p_extend->compare_oc0_ipl, p_instance_ctrl);
    r_cmtw_enable_irq(p_extend->compare_oc1_irq, (uint32_t) p_extend->compare_oc1_ipl, p_instance_ctrl);
    ...
    r_cmtw_enable_irq(p_extend->capture_ic0_irq, (uint32_t) p_extend->capture_ic0_ipl, p_instance_ctrl);
    r_cmtw_enable_irq(p_extend->capture_ic1_irq, (uint32_t) p_extend->capture_ic1_ipl, p_instance_ctrl);
    ...
    r_cmtw_enable_irq(p_cfg->cycle_end_irq, (uint32_t) p_cfg->cycle_end_ipl, p_instance_ctrl);
...
static uint32_t r_cmtw_clock_frequency_get (R_CMTW0_Type * p_cmtw_regs)
{
    uint32_t           clock_freq_hz = R_FSP_SystemClockHzGet(BSP_FEATURE_CMTW_CLOCK);
...
void cmtw_oc0_int_isr (void)
{
    ...
    if (NULL != p_instance_ctrl->p_callback)
    {
        /* In Periodic mode, the output signal toggle every timer_cfg_t::period_counts / 2 period, and Output Compare 0 interrupt occur each time.
         * This interrupt is notified via a callback function as a TIMER_EVENT_COMPARE_A event. */
        if (TIMER_MODE_PERIODIC == p_instance_ctrl->p_cfg->mode)
        {
            /* To match the behavior of the Timer API behavior,
             * TIMER_EVENT_CYCLE_END is notified via the callback function only after the timer_cfg_t::period_counts period has elapsed. */
            if (true == p_instance_ctrl->period_count_passed)
            {
                r_cmtw_call_callback(p_instance_ctrl, TIMER_EVENT_CYCLE_END, 0);
            }
        }
    ...
    p_instance_ctrl->period_count_passed = (bool) !p_instance_ctrl->period_count_passed;
