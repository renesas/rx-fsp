...
#define IIC_B_SLAVE_PRV_ICDR_SIZE                (0xFFU)
#define IIC_B_SLAVE_CODE_10BIT                   (0xF0U)

#if IIC_B_SLAVE_CFG_DTC_ENABLE
 #define IIC_B_SLAVE_DTC_RX_TRANSFER_SETTINGS    (((uint32_t) TRANSFER_MODE_NORMAL <<                           \
 ...
 #define IIC_B_SLAVE_DTC_TX_TRANSFER_SETTINGS    (((uint32_t) TRANSFER_MODE_NORMAL <<                           \
                                                   TRANSFER_SETTINGS_MODE_BITS) |                               \
                                                  ((uint32_t) TRANSFER_SIZE_1_BYTE <<                           \
                                                   TRANSFER_SETTINGS_SIZE_BITS) |                               \
                                                  ((uint32_t) TRANSFER_ADDR_MODE_INCREMENTED <<                 \
...
#define IIC_B_SLAVE_PRV_ICSIER_INIT_MASK         (R_I3C0_ICSIER_NAKIE_Msk | R_I3C0_ICSIER_ALIE_Msk | \
                                                  R_I3C0_ICSIER_TMOIE_Msk)
#define IIC_B_SLAVE_PRV_ICSER_INIT_MASK          (R_I3C0_ICSER_NAKDE_Msk | R_I3C0_ICSER_ALE_Msk | \
                                                  R_I3C0_ICSER_TMOE_Msk | R_I3C0_ICSER_TEDE_Msk)
#define IIC_B_SLAVE_PRV_ICCSIER_INIT_MASK        (R_I3C0_ICCSIER_TIE_Msk | R_I3C0_ICCSIER_RIE_Msk)
#define IIC_B_SLAVE_PRV_ICCSER_INIT_MASK         (R_I3C0_ICCSER_TDE_Msk | R_I3C0_ICCSER_RDE_Msk)
#define IIC_B_SLAVE_PRV_ICSR2_ERR_STATUS_MASK    (R_I3C0_ICSR2_START_Msk | R_I3C0_ICSR2_STOP_Msk | \
                                                  R_I3C0_ICSR2_NACKF_Msk | R_I3C0_ICSR2_AL_Msk |   \
                                                  R_I3C0_ICSR2_TMOF_Msk)
 ...
 * See Table "Access cycles for non-GPT modules (1 of 2)" in the I/O Registers section of the relevant hardware manual.
 */
...
fsp_err_t R_IIC_B_SLAVE_Open (i2c_slave_ctrl_t * const p_api_ctrl, i2c_slave_cfg_t const * const p_cfg)
{
    ...
    FSP_ERROR_RETURN(BSP_FEATURE_IIC_B_VALID_CHANNEL_MASK & (1U << p_cfg->channel), FSP_ERR_IP_CHANNEL_NOT_PRESENT);
    ...
    R_BSP_IrqCfgEnable(p_ctrl->p_cfg->eri_irq, (uint32_t) p_ctrl->p_cfg->eri_ipl, p_ctrl);
    R_BSP_IrqCfgEnable(p_ctrl->p_cfg->txi_irq, (uint32_t) p_ctrl->p_cfg->ipl, p_ctrl);
    R_BSP_IrqCfgEnable(p_ctrl->p_cfg->tei_irq, (uint32_t) p_ctrl->p_cfg->ipl, p_ctrl);
    R_BSP_IrqCfgEnable(p_ctrl->p_cfg->rxi_irq, (uint32_t) p_ctrl->p_cfg->ipl, p_ctrl);
...
fsp_err_t R_IIC_B_SLAVE_Close (i2c_slave_ctrl_t * const p_api_ctrl)
{
    ...
    /* Set ICRCR.MRST to 1: Reset all registers and internal state; ICRCR.MRST is already 0. */
                                         ...
                                         iic_b_slave_transfer_dir_t direction)
{
    ...
    p_ctrl->p_reg->ICACKR = R_I3C0_ICACKR_ACKWP_Msk;
...
static void iic_b_slave_notify (iic_b_slave_instance_ctrl_t * const p_ctrl, i2c_slave_event_t const slave_event)
{
     ...
     */
    p_ctrl->p_reg->ICSER   = IIC_B_SLAVE_PRV_ICSER_INIT_MASK & ~R_I3C0_ICSER_TMOE_Msk;
    p_ctrl->p_reg->ICCSIER = IIC_B_SLAVE_PRV_ICCSIER_INIT_MASK;
    ...
    if (p_ctrl->transaction_completed)
    {
        ...
        uint32_t bst_status = p_ctrl->p_reg->ICSR2;
        bst_status          &= ~bst_status;
        p_ctrl->p_reg->ICSR2 = bst_status;
    }
    /*
...
static void iic_b_slave_callback_request (iic_b_slave_instance_ctrl_t * const p_ctrl, i2c_slave_event_t slave_event)
{
    ...
    p_ctrl->p_reg->ICTOR = (uint32_t) R_I3C0_ICTOR_TMOL_Msk;

    /* Enable timeout function */
    p_ctrl->p_reg->ICSER_b.TMOE   = 1U;
...
static void iic_b_open_hw_slave (iic_b_slave_instance_ctrl_t * const p_ctrl)
{
    ...
    /* Waiting for reset completion ICRCR.MRST to 0;  ICRCR.ISRST is already 0. */
    FSP_HARDWARE_REGISTER_WAIT(0U, p_ctrl->p_reg->ICRCR);

    /* Set Slave address in SAR0 and set ICSCR */
    p_ctrl->p_reg->SAR0 = (uint32_t) (p_ctrl->p_cfg->slave) |
                          (((uint32_t) ((p_ctrl->p_cfg->addr_mode) ==
                                        I2C_SLAVE_ADDR_MODE_10BIT)) << R_I3C0_SAR0_FS_Pos);

    /* Enable the slave address and general call based on user config */
    p_ctrl->p_reg->ICSCR = (uint32_t) (R_I3C0_ICSCR_SAR0E_Msk |
                                       ((uint32_t) p_ctrl->p_cfg->general_call_enable << R_I3C0_ICSCR_GCAE_Pos));
    ...
    p_ctrl->p_reg->ICICR =
        ((uint32_t) (digital_filter_stages >
                     0U ? ((digital_filter_stages - 1U) & R_I3C0_ICICR_NF_Msk) : 0U) << R_I3C0_ICICR_NF_Pos) |
        R_I3C0_ICICR_NFE_Msk;
     ...
     * 6. Bus selection: I2C by default.
     */
    p_ctrl->p_reg->ICFER = (((uint32_t) ((I2C_SLAVE_RATE_FASTPLUS == p_ctrl->p_cfg->rate) << R_I3C0_ICFER_FMPE_Pos)) |
                            R_I3C0_ICFER_MALE_Msk | R_I3C0_ICFER_NALE_Msk | R_I3C0_ICFER_SALE_Msk |
                            R_I3C0_ICFER_SCLE_Msk);
    ...
    /* Clock settings - Set ICRCCR, ICSBR and ICEBR registers: Set transfer bit rate.
    ...
    p_ctrl->p_reg->ICRCCR =
        (uint32_t) (((iic_b_slave_extended_cfg_t *) p_ctrl->p_cfg->p_extend)->clock_settings.cks_value &
                    R_I3C0_ICRCCR_CKS_Msk);

    /* ICSBR.ODHW need not be updated. */
    p_ctrl->p_reg->ICSBR =
    ...
    /* Set WAITRE bit based on user config.
     ...
     * Set the WAITRE = 1 so that the SCLn line is held low from the falling edge of the 9th clock cycle.
     ...
     *         WAITRE = 1 will be able to handle that scenario,
     ...
     *
     * Note 2: When WAITRE = 1, this driver no longer supports data reception utilizing the double buffer HW setup.
     *
     * Note 3: WAITRE bit is dont-care during transmission.
     */
    p_ctrl->p_reg->ICCSCR = (uint32_t) (p_ctrl->p_cfg->clock_stretching_enable << R_I3C0_ICCSCR_WAITRE_Pos);
     ...
     */
    p_ctrl->p_reg->ICSER = IIC_B_SLAVE_PRV_ICSER_INIT_MASK;

    /* Enable status for Transmit Data Buffer Empty and Receive Data Buffer Full */
    p_ctrl->p_reg->ICCSER = IIC_B_SLAVE_PRV_ICCSER_INIT_MASK;
...
static void iic_b_slave_initiate_transaction (iic_b_slave_instance_ctrl_t * p_ctrl, i2c_slave_event_t slave_event)
{
     ...
     */
    p_ctrl->p_reg->ICSIER = IIC_B_SLAVE_PRV_ICSIER_INIT_MASK;
    ...
    else
    {
             ...
             (p_ctrl->p_cfg->addr_mode == I2C_SLAVE_ADDR_MODE_10BIT)))
        {
            ...
            p_ctrl->p_reg->ICSR2 &= ((uint32_t) ~(R_I3C0_ICSR2_START_Msk | R_I3C0_ICSR2_STOP_Msk));
        }
         ...
         */
        p_ctrl->p_reg->ICSER  = IIC_B_SLAVE_PRV_ICSER_INIT_MASK | R_I3C0_ICSER_STDE_Msk | R_I3C0_ICSER_SPDE_Msk;
        p_ctrl->p_reg->ICSIER = IIC_B_SLAVE_PRV_ICSIER_INIT_MASK | R_I3C0_ICSIER_STIE_Msk | R_I3C0_ICSIER_SPIE_Msk;
...
static void iic_b_rxi_slave (iic_b_slave_instance_ctrl_t * p_ctrl)
{
    ...
    if (!p_ctrl->do_dummy_read)
    {
        ...
        volatile uint8_t dummy_read = (uint8_t) (p_ctrl->p_reg->ICDR & IIC_B_SLAVE_PRV_ICDR_SIZE);
        ...
        {
            p_ctrl->p_reg->ICSER  = IIC_B_SLAVE_PRV_ICSER_INIT_MASK | R_I3C0_ICSER_SPDE_Msk;
            p_ctrl->p_reg->ICSR2 &= ((uint32_t) ~(R_I3C0_ICSR2_STOP_Msk));
        }
         ...
         */
        p_ctrl->p_reg->ICSIER = R_I3C0_ICSIER_STIE_Msk | R_I3C0_ICSIER_SPIE_Msk;
    ...
    else
    {
        ...
        if (!p_ctrl->notify_request)
        {
            ...
            i2c_slave_event_t receive_callback_event =
                (R_I3C0_ICSSR_GCA_Msk ==
                 (p_ctrl->p_reg->ICSSR &
                  R_I3C0_ICSSR_GCA_Msk)) ? I2C_SLAVE_EVENT_GENERAL_CALL : I2C_SLAVE_EVENT_RX_REQUEST;
        ...
        if (IIC_B_SLAVE_TRANSFER_DIR_MASTER_WRITE_SLAVE_READ != p_ctrl->direction)
        {
             ...
             * in the IIC section of the relevant hardware manual.
             */
            p_ctrl->p_reg->ICACKR = R_I3C0_ICACKR_ACKWP_Msk | R_I3C0_ICACKR_ACKBT_Msk;
        }
        else
#endif
        {
            if (0U == p_ctrl->total)   /* Send NACK */
            {
                ...
                volatile uint32_t dummy_read = p_ctrl->p_reg->ICDR;
                FSP_PARAMETER_NOT_USED(dummy_read);
...
#endif
                {
                    ...
                    else
                    {
                        ...
                        p_ctrl->p_buff[p_ctrl->loaded++] =
                            (uint8_t) (p_ctrl->p_reg->ICDR & IIC_B_SLAVE_PRV_ICDR_SIZE);
                        ...
                        {
                            uint32_t volatile const * p_iic_b_slave_rx_buffer = &(p_ctrl->p_reg->ICDR);
...
static void iic_b_txi_slave (iic_b_slave_instance_ctrl_t * p_ctrl)
{
    ...
    else
    {
        ...
        else
        {
            ...
            p_ctrl->p_reg->ICDR = (uint32_t) p_ctrl->p_buff[p_ctrl->loaded];
            p_ctrl->loaded++;
            ...
            {
                uint32_t volatile const * p_iic_b_slave_tx_buffer = &(p_ctrl->p_reg->ICDR);
...
static void iic_b_tei_slave (iic_b_slave_instance_ctrl_t * p_ctrl)
{
    ...
    if (0 == p_ctrl->p_reg->ICACKR_b.ACKBR)
    {
    ...
    IIC_B_SLAVE_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->ICSIER_b.TEIE, 0U, timeout_count);
}
...
static void iic_b_err_slave (iic_b_slave_instance_ctrl_t * p_ctrl)
{
    uint32_t error_events = IIC_B_SLAVE_PRV_ICSR2_ERR_STATUS_MASK & p_ctrl->p_reg->ICSR2;

    /* Timeout or Arbitration loss detected */
    if ((error_events & R_I3C0_ICSR2_TMOF_Msk) || (error_events & R_I3C0_ICSR2_AL_Msk))
    {
    ...
    else if ((error_events & R_I3C0_ICSR2_START_Msk) || (error_events & R_I3C0_ICSR2_STOP_Msk))
    {
    ...
    /* NACK detected */
    else if (error_events & R_I3C0_ICSR2_NACKF_Msk)
    {
         ...
         */
        p_ctrl->p_reg->ICSIER &= (uint32_t) ~R_I3C0_ICSIER_NAKIE_Msk;
                                                 ...
                                                 iic_b_slave_transfer_dir_t  direction)
{
    ...
    {
        p_cfg->transfer_settings_u.transfer_settings_word = (uint32_t) IIC_B_SLAVE_DTC_RX_TRANSFER_SETTINGS;
    }
    else
    {
        p_cfg->transfer_settings_u.transfer_settings_word = (uint32_t) IIC_B_SLAVE_DTC_TX_TRANSFER_SETTINGS;
    }
