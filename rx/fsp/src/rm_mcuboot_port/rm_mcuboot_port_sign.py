...
with open(input_file, "rb") as f:
    ...
    if b"\x7fELF" == contents:
        ...
        elif shutil.which("rx-elf-objcopy") is not None:
            conversion_tool = "rx-elf-objcopy"
        elif shutil.which("rlink") is not None:
            conversion_tool = "rlink"
        else:
            print(
        ...
        elif "rx-elf-objcopy" in conversion_tool:
            conversion_command = [conversion_tool, "-O", "binary", input_file, temp_unsigned_binary]
        elif "rlink" in conversion_tool:
            conversion_command = [conversion_tool, "form=binary", "--output=" + temp_unsigned_binary, input_file]
        else:
            print(
                "ERROR: Conversion tool not supported. MCUBOOT_APP_BIN_CONVERTER must be objcopy, arm-none-eabi-objcopy, fromelf, ielftool, rx-elf-objcopy or rlink.",
                file=sys.stderr,
            )
            sys.exit(1)
...
sys.path.append(boot_project_root + "/rx/mcu-tools/MCUboot/scripts")

from imgtool import main

if __name__ == "__main__":
    print("python imgtool.py " + " ".join(sys.argv[1:]))
    script_path = boot_project_root + "/rx/mcu-tools/MCUboot/scripts/imgtool.py"
