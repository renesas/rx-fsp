...
#define DMAC_PRV_REG(ch)                  ((R_DMAC0_Type *) (R_DMAC0_BASE + (R_DMAC1_BASE - R_DMAC0_BASE) * \
                                                             (uint32_t) (ch)))
...
fsp_err_t R_DMAC_SoftwareStart (transfer_ctrl_t * const p_api_ctrl, transfer_start_mode_t mode)
{
    ...
    FSP_ERROR_RETURN(DMAC_SOFTWARE_TRIGGER == p_extend->irq_activation_source, FSP_ERR_UNSUPPORTED);
#endif
...
fsp_err_t R_DMAC_Close (transfer_ctrl_t * const p_api_ctrl)
{
    ...
    R_ICU->DMRSR[p_extend->channel].DMRSR = 0U;
...
static void r_dmac_prv_disable (dmac_instance_ctrl_t * p_ctrl)
{
    ...
    R_ICU->DMRSR[p_extend->channel].DMRSR = 0;

    p_ctrl->p_reg->DMCNT = 0;

    if (DMAC_SOFTWARE_TRIGGER != p_extend->irq_activation_source)
    {
        /* Configure the activation source. */
        R_ICU->DMRSR[p_extend->channel].DMRSR = (uint8_t) p_extend->irq_activation_source;
    }
...
static void r_dmac_config_transfer_info (dmac_instance_ctrl_t * p_ctrl, transfer_info_t * p_info)
{
    ...
    dmtmd |= (uint32_t) (p_info->transfer_settings_u.transfer_settings_word_b.size << DMAC_PRV_DMTMD_SZ_OFFSET);

    /* Configure source and destination address mode. */
    dmamd |=
        (uint32_t) (p_info->transfer_settings_u.transfer_settings_word_b.src_addr_mode << DMAC_PRV_DMAMD_SM_OFFSET);
    dmamd |=
        (uint32_t) (p_info->transfer_settings_u.transfer_settings_word_b.dest_addr_mode << DMAC_PRV_DMAMD_DM_OFFSET);

    /* Configure the transfer mode. */
    dmtmd |= (uint32_t) p_info->transfer_settings_u.transfer_settings_word_b.mode << DMAC_PRV_DMTMD_MD_OFFSET;
    ...
    if (NULL != p_ctrl->p_callback)
    {
         ...
         * used with Repeat-Block mode. Reference "DMINT : DMA Interrupt Setting Register"
        ...
        /* Enable the IRQ according to DMAC channels */
...
#if BSP_FEATURE_DMAC_HAS_ALL_CHANNELS_OFFSET
    p_ctrl->p_reg->DMOFR = (uint32_t) p_extend->offset;
#else
    R_DMA->DMOFR = (uint32_t) p_extend->offset;
#endif

    p_ctrl->p_reg->DMINT = (uint8_t) dmint;

    /* Clear the interrupt status flag at the beginning of each transfer when configure this register into 0
     * or Issue an interrupt to the CPU at the end of each transfer when configure this register into 1,
     * Reference "DMA Request Source Flag Control Register" of the relevant hardware manual. */
    if (DMAC_SOFTWARE_TRIGGER != p_extend->irq_activation_source)
    {
        p_ctrl->p_reg->DMCSL = (uint8_t) p_extend->interrupt_status_flag_retained;
    }
}
...
static fsp_err_t r_dmac_enable_parameter_checking (dmac_instance_ctrl_t * const p_ctrl)
{
    void const    * p_src  = (void const *) (uintptr_t) p_ctrl->p_reg->DMSAR;
    void const    * p_dest = (void const *) (uintptr_t) p_ctrl->p_reg->DMDAR;
...
void dmac_int_isr (void)
{
    ...
    if (!(irq >> 8U))
    {
        /* For DMAC channel 0 - 3 (DMAC0I, DMAC1I, DMAC2I, DMAC3I) are the edge interrupt  */
    ...
    /* Clear ESIF and DTIF flag
     * DMAC channel 4 - 7 (DMAC74I) is a level interrupt, IR flag cannot be cleared by
     * R_BSP_IrqStatusClear function. It can only be cleared after writing the DMAC status
     * register into value 0. */
    if (p_ctrl->p_reg->DMSTS_b.ESIF == 1)
    {
        p_ctrl->p_reg->DMSTS_b.ESIF = 0U;
    }

    if (p_ctrl->p_reg->DMSTS_b.DTIF == 1)
    {
        p_ctrl->p_reg->DMSTS_b.DTIF = 0U;
    }
