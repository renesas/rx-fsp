...
#define CANFD_PRV_CTR_MODE_MASK           (R_CANFD_GCR_SLPRQ_Msk + R_CANFD_GCR_MDC_Msk)
...
/** Whether or not the peripheral contains CFTISR and CFRISR registers. */
#define CANFD_PRV_CFIFO_HAS_ISTS          (!(BSP_FEATURE_CANFD_NUM_CHANNELS == 1))

#define CANFD_PRV_CFIFO_CHANNEL_OFFSET    (1U)

#define R_CANFD_CFDRM_RM_TYPE             R_CANFD_RMB_RM_Type

#define R_CANFD_PRV_RXMB_PTR(buffer)    ((volatile R_CANFD_CFDRM_RM_TYPE *) &p_reg->RMB[buffer >>      \
                                                                                        3].RM[buffer & \
                                                                                              7U])
#define CANFD_PRV_RX_FIFO_MAX             (2U)
#define CANFD_PRV_COMMON_FIFO_MAX         (1U)
#define CANFD_PRV_CFDTMIEC_LENGTH         (1)
#define CANFD_PRV_RMID_MASK               (R_CANFD_RMB_RM_HF0_ID_Msk)
#define CANFD_PRV_RMRTR_POSITION          (R_CANFD_RMB_RM_HF0_RTR_Pos)
#define CANFD_PRV_RMRTR_MASK              (R_CANFD_RMB_RM_HF0_RTR_Msk)
#define CANFD_PRV_RMIDE_POSITION          (R_CANFD_RMB_RM_HF0_IDE_Pos)
#define CANFD_PRV_RMDLC_POSITION          (R_CANFD_RMB_RM_HF1_DLC_Pos)
#define CANFD_PRV_FESR_RFEMPX_MASK        (0x3U)
#define CANFD_PRV_FESR_CFEMPX_MASK        (0x100U)

#if BSP_FEATURE_CANFD_NUM_INSTANCES > 1 || BSP_FEATURE_CANFD_SUPPORT_2_TYPES
...
#define CANFD_WORD_DATA_BITS_LENGTH        (32U)
#define CANFD_PRV_BIT_MASK_VALUE           (0x80000000U)

#if CANFD_PRV_CFIFO_HAS_ISTS
 #define CANFD_PRV_CFRISRF_Msk             (0x3FUL)
#endif

#if BSP_FEATURE_CANFD_SUPPORT_2_TYPES
 #define CANFD_PRV_AFL_CHANNEL_RULE_NUM    (CANFD_CFG_AFL_CH2_RULE_NUM)
#else
 #define CANFD_PRV_AFL_CHANNEL_RULE_NUM    (CANFD_CFG_AFL_CH0_RULE_NUM)
#endif
...
static uint32_t r_canfd_reverse_bit_order(uint32_t val);
static uint8_t  r_canfd_count_leading_zeros(uint32_t val);
...
static canfd_instance_ctrl_t * gp_ctrl[BSP_FEATURE_CANFD_NUM_INSTANCES * BSP_FEATURE_CANFD_NUM_CHANNELS] =
{
    NULL
};
...
fsp_err_t R_CANFD_Open (can_ctrl_t * const p_api_ctrl, can_cfg_t const * const p_cfg)
{
    ...
    FSP_ERROR_RETURN(NULL == gp_ctrl[0], FSP_ERR_IN_USE);
 #else
    FSP_ERROR_RETURN(channel < BSP_FEATURE_CANFD_NUM_CHANNELS * BSP_FEATURE_CANFD_NUM_INSTANCES,
                     FSP_ERR_IP_CHANNEL_NOT_PRESENT);
    ...
    FSP_ERROR_RETURN(VECTOR_NUMBER_CANFD_RFRI >= 0, FSP_ERR_CAN_INIT_FAILED);
    FSP_ERROR_RETURN(VECTOR_NUMBER_CANFD_GLEI >= 0, FSP_ERR_CAN_INIT_FAILED);
    ...
    R_CANFD_Type * p_reg =
        (R_CANFD_Type *) ((uint32_t) R_CANFD0 +
                          (channel * ((uint32_t) R_CANFD1 - (uint32_t) R_CANFD0)));
#else
    ...
    FSP_HARDWARE_REGISTER_WAIT((p_reg->GSR & R_CANFD_GSR_RAMST_Msk), 0);
...
#if BSP_FEATURE_CANFD_NUM_INSTANCES > 1
    p_reg->AFCFG = (CANFD_PRV_AFL_CHANNEL_RULE_NUM << R_CANFD_AFCFG_RN0_Pos);
#else
    p_reg->AFCFG = (CANFD_PRV_AFL_CHANNEL_RULE_NUM << R_CANFD_AFCFG_RN0_Pos) |
    ...
    p_reg->GFDCFG = CANFD_CFG_FD_PROTOCOL_EXCEPTION;
    ...
    R_BSP_IrqCfgEnable(VECTOR_NUMBER_CANFD_RFRI, (uint32_t) p_global_cfg->rx_fifo_ipl, NULL);
     ...
     * See "Common FIFO 0 Configuration Register" description in the CANFD section of the relevant hardware manual */
    p_reg->CFCR[0] = p_global_cfg->common_fifo_config[0] & ~R_CANFD_CFCR_CFE_Msk;
...
#endif
    {
        ...
        R_BSP_IrqCfgEnable(VECTOR_NUMBER_CANFD_GLEI, (uint32_t) p_global_cfg->global_err_ipl, p_ctrl);
    }

    uint32_t interlaced_channel = CANFD_INTER_CH(channel);

    /* Track ctrl struct */
    gp_ctrl[interlaced_channel] = p_ctrl;

    /* Get AFL entry and limit */
    uint32_t afl_entry = 0;
#if BSP_FEATURE_CANFD_NUM_INSTANCES > 1
    uint32_t afl_max = CANFD_PRV_AFL_CHANNEL_RULE_NUM;
    if (1U == channel)
    {
    ...
    if (1U == channel)
    {
        afl_entry += CANFD_PRV_AFL_CHANNEL_RULE_NUM;
        afl_max   += CANFD_CFG_AFL_CH1_RULE_NUM;
    }
#endif

    /* Unlock AFL */
    p_reg->AFCR = R_CANFD_AFCR_AFLWE_Msk;

    /* Write all configured AFL entries */
    R_CANFD_AFL_Type * p_afl = (R_CANFD_AFL_Type *) p_extend->p_afl;
    for ( ; afl_entry < afl_max; afl_entry++)
    {
        ...
        /* Set AFL page */
        p_reg->AFCR = (afl_entry >> 4) | R_CANFD_AFCR_AFLWE_Msk;

        /* Get pointer to current AFL rule and set it to the rule pointed to by p_afl */
        volatile R_CANFD_AFL_Type * afl = &p_reg->AFL[afl_entry & 0xF];
        *afl = *p_afl++;

        /* Set Information Label 0 to the channel being configured */
        afl->PTR0_b.IFL0 = interlaced_channel & 1U;
    }
    ...
    p_reg->CFDC[interlaced_channel].NBCR =
        (uint32_t) (((p_cfg->p_bit_timing->baud_rate_prescaler - 1) & R_CANFD_CFDC_NBCR_BRP_Msk) <<
                    R_CANFD_CFDC_NBCR_BRP_Pos) |
        ((p_cfg->p_bit_timing->time_segment_1 - 1U) << R_CANFD_CFDC_NBCR_TSEG1_Pos) |
        ((p_cfg->p_bit_timing->time_segment_2 - 1U) << R_CANFD_CFDC_NBCR_TSEG2_Pos) |
        ((p_cfg->p_bit_timing->synchronization_jump_width - 1U) << R_CANFD_CFDC_NBCR_SJW_Pos);

#if BSP_FEATURE_CANFD_FD_SUPPORT

    /* Configure data bitrate for rate switching on FD frames */
    p_reg->CFDC2[interlaced_channel].DBCR =
        (uint32_t) (((p_extend->p_data_timing->baud_rate_prescaler - 1) & R_CANFD_CFDC2_DBCR_BRP_Msk) <<
                    R_CANFD_CFDC2_DBCR_BRP_Pos) |
        ((p_extend->p_data_timing->time_segment_1 - 1U) << R_CANFD_CFDC2_DBCR_TSEG1_Pos) |
        ((p_extend->p_data_timing->time_segment_2 - 1U) << R_CANFD_CFDC2_DBCR_TSEG2_Pos) |
        ((p_extend->p_data_timing->synchronization_jump_width - 1U) << R_CANFD_CFDC2_DBCR_SJW_Pos);
    ...
    p_reg->CFDC2[interlaced_channel].FDCFG =
        ...
        R_CANFD_CFDC2_FDCFG_TESI_Msk | 1U;
#endif

    /* Write TX message buffer interrupt enable bits */
    for (uint32_t i = 0; i < CANFD_PRV_CFDTMIEC_LENGTH; i++)
    {
        p_reg->TMIER[(interlaced_channel * CANFD_PRV_CFDTMIEC_LENGTH) + i] =
            (uint32_t) (p_extend->txmb_txi_enable >> (i * CANFD_WORD_DATA_BITS_LENGTH));
    }

    /* Configure channel error interrupts */
    p_reg->CFDC[interlaced_channel].CHCR = p_extend->error_interrupts | R_CANFD_CFDC_CHCR_MDC_Msk;
...
fsp_err_t R_CANFD_Close (can_ctrl_t * const p_api_ctrl)
{
...
#endif
    {
        R_BSP_IrqDisable(VECTOR_NUMBER_CANFD_GLEI);
    ...
    /* Disable RX FIFO interrupt */
    R_BSP_IrqDisable(VECTOR_NUMBER_CANFD_RFRI);
#endif
...
fsp_err_t R_CANFD_Write (can_ctrl_t * const p_api_ctrl, uint32_t buffer, can_frame_t * const p_frame)
{
    ...
    /* CANFD has 4 TX message buffers + 1 common FIFO */
    ...
    const bool     is_cfifo           = (bool) (buffer >= (uint32_t) CANFD_TX_BUFFER_FIFO_COMMON_0);
    const uint32_t interlaced_channel = CANFD_INTER_CH(p_ctrl->p_cfg->channel);

    const uint32_t id = p_frame->id | ((uint32_t) p_frame->type << R_CANFD_TMB_HF0_RTR_Pos) |
    ...
    if (!is_cfifo)
    {
        ...
        /* Set DLC */
        p_ctrl->p_reg->TMB[buffer_idx].HF1 = (uint32_t) r_canfd_bytes_to_dlc(p_frame->data_length_code) <<
        ...
        /* Set DLC */
        p_ctrl->p_reg->TMB[buffer_idx].HF1 = (uint32_t) p_frame->data_length_code << R_CANFD_TMB_HF1_DLC_Pos;
#endif
    ...
    else
    {
        ...
        /* Set DLC. */
        p_ctrl->p_reg->CFB[buffer_idx].HF1 = (uint32_t) r_canfd_bytes_to_dlc(p_frame->data_length_code) <<
                                             R_CANFD_CFB_HF1_DLC_Pos;
        ...
        /* Set DLC. */
        p_ctrl->p_reg->TMB[buffer_idx].HF1 = (uint32_t) p_frame->data_length_code << R_CANFD_CFB_HF1_DLC_Pos;
#endif
    ...
    else
    {
        ...
        p_ctrl->p_reg->CFPCR[buffer_idx] = R_CANFD_CFPCR_CFPC_Msk;
    }
...
fsp_err_t R_CANFD_Read (can_ctrl_t * const p_api_ctrl, uint32_t buffer, can_frame_t * const p_frame)
{
    ...
    {
        not_empty = !(p_ctrl->p_reg->FESR & (1U << (buffer - CANFD_PRV_RXMB_MAX)));
    }
    else
    {
        ...
        not_empty = (~p_ctrl->p_reg->FESR & (1U << (R_CANFD_FESR_CFEMP0_Pos + cfifo_idx))) != 0;
    }
                                  ...
                                  can_test_mode_t      test_mode)
{
    ...
    if (p_ctrl->test_mode != test_mode)
    {
        ...
        chcr &= ~(R_CANFD_CFDC_CHCR_CTME_Msk | R_CANFD_CFDC_CHCR_CTMS_Msk);
        p_ctrl->p_reg->CFDC[interlaced_channel].CHCR = chcr |
                                                       ((uint32_t) test_mode << R_CANFD_CFDC_CHCR_CTME_Pos);
...
fsp_err_t R_CANFD_InfoGet (can_ctrl_t * const p_api_ctrl, can_info_t * const p_info)
{
    ...
    p_info->error_count_receive =
        (uint8_t) ((cfdcnsts & R_CANFD_CFDC_CHSR_REC_Msk) >> R_CANFD_CFDC_CHSR_REC_Pos);
    p_info->error_count_transmit =
        (uint8_t) ((cfdcnsts & R_CANFD_CFDC_CHSR_TEC_Msk) >> R_CANFD_CFDC_CHSR_TEC_Pos);
    ...
    p_info->rx_fifo_status = (~p_ctrl->p_reg->FESR) &
                             (CANFD_PRV_FESR_RFEMPX_MASK | CANFD_PRV_FESR_CFEMPX_MASK);
...
static void r_canfd_mb_read (R_CANFD_Type * p_reg, uint32_t buffer, can_frame_t * const frame)
{
    const bool is_mb    = (bool) (buffer < CANFD_PRV_RXMB_MAX);
    const bool is_cfifo = (bool) (buffer >= (uint32_t) CANFD_RX_BUFFER_FIFO_COMMON_0);

    /* Get pointer to message buffer (FIFOs use the same buffer structure) */
    volatile R_CANFD_CFDRM_RM_TYPE * mb_regs;
    if (is_mb)
    {
        mb_regs = R_CANFD_PRV_RXMB_PTR(buffer);
    }
    else if (is_cfifo)
    {
        mb_regs =
            (volatile R_CANFD_CFDRM_RM_TYPE *) &(p_reg->CFB[buffer -
                                                            (uint32_t) CANFD_RX_BUFFER_FIFO_COMMON_0]);
    }
    else
    {
        mb_regs = (volatile R_CANFD_CFDRM_RM_TYPE *) &(p_reg->RFB[buffer - CANFD_PRV_RXMB_MAX]);
    }
    ...
    frame->data_length_code = dlc_to_bytes[mb_regs->HF1 >> CANFD_PRV_RMDLC_POSITION];
    ...
    else if (is_cfifo)
    {
        ...
        p_reg->CFPCR[buffer - (uint32_t) CANFD_RX_BUFFER_FIFO_COMMON_0] = R_CANFD_CFPCR_CFPC_Msk;
    }
    else
    {
        /* Increment RX FIFO pointer */
        p_reg->RFPCR[buffer - CANFD_PRV_RXMB_MAX] = UINT8_MAX;
    }
}
...
static void r_canfd_global_error_handler (uint32_t instance)
{
    ...
    if (args.error & CANFD_ERROR_GLOBAL_MESSAGE_LOST)
    {
        ...
        args.mb_u.buffer = r_canfd_count_leading_zeros(r_canfd_reverse_bit_order(p_ctrl->p_reg->FMLSR));

        if (args.mb_u.buffer < CANFD_PRV_RX_FIFO_MAX)
        {
            p_ctrl->p_reg->RFSR[args.mb_u.buffer] &= ~R_CANFD_RFSR_LOST_Msk;
        }
        else
        {
            p_ctrl->p_reg->CFSR[0] &= ~R_CANFD_CFSR_LOST_Msk;
        }
    }
...
void canfd_error_isr (void)
{
    ...
    if (VECTOR_NUMBER_CANFD_GLEI == irq)
    {
#if BSP_FEATURE_CANFD_NUM_INSTANCES > 1
...
static void r_canfd_rx_fifo_handler (uint32_t instance)
{
    ...
    R_CANFD_Type * p_reg =
        (R_CANFD_Type *) ((uint32_t) R_CANFD0 +
                          (instance * ((uint32_t) R_CANFD1 - (uint32_t) R_CANFD0)));
#else
    ...
    uint32_t fifo = r_canfd_count_leading_zeros(r_canfd_reverse_bit_order(p_reg->RFISR));
    ...
    if (fifo < CANFD_PRV_RX_FIFO_MAX)
    {
        ...
        args.mb_u.buffer = fifo + CANFD_PRV_RXMB_MAX;
        ...
        p_reg->RFSR[fifo] &= ~R_CANFD_RFSR_RFIF_Msk;
    }

    if (!p_reg->RFISR)
    {
        /* Clear interrupt in NVIC if there are no pending RX FIFO IRQs */
        R_BSP_IrqStatusClear(VECTOR_NUMBER_CANFD_RFRI);
    }
}
...
void canfd_channel_tx_isr (void)
{
    ...
    while (*p_tisr)
    {
        ...
        {
            p_cfdtm_sts = &p_ctrl->p_reg->TMTCSR[interlaced_channel];
            args.event  = CAN_EVENT_TX_COMPLETE;
        }
        else if (tisr & R_CANFD_TISR_CFTIF0_Msk)
        {
            is_cfifo = true;
#if CANFD_PRV_CFIFO_HAS_ISTS
            p_cfdtm_sts = &p_ctrl->p_reg->CFTISR;
#endif
            args.event = (p_extend->p_global_cfg->common_fifo_config[interlaced_channel] & R_CANFD_CFCR_CFIM_Msk) ?
                         CAN_EVENT_TX_COMPLETE : CAN_EVENT_TX_FIFO_EMPTY;
        }
        else
        {
            p_cfdtm_sts = &p_ctrl->p_reg->TMTASR[interlaced_channel];
        ...
        if (!is_cfifo)
        {
            txmb = r_canfd_count_leading_zeros(r_canfd_reverse_bit_order(*p_cfdtm_sts));
            txmb =
                (txmb <
                 8) ? txmb : r_canfd_count_leading_zeros(r_canfd_reverse_bit_order(*(p_cfdtm_sts + 1))) +
                CANFD_PRV_TXMB_OFFSET;;

            /* Clear TX complete/abort flags */
            p_ctrl->p_reg->TMSR_b[txmb + (CANFD_PRV_TXMB_CHANNEL_OFFSET * interlaced_channel)].TXRF = 0;
        }
        else
        {
#if CANFD_PRV_CFIFO_HAS_ISTS
            ...
            txmb = r_canfd_count_leading_zeros(r_canfd_reverse_bit_order(*p_cfdtm_sts & cfdtm_mask));
#else
            txmb = 0;
#endif

            /* Clear the interrupt flag for Common FIFO TX. */
            p_ctrl->p_reg->CFSR[txmb] &= ~R_CANFD_CFSR_CFTIF_Msk;
...
void canfd_common_fifo_rx_isr (void)
{
    ...
    uint32_t fifo = r_canfd_count_leading_zeros(r_canfd_reverse_bit_order(p_ctrl->p_reg->CFRISR &
                                                                          CANFD_PRV_CFRISRF_Msk)) &
                    0x1FU;
#else
    uint32_t fifo = 0;
#endif
    ...
    {
        args.mb_u.buffer = fifo % (CANFD_PRV_CFIFO_CHANNEL_OFFSET + 1);
    }
    else
    {
    ...
    args.mb_u.buffer += (uint32_t) CANFD_RX_BUFFER_FIFO_COMMON_0;

    /* Read from the FIFO until it is empty */
    while (!(p_ctrl->p_reg->FESR & (1U << (R_CANFD_FESR_CFEMP0_Pos + fifo))))
    {
        /* Read and index FIFO */
        ...
        /* Set the remaining callback arguments */
        args.p_context = gp_ctrl[interlaced_channel]->p_context;
        r_canfd_call_callback(gp_ctrl[interlaced_channel], &args);
    }

    /* Clear Common FIFO RX Interrupt Flag */
    p_ctrl->p_reg->CFSR[fifo] &= ~R_CANFD_CFSR_CFRIF_Msk;
    ...
    if (!(p_ctrl->p_reg->CFSR[0] & R_CANFD_CFSR_CFRIF_Msk))
#endif
    {
        /* Clear interrupt in NVIC if there are no pending RX FIFO IRQs */
        R_BSP_IrqStatusClear(irq);
...
static void r_canfd_mode_transition (canfd_instance_ctrl_t * p_ctrl, can_operation_mode_t operation_mode)
{
    ...
    if (global_mode)
    {
        ...
        if (!(cfdgctr & R_CANFD_GSR_RSTST_Msk) && (operation_mode & CAN_OPERATION_MODE_RESET))
        {
            ...
            for (uint32_t i = 0; i < CANFD_PRV_RX_FIFO_MAX; i++)
            {
                p_ctrl->p_reg->RFCR[i] &= ~R_CANFD_RFCR_RFE_Msk;
            }

            /* Disable Common FIFOs */
            for (uint32_t i = 0; i < CANFD_PRV_COMMON_FIFO_MAX * BSP_FEATURE_CANFD_NUM_CHANNELS; i++)
            {
                p_ctrl->p_reg->CFCR[i] &= ~R_CANFD_CFCR_CFE_Msk;
            }
        }

        r_canfd_mode_ctr_set(&p_ctrl->p_reg->GCR, operation_mode);
    ...
    else
    {
        ...
        if (((chcr & R_CANFD_CFDC_CHCR_SLPRQ_Msk) && (!(CAN_OPERATION_MODE_RESET & operation_mode))) ||
            ((!(chcr & CANFD_PRV_CTR_RESET_BIT)) && (CAN_OPERATION_MODE_SLEEP == operation_mode)))
        {
            ...
            r_canfd_mode_ctr_set(&p_ctrl->p_reg->CFDC[interlaced_channel].CHCR, CAN_OPERATION_MODE_RESET);
        }

        /* Request transition to selected mode */
        r_canfd_mode_ctr_set(&p_ctrl->p_reg->CFDC[interlaced_channel].CHCR, operation_mode);
        ...
        if ((chcr & R_CANFD_CFDC_CHCR_MDC_Msk) && !(operation_mode & CAN_OPERATION_MODE_RESET))
        {
            /* Get global config */
            canfd_global_cfg_t * p_global_cfg =
                ((canfd_extended_cfg_t *) p_ctrl->p_cfg->p_extend)->p_global_cfg;
            ...
            for (uint32_t i = 0; i < CANFD_PRV_COMMON_FIFO_MAX; i++)
            {
                p_ctrl->p_reg->CFCR[ch_offset + i] |=
                    (p_global_cfg->common_fifo_config[ch_offset + i] & R_CANFD_CFCR_CFE_Msk);
            }
        }
    }

    p_ctrl->operation_mode =
        (can_operation_mode_t) (p_ctrl->p_reg->CFDC[interlaced_channel].CHCR & CANFD_PRV_CTR_MODE_MASK);
}
...
static uint32_t r_canfd_reverse_bit_order (uint32_t val)
{
    ...
    for (index = 0U; index < CANFD_WORD_DATA_BITS_LENGTH; index++)
    {
...
static uint8_t r_canfd_count_leading_zeros (uint32_t val)
{
    ...
    if (val != 0)
    {
        while ((val & CANFD_PRV_BIT_MASK_VALUE) == 0)
        {
            leading_zeros_count++;
            val <<= 1;
        }
    ...
    {
        leading_zeros_count = CANFD_WORD_DATA_BITS_LENGTH;
    }
