                                ...
                                lfs_size_t                size)
{
    ...
    memcpy(buffer,
           (uint8_t *) (uintptr_t) (p_instance_ctrl->start_address +
                                 ...
                                 lfs_size_t                size)
{
    ...
    for (uint32_t offset = 0U; offset < byte_count; offset += current_write_size)
    {
        ...
        p_local_src_address = (uint8_t *) (uintptr_t) ((uint32_t) p_src + offset);

        /* Program using underlying SPI memory driver */
        err =
            p_spi_flash_instance->p_api->write(p_spi_flash_instance->p_ctrl,
                                               p_local_src_address,
                                               (uint8_t *) (uintptr_t) p_flash_addr + offset,
                                               current_write_size);
