...
#if BSP_PERIPHERAL_ESWM_PRESENT
static void rm_netxduo_ether_switch_close(rm_netxduo_ether_instance_t * p_netxduo_ether_instance);
    ...
    switch (driver_req_ptr->nx_ip_driver_command)
    {
        case NX_LINK_INITIALIZE:
        {
            ...
            nx_ip_interface_mtu_set(driver_req_ptr->nx_ip_driver_ptr,
                                    ...
                                    (ULONG) p_netxduo_ether_instance->p_cfg->mtu);
        ...
        case NX_LINK_ENABLE:
        {
            ...
            if (TX_SUCCESS !=
                tx_semaphore_create(&p_netxduo_ether_instance->p_ctrl->ether_tx_semaphore, "ether_tx_semaphore",
                                    (ULONG) p_ether_instance->p_cfg->num_tx_descriptors))
            {
        ...
        case NX_LINK_DISABLE:
        {
            ...
            /* The switch is opened by the ether driver during NX_LINK_ENABLE, so close it here. */
            rm_netxduo_ether_switch_close(p_netxduo_ether_instance);
#endif
        ...
        case NX_LINK_RARP_SEND:
        {
            ...
            *(ethernet_frame_ptr + 3) = (interface_ptr->nx_interface_physical_address_lsw << 16);
            ...
            {
                memset((void *) (p_packet_prepend + packet_length), 0,
                       (size_t) (NX_ETHERNET_MIN_TRANSMIT_SIZE - packet_length));
            ...
            fsp_err_t err =
                p_ether_instance->p_api->write(p_ether_instance->p_ctrl,
                                               (void *) p_packet_prepend,
...
/*******************************************************************************************************************//**
 * Close the layer3 switch that the ether driver opened during NX_LINK_ENABLE.
 *
 * The switch allocates descriptor queues on every open and only resets its allocator when it is closed, so it must
 * be closed here for a subsequent open to succeed.
 **********************************************************************************************************************/
static void rm_netxduo_ether_switch_close (rm_netxduo_ether_instance_t * p_netxduo_ether_instance)
{
    ether_instance_t const    * p_ether_instance = p_netxduo_ether_instance->p_cfg->p_ether_instance;
    rmac_extended_cfg_t const * p_rmac_extend    = (rmac_extended_cfg_t const *) p_ether_instance->p_cfg->p_extend;
    bool other_channel_active = false;

    /* Both pointers are dereferenced below without any further check, and neither is guaranteed by the RMAC driver.
     * If no switch instance is configured, the driver never opened one, so there is nothing to close. */
    if ((NULL == p_rmac_extend) || (NULL == p_rmac_extend->p_ether_switch))
    {
        return;
    }

    layer3_switch_instance_ctrl_t * p_switch_ctrl =
        (layer3_switch_instance_ctrl_t *) p_rmac_extend->p_ether_switch->p_ctrl;

    /* The switch is shared by every RMAC channel that uses it. Closing it while another channel is still using it
     * would break that channel, so only close it if this is the only (or last) active channel. */
    for (uint8_t port = 0; port < BSP_FEATURE_ETHER_NUM_CHANNELS; port++)
    {
        /*
         * A switch port maps one-to-one to an RMAC channel, so p_ether_instance->p_cfg->channel is the port owned
         * by this interface. It is skipped because this is the channel being closed, so it must not be counted as
         * an active channel.
         *
         * For the remaining ports, p_port_cfg_list[port].p_callback tells whether that port is still in use: the
         * RMAC driver registers its switch interrupt callback through R_LAYER3_SWITCH_ConfigurePort() when the
         * channel link goes up, and clears it (p_callback = NULL) again in R_RMAC_Close(). A non-NULL entry
         * therefore means another RMAC channel is still open on the switch and the switch must be kept open.
         */
        if ((port != p_ether_instance->p_cfg->channel) &&
            (NULL != p_switch_ctrl->p_port_cfg_list[port].p_callback))
        {
    ...
    {
        fsp_err_t err = p_rmac_extend->p_ether_switch->p_api->close(p_rmac_extend->p_ether_switch->p_ctrl);

        if ((FSP_SUCCESS != err) && (FSP_ERR_NOT_OPEN != err))
        {
            FSP_LOG_PRINT("Failed to close Layer3 switch.");
        }
    }
}
    ...
    do
    {
        ...
        else
        {
            ...
            p_nx_packet->nx_packet_prepend_ptr =
                (UCHAR *) ((((uintptr_t) p_nx_packet->nx_packet_prepend_ptr + 3U) & ~(3U)) + 2);
#else
             ...
             */
            p_nx_packet->nx_packet_prepend_ptr =
                (UCHAR *) ((((uintptr_t) p_nx_packet->nx_packet_prepend_ptr + 31U) & ~(31U)) + 2);
#endif
        ...
        case ETHER_EVENT_LINK_ON:
        {
            ...
            for (uint32_t i = 0; i < p_ether_instance->p_cfg->num_rx_descriptors; i++)
            {
                ...
                p_rx_buffers[i]->nx_packet_prepend_ptr =
                    (UCHAR *) ((((uintptr_t) p_rx_buffers[i]->nx_packet_prepend_ptr + 3U) & ~(3U)) + 2);
 #else
                 ...
                 */
                p_rx_buffers[i]->nx_packet_prepend_ptr =
                    (UCHAR *) (((uintptr_t) p_rx_buffers[i]->nx_packet_prepend_ptr + 31U) & ~(31U));
 #endif
            ...
            _nx_ip_driver_link_status_event(p_netxduo_ether_instance->p_ctrl->p_ip,
                                            (UINT) p_netxduo_ether_instance->p_ctrl->p_interface->nx_interface_index);
            break;
        }
