...
#define CGC_PRV_SCKDIVCR_DIV_3_BITS           (0x88888888U)
  ...
  #if !(BSP_FEATURE_CGC_HAS_CPUCLK1 | BSP_FEATURE_CGC_HAS_NPUCLK)
   #define CGC_PRV_SCKDIVCR2_DIV_3_BITS       (0x8008U)
   #define CGC_PRV_SCKDIVCR2_NUM_CLOCKS       (2U)
  #else
...
static bool r_cgc_low_speed_or_voltage_mode_possible(uint32_t sckdivcr, uint8_t ostdcr);
 ...
 #if BSP_FEATURE_CGC_OSCILLATON_STOP_DETECT && BSP_FEATURE_CGC_OSTD_SUPPORT_NMI
static void r_cgc_nmi_internal_callback(bsp_grp_irq_nmi_t irq);

 #endif

 #if BSP_FEATURE_CGC_HAS_OSTDCSE || BSP_FEATURE_CGC_OSTD_SUPPORT_IRQ
...
fsp_err_t R_CGC_ClocksCfg (cgc_ctrl_t * const p_ctrl, cgc_clocks_cfg_t const * const p_clock_cfg)
{
...
#if !BSP_FEATURE_CGC_REGISTER_SET_B
 #if !BSP_FEATURE_CGC_HAS_CPUCLK
    divider_cfg.sckdivcr2_u.sckdivcr2 = 0;
 #endif
    ...
    divider_cfg.sckdivcr_u.sckdivcr_b.bclk_div = divider_cfg.sckdivcr_u.sckdivcr_b.pclkb_div;
 #endif
    ...
    divider_cfg.sckdivcr2_u.sckdivcr2_b.npuclk_div = divider_cfg.sckdivcr2_u.sckdivcr2_b.mriclk_div;
 #endif
#endif
  ...
  #if BSP_PRV_PLL2_SUPPORTED
    uint8_t start_hoco = (uint8_t) ((CGC_CLOCK_CHANGE_START == p_clock_cfg->hoco_state) ||
    ...
    bool        pll_active         = (bool) r_cgc_clock_run_state_get(CGC_CLOCK_PLL);
    if (pll_active)
    {
    ...
    if (CGC_CLOCK_CHANGE_START == p_clock_cfg->pll_state)
    {
        err =
            r_cgc_pll_parameter_check(&p_clock_cfg->pll_cfg,
                                      (bool) (CGC_CLOCK_CHANGE_START != options[p_clock_cfg->pll_cfg.source_clock]));
        FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
    }
 #endif
#endif
    ...
    if (CGC_CLOCK_CHANGE_START == p_clock_cfg->pll2_state)
    {
        err =
            r_cgc_pll_parameter_check(&p_clock_cfg->pll2_cfg,
                                      (bool) (CGC_CLOCK_CHANGE_START != options[p_clock_cfg->pll2_cfg.source_clock]));
        FSP_ERROR_RETURN(FSP_SUCCESS == err, err);
    }
 #endif
#endif
    ...
    if (g_cgc_software_wait_us[requested_system_clock] > 0U)
    {
        ...
        {
            R_BSP_SoftwareDelay((uint32_t) g_cgc_software_wait_us[requested_system_clock],
                                BSP_DELAY_UNITS_MICROSECONDS);
        }
    }

#if !BSP_FEATURE_CGC_REGISTER_SET_B
    bsp_prv_clock_set((uint32_t) requested_system_clock,
                      divider_cfg.sckdivcr_u.sckdivcr_w,
                      (uint16_t) divider_cfg.sckdivcr2_u.sckdivcr2);
#else
                                ...
                                cgc_divider_cfg_t const * const p_divider_cfg)
{
    ...
    cgc_divider_cfg_t divider_cfg;
#if !BSP_FEATURE_CGC_REGISTER_SET_B
    divider_cfg.sckdivcr_u.sckdivcr_w = p_divider_cfg->sckdivcr_u.sckdivcr_w;
 #if BSP_FEATURE_CGC_HAS_CPUCLK
    divider_cfg.sckdivcr2_u.sckdivcr2 = p_divider_cfg->sckdivcr2_u.sckdivcr2;
 #else
    divider_cfg.sckdivcr2_u.sckdivcr2 = 0;
 #endif
#else
    ...
    divider_cfg.sckdivcr2_u.sckdivcr2_b.cpuclk1_div = divider_cfg.sckdivcr2_u.sckdivcr2_b.mriclk_div;
#endif
    ...
    if (CGC_CLOCK_PLL == clock_source)
    {
        ...
        uint8_t  cpuclk_div         = (uint8_t) divider_cfg.sckdivcr2_u.sckdivcr2_b.cpuclk_div;
...
#else
    bsp_prv_clock_set((uint32_t) clock_source,
                      divider_cfg.sckdivcr_u.sckdivcr_w,
                      (uint16_t) divider_cfg.sckdivcr2_u.sckdivcr2);
#endif
...
#else
        p_divider_cfg->sckdivcr_u.sckdivcr_w = R_SYSTEM->SCKDIVCR;
 #if BSP_FEATURE_CGC_HAS_CPUCLK
        p_divider_cfg->sckdivcr2_u.sckdivcr2 = R_SYSTEM->SCKDIVCR2;
 #endif
#endif
    }
...
fsp_err_t R_CGC_OscStopDetectEnable (cgc_ctrl_t * const p_ctrl)
{
 ...
 #if BSP_FEATURE_CGC_OSTD_SUPPORT_NMI
...
static bool r_cgc_low_speed_or_voltage_mode_possible (uint32_t sckdivcr, uint8_t ostdcr)
{
 ...
 #if !BSP_FEATURE_CGC_LOW_SPEED_SUPPORT_FULL_DIVISION
...
static bool r_cgc_stabilization_check (cgc_clock_t clock, cgc_prv_clock_state_t status)
{
    ...
    return (bool) (oscsf_bit == (uint32_t) status);
}
...
static fsp_err_t r_cgc_check_config_dividers (cgc_divider_cfg_t const * const p_divider_cfg)
{
    ...
    FSP_ASSERT(p_divider_cfg->sckdivcr_u.sckdivcr_b.iclk_div <= p_divider_cfg->sckdivcr_u.sckdivcr_b.pclka_div);

    /* PCLKA frequency must be >= PCLKB, so PCLKA divider must be <= PCLKB. */
    FSP_ASSERT(p_divider_cfg->sckdivcr_u.sckdivcr_b.pclka_div <= p_divider_cfg->sckdivcr_u.sckdivcr_b.pclkb_div);

    /* PCLKD frequency must be >= PCLKA, so PCLKD divider must be <= PCLKA. */
    FSP_ASSERT(p_divider_cfg->sckdivcr_u.sckdivcr_b.pclkd_div <= p_divider_cfg->sckdivcr_u.sckdivcr_b.pclka_div);
    ...
    FSP_ASSERT(p_divider_cfg->sckdivcr_u.sckdivcr_b.iclk_div <= p_divider_cfg->sckdivcr_u.sckdivcr_b.pclkb_div);

    /* PCLKD frequency must be >= PCLKB, so PCLKD divider must be <= PCLKB. */
    FSP_ASSERT(p_divider_cfg->sckdivcr_u.sckdivcr_b.pclkd_div <= p_divider_cfg->sckdivcr_u.sckdivcr_b.pclkb_div);
 #endif

 #if BSP_FEATURE_CGC_HAS_FCLK

    /* ICLK frequency must be >= FCLK, so ICLK divider must be <= FCLK. */
    FSP_ASSERT(p_divider_cfg->sckdivcr_u.sckdivcr_b.iclk_div <= p_divider_cfg->sckdivcr_u.sckdivcr_b.fclk_div);
 #endif

 #if BSP_FEATURE_CGC_HAS_BCLK

    /* ICLK frequency must be >= BCLK, so ICLK divider must be <= BCLK. */
    FSP_ASSERT(p_divider_cfg->sckdivcr_u.sckdivcr_b.iclk_div <= p_divider_cfg->sckdivcr_u.sckdivcr_b.bclk_div);
 #endif
...
static fsp_err_t r_cgc_pll_parameter_check (cgc_pll_cfg_t const * const p_pll_cfg, bool verify_source_stable)
{
    ...
    /* PLL input divider supports only divisions of 1, 2, or 3. */
    FSP_ASSERT((CGC_PLL_DIV_1 == p_pll_cfg->divider) ||
               (CGC_PLL_DIV_2 == p_pll_cfg->divider) ||
               (CGC_PLL_DIV_3 == p_pll_cfg->divider));
                                         ...
                                         uint32_t * const            p_pll_hz)
{
    ...
    if (fractional < 3U)
    {
        ...
        fractional = (uint8_t) (fractional * CGC_PRV_PLL_MUL_NF_COEFF);
    }
    else
    {
   ...
   #if (BSP_FEATURE_CGC_PLL_OUT_MAX_HZ == BSP_FEATURE_CGC_PLLP_OUT_MAX_HZ) && \
    (BSP_FEATURE_CGC_PLLP_OUT_MAX_HZ == BSP_FEATURE_CGC_PLLQ_OUT_MAX_HZ) &&   \
    (BSP_FEATURE_CGC_PLLQ_OUT_MAX_HZ == BSP_FEATURE_CGC_PLLR_OUT_MAX_HZ)
   ...
   #else
    uint32_t pllp_out_limit_hz = BSP_FEATURE_CGC_PLLP_OUT_MAX_HZ;
    uint32_t pllq_out_limit_hz = BSP_FEATURE_CGC_PLLQ_OUT_MAX_HZ;
    uint32_t pllr_out_limit_hz = BSP_FEATURE_CGC_PLLR_OUT_MAX_HZ;
    if (CGC_CLOCK_PLL2 == pll)
    {
        pllp_out_limit_hz = BSP_FEATURE_CGC_PLL2_OUT_MAX_HZ;
        pllq_out_limit_hz = BSP_FEATURE_CGC_PLL2_OUT_MAX_HZ;
        pllr_out_limit_hz = BSP_FEATURE_CGC_PLL2_OUT_MAX_HZ;
    }

    FSP_ASSERT(pllp_out_limit_hz >= p_pll_hz[0]);
    FSP_ASSERT(pllq_out_limit_hz >= p_pll_hz[1]);
    FSP_ASSERT(pllr_out_limit_hz >= p_pll_hz[2]);
   #endif
  #endif
 #elif 4U == BSP_FEATURE_CGC_PLLCCR_TYPE
    FSP_PARAMETER_NOT_USED(pll);
                           ...
                           uint32_t                    pllccr)
{
    ...
    bsp_prv_prepare_pll((uint32_t) pll, pll_hz);
...
static fsp_err_t r_cgc_open_irq_cfg (cgc_instance_ctrl_t * const p_instance_ctrl, cgc_extended_cfg_t * const p_extend)
{
    ...
    {
        R_BSP_IrqCfgEnable(p_extend->mostd_irq, (uint32_t) p_extend->mostd_ipl, p_instance_ctrl);
    }
...
static fsp_err_t r_cgc_divider_constraint_check (cgc_divider_cfg_t const * const p_divider_cfg)
{
    ...
    FSP_ASSERT(p_divider_cfg->sckdivcr2_u.sckdivcr2_b.cpuclk_div <= p_divider_cfg->sckdivcr_u.sckdivcr_b.iclk_div);

    /* If any divider is using /3, /6, /12 or /24, all must use /1, /3, /6, /12 or /24. */
    if ((0U != (p_divider_cfg->sckdivcr2_u.sckdivcr2 & CGC_PRV_SCKDIVCR2_DIV_3_BITS)) ||
        (0U != (p_divider_cfg->sckdivcr_u.sckdivcr_w & CGC_PRV_SCKDIVCR_DIV_3_BITS)))
    {
        uint32_t tmp_divider = p_divider_cfg->sckdivcr_u.sckdivcr_w;
        ...
        tmp_divider = p_divider_cfg->sckdivcr2_u.sckdivcr2;
    ...
    FSP_ASSERT(p_divider_cfg->sckdivcr2_u.sckdivcr2_b.mriclk_div <= p_divider_cfg->sckdivcr_u.sckdivcr_b.iclk_div);

    /* CPUCLK divider must be less than or equal to MRICLK divider. */
    FSP_ASSERT(p_divider_cfg->sckdivcr2_u.sckdivcr2_b.cpuclk_div <= p_divider_cfg->sckdivcr2_u.sckdivcr2_b.mriclk_div);
 #endif
