...
static uint32_t g_usb_hmsc_data[USB_NUM_USBIP][USB_VALUE_512 / sizeof(uint32_t)]; /* Hi-Speed MAXPS */
 #if (BSP_CFG_RTOS == 0)
...
void usb_hmsc_strg_drive_search_act (usb_utr_t * mess)
{
    ...
    switch (g_usb_hmsc_drive_search_seq[mess->ip])
    {
        case USB_SEQ_0:                /* Get MAX_LUN */
        {
            ...
            {
                err = usb_hmsc_get_max_unit(mess,
                                            addr,
                                            (uint8_t *) &g_usb_hmsc_data[mess->ip],
                                            usb_hmsc_strg_check_result);
                if (USB_QOVR == err)
                {
        ...
        case USB_SEQ_1:                /* Check result */
        {
            ...
            USB_PRINTF1(" Unit number is %d\n", (uint8_t *) &g_usb_hmsc_data[mess->ip][0]);
...
{
    uint16_t       retval;
    static uint8_t g_usb_hmsc_rs_data[USB_VALUE_512] BSP_ALIGN_VARIABLE(4); /* Request Sense Data Buffer */
...
uint16_t usb_hmsc_strg_drive_search (usb_utr_t * ptr, uint16_t addr, usb_cb_t complete)
{
    ...
    err = usb_hmsc_get_max_unit(ptr, addr, (uint8_t *) &g_usb_hmsc_data[ptr->ip], usb_hmsc_trans_result);
    if (USB_ERROR == err)
    {
        USB_PRINTF0("### GetMaxLUN error\n");
    ...
    USB_PRINTF1(" Unit number is %d\n", (uint8_t *) &g_usb_hmsc_data[ptr->ip][0]);
    USB_PRINTF0("\nPlease wait device ready\n");
