...
usb_msc_cbw_t      g_usb_hmsc_cbw[USB_NUM_USBIP][USB_MAXSTRAGE]  BSP_ALIGN_VARIABLE(4); /* CBW headder */
usb_msc_csw_t      g_usb_hmsc_csw[USB_NUM_USBIP][USB_MAXSTRAGE]  BSP_ALIGN_VARIABLE(4); /* CSW headder */
...
static void usb_hmsc_enumeration (usb_utr_t * mess)
{
    ...
    if (USB_ERROR == result)
    {
        ...
        g_usb_hmsc_init_seq[mess->ip] = USB_SEQ_0;
        usb_hstd_connect_err_event_set((uint16_t) mess->ip);
    }
}
...
static uint16_t usb_hmsc_send_cbw (usb_utr_t * ptr, uint16_t side)
{
    ...
    usb_hstd_set_pipe_info((uint16_t) ptr->ip, pipe, &g_usb_hmsc_pipe_table[side][USB_PIPE_DIR_OUT]);
...
static uint16_t usb_hmsc_get_data (usb_utr_t * ptr, uint16_t side, uint8_t * buff, uint32_t size)
{
    ...
    usb_hstd_set_pipe_info((uint16_t) ptr->ip, pipe, &g_usb_hmsc_pipe_table[side][USB_PIPE_DIR_IN]);
    usb_hstd_set_pipe_reg(ptr, pipe);
...
static void usb_hmsc_configured (usb_utr_t * ptr, uint16_t devadr, uint16_t data2)
{
    ...
    if (USB_ERROR == ret)
    {
        ...
        usb_hstd_connect_err_event_set((uint16_t) ptr->ip);
    }
...
static void usb_hmsc_detach (usb_utr_t * ptr, uint16_t addr, uint16_t data2)
{
    ...
    /* Clear pipe table. */
    usb_hstd_clr_pipe_table((uint16_t) ptr->ip, addr);
...
void usb_hmsc_drive_to_addr (uint16_t side, usb_utr_t * devadr)
{
    ...
    devadr->ipp     = usb_hstd_get_usb_ip_adr((uint16_t) devadr->ip);
}
...
void usb_hmsc_strg_cmd_complete (usb_utr_t * mess, uint16_t devadr, uint16_t data2)
{
    ...
    ctrl.device_address = (uint8_t) (usb_hstd_get_devsel(mess, (uint16_t) ctrl.pipe) >> 12);
    ctrl.data_size      = 0;
