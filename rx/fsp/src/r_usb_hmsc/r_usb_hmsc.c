...
fsp_err_t R_USB_HMSC_StorageCommand (usb_ctrl_t * const p_ctrl, uint8_t * buf, uint8_t command, uint8_t destination)
{
    ...
    /* Argument Checking */
    FSP_ASSERT(!((USB_IP0 != p_api_ctrl->module_number) && (USB_IP1 != p_api_ctrl->module_number)))
    ...
    utr.p_transfer_rx          = p_api_ctrl->p_transfer_rx;
    utr.p_transfer_tx          = p_api_ctrl->p_transfer_tx;
    ...
    err = R_USB_HMSC_DriveNumberGet(p_api_ctrl, &side, p_api_ctrl->device_address);
    FSP_ERROR_RETURN(FSP_SUCCESS == err, FSP_ERR_USB_FAILED)

    ret = usb_hmsc_strg_user_command(&utr, (uint16_t) side, (uint16_t) command, buf, usb_hmsc_strg_cmd_complete);
 #if USB_CFG_PARAM_CHECKING_ENABLE == BSP_CFG_PARAM_CHECKING_ENABLE
    ...
    if (USB_OK != ret)
    {
        ...
        {
            p_api_ctrl->status         = USB_CSW_RESULT_FAIL;
            p_api_ctrl->module_number  = utr.ip;                /* Module number setting */
            p_api_ctrl->pipe           = (uint8_t) utr.keyword; /* Pipe number setting */
            p_api_ctrl->device_address = (uint8_t) (usb_hstd_get_devsel(&utr, (uint16_t) p_api_ctrl->pipe) >> 12);
            p_api_ctrl->data_size      = 0;
  #if (BSP_CFG_RTOS == 2)
            p_api_ctrl->p_data = (void *) xTaskGetCurrentTaskHandle();
  #endif                                                            /* (BSP_CFG_RTOS == 2) */
            usb_set_event(USB_STATUS_MSC_CMD_COMPLETE, p_api_ctrl); /* Set Event(USB receive complete)  */
            err = FSP_SUCCESS;
        }
        else
        {
    ...
    else
    {
        ...
        switch (utr.result)
        {
            case USB_HMSC_OK:
            {
                p_api_ctrl->data_size = utr.tranlen;
                p_api_ctrl->status    = USB_CSW_RESULT_SUCCESS;
                break;
            }
            ...
            {
                p_api_ctrl->status = USB_CSW_RESULT_PHASE;
                break;
            }
...
fsp_err_t R_USB_HMSC_DriveNumberGet (usb_ctrl_t * const p_ctrl, uint8_t * p_drive, uint8_t destination)
{
    ...
    address = p_api_ctrl->device_address;
                                        ...
                                        uint16_t        sector_count)
{
    ...
    if (ptr.ip)
    {
 #if defined(VECTOR_NUMBER_USBHS_USBHSR)
        p_cfg = (usb_cfg_t *) R_FSP_IsrContextGet((IRQn_Type) VECTOR_NUMBER_USBHS_USBHSR);
 #endif                                /* #if defined(VECTOR_NUMBER_USBHS_USBHSR) */
    }
    else
    {
 #if defined(VECTOR_NUMBER_USB0_STATUS)
        p_cfg = (usb_cfg_t *) R_FSP_IsrContextGet((IRQn_Type) VECTOR_NUMBER_USB0_STATUS);
 #endif                                /* #if defined(VECTOR_NUMBER_USB0_STATUS) */
    }
