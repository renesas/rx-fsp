...
#define SCI_B_I2C_PRV_DTC_RX_FOR_READ_TRANSFER_SETTINGS     ((uint32_t) (TRANSFER_MODE_NORMAL <<            \
                                                             ...
                                                             (uint32_t) (TRANSFER_IRQ_END <<                \
                                                                         TRANSFER_SETTINGS_IRQ_BITS) |      \
                                                             (uint32_t) (TRANSFER_ADDR_MODE_INCREMENTED <<  \
                                                                         TRANSFER_SETTINGS_DEST_ADDR_BITS))
#define SCI_B_I2C_PRV_DTC_TX_FOR_READ_TRANSFER_SETTINGS     ((uint32_t) (TRANSFER_MODE_NORMAL <<            \
                                                                         TRANSFER_SETTINGS_MODE_BITS) |     \
                                                             (uint32_t) (TRANSFER_SIZE_1_BYTE <<            \
                                                                         TRANSFER_SETTINGS_SIZE_BITS) |     \
                                                             (uint32_t) (TRANSFER_ADDR_MODE_FIXED <<        \
                                                             ...
                                                             (uint32_t) (TRANSFER_ADDR_MODE_FIXED <<        \
                                                                         TRANSFER_SETTINGS_DEST_ADDR_BITS))
#define SCI_B_I2C_PRV_DTC_TX_FOR_WRITE_TRANSFER_SETTINGS    ((uint32_t) (TRANSFER_MODE_NORMAL <<            \
                                                                         TRANSFER_SETTINGS_MODE_BITS) |     \
                                                             (uint32_t) (TRANSFER_SIZE_1_BYTE <<            \
                                                                         TRANSFER_SETTINGS_SIZE_BITS) |     \
                                                             (uint32_t) (TRANSFER_ADDR_MODE_INCREMENTED <<  \
...
#define SCI_B_I2C_PRV_GENERATE_REQUEST(R, X)    ((R & ~(R_SCI_B0_SIMR_IICSDAS_Msk | R_SCI_B0_SIMR_IICSCLS_Msk))          \
                                                 | (1U << R_SCI_B0_SIMR_IICSDAS_Pos) | (1U << R_SCI_B0_SIMR_IICSCLS_Pos) \
                                                 | X)
...
#else
    p_ctrl->p_reg = (R_SCI_B0_Type *) p_rsci_base_address[p_cfg->channel];
#endif
...
fsp_err_t R_SCI_B_I2C_StatusGet (i2c_master_ctrl_t * const p_api_ctrl, i2c_master_status_t * p_status)
{
    ...
    p_status->open = (bool) (SCI_B_I2C_OPEN == p_ctrl->open);
...
#if BSP_FEATURE_SCI_HAS_SISSR
    FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SISSR, 0U);
#else
    FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SCR0, 0U);
#endif

    /* Set SDA and SCL to high impedance state. */
    p_ctrl->p_reg->SIMR |= R_SCI_B0_SIMR_IICSCLS_Msk | R_SCI_B0_SIMR_IICSDAS_Msk;
...
static void sci_b_i2c_open_hw_master (sci_b_i2c_instance_ctrl_t * const p_ctrl, i2c_master_cfg_t const * const p_cfg)
{
    ...
    sci_b_i2c_extended_cfg_t * p_extend = (sci_b_i2c_extended_cfg_t *) p_ctrl->p_cfg->p_extend;
     ...
     */
    uint32_t simr = R_SCI_B0_SIMR_IICSCLS_Msk | R_SCI_B0_SIMR_IICSDAS_Msk;
    simr |= (uint32_t) (p_extend->clock_settings.cycles_value & R_SCI_B0_SIMR_IICDL_Msk) <<
            R_SCI_B0_SIMR_IICDL_Pos;
    simr               |= R_SCI_B0_SIMR_IICINTM_Msk | R_SCI_B0_SIMR_IICCSC_Msk | R_SCI_B0_SIMR_IICACKT_Msk;
    p_ctrl->p_reg->SIMR = simr;
     ...
     * - Set Simple IIC Mode.
     */
    uint32_t scr3 = 2U << R_SCI_B0_SCR3_CHR_Pos;
    scr3               |= (uint32_t) p_extend->clock_settings.clock_source << R_SCI_B0_SCR3_SYNDIS_Pos;
    scr3               |= 4U << R_SCI_B0_SCR3_MOD_Pos;
     ...
     */
    uint32_t scr2 = 4U << R_SCI_B0_SCR2_BCP_Pos;
    scr2               |= (uint32_t) p_extend->clock_settings.brr_value << R_SCI_B0_SCR2_BRR_Pos;
    scr2               |= (uint32_t) p_extend->clock_settings.bitrate_modulation << R_SCI_B0_SCR2_BRME_Pos;
    scr2               |= (uint32_t) p_extend->clock_settings.clk_divisor_value << R_SCI_B0_SCR2_CKS_Pos;
    scr2               |= (uint32_t) p_extend->clock_settings.mddr_value << R_SCI_B0_SCR2_MDDR_Pos;
    p_ctrl->p_reg->SCR2 = scr2;

    /* Write settings to SCR1:
     * - Write the Noise filter setting.
     * - Enable the Noise filter.
     */
    p_ctrl->p_reg->SCR1 = (uint32_t) (p_extend->clock_settings.snfr_value << R_SCI_B0_SCR1_NFCS_Pos) |
                          R_SCI_B0_SCR1_NFEN_Msk;

    /* Clear status flags. */
    p_ctrl->p_reg->SSCR = (R_SCI_B0_SSCR_RDRFC_Msk |
                           R_SCI_B0_SSCR_AFERC_Msk |
                           R_SCI_B0_SSCR_APERC_Msk |
                           R_SCI_B0_SSCR_MFFC_Msk |
                           R_SCI_B0_SSCR_ORERC_Msk |
                           R_SCI_B0_SSCR_DPERC_Msk |
                           R_SCI_B0_SSCR_DCMFC_Msk |
                           R_SCI_B0_SSCR_ERSC_Msk);

    p_ctrl->p_reg->SISCR = R_SCI_B0_SISCR_IICSTIFC_Msk;
    ...
    /* Step #9 of writing to SCR0 is done under helper function sci_b_i2c_run_hw_master */
}
...
static void sci_b_i2c_run_hw_master (sci_b_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    p_ctrl->p_reg->SCR0 =
        (R_SCI_B0_SCR0_TE_Msk | R_SCI_B0_SCR0_RE_Msk | R_SCI_B0_SCR0_TEIE_Msk | R_SCI_B0_SCR0_TIE_Msk);

    /* If TCLK is slower than the bus clock, then wait for the internal state to be updated (See
     * "Notes regarding register access when operation clock (TCLK) is slower than bus clock (PCLK)"
     * in the SCI section of the relevant hardware manual.) */
#if BSP_FEATURE_SCI_HAS_SISSR
    FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SISSR, (R_SCI_B0_SISSR_ITEF_Msk | R_SCI_B0_SISSR_IREF_Msk));
#else
    FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SCR0_b.TE, 1U);
    FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SCR0_b.RE, 1U);
#endif
    ...
    if (!p_ctrl->restarted)
    {
         ...
         */
        p_ctrl->p_reg->SIMR = SCI_B_I2C_PRV_GENERATE_REQUEST(p_ctrl->p_reg->SIMR, R_SCI_B0_SIMR_IICSTAREQ_Msk);
    }
    else
    {
...
static void sci_b_i2c_txi_handler (sci_b_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if (0U == (p_ctrl->p_reg->SISR & R_SCI_B0_SISR_IICACKR_Msk))
    {
        ...
        if (p_ctrl->addr_total != p_ctrl->addr_loaded)
        {
            ...
            if ((p_ctrl->read) && (2U == p_ctrl->addr_loaded))
            {
                 ...
                 * - IICDL, IICINTM, IICCSC, and IICACKT settings must be preserved.
                 */
                p_ctrl->p_reg->SIMR = SCI_B_I2C_PRV_GENERATE_REQUEST(p_ctrl->p_reg->SIMR, R_SCI_B0_SIMR_IICRSTAREQ_Msk);
            }
...
static void sci_b_i2c_tei_handler (sci_b_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    else if (0U == p_ctrl->remain)
    {
        ...
        if (p_ctrl->restarted)
        {
            ...
            p_ctrl->p_reg->SIMR &= ~(R_SCI_B0_SIMR_IICSDAS_Msk | R_SCI_B0_SIMR_IICSCLS_Msk);
        }
        else
        {
            /* Set SDA and SCL to high impedance state. */
            ...
            p_ctrl->p_reg->SCR0 &= ~(R_SCI_B0_SCR0_TE_Msk | R_SCI_B0_SCR0_RE_Msk);
            ...
            FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SISSR, 0U);
#else
            FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SCR0_b.TE, 0U);
            FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SCR0_b.RE, 0U);
#endif
        }
                                               ...
                                               sci_b_i2c_dtc_interrupt_trigger_t trigger)
{
    ...
    {
        p_cfg->transfer_settings_u.transfer_settings_word = SCI_B_I2C_PRV_DTC_RX_FOR_READ_TRANSFER_SETTINGS;
    ...
    else
    {
         ...
         * (see Figure "Example of the Procedure for Master Reception Operations in Simple I2C Mode" in the relevant hardware manual.)*/

        /* In case of Write operation this will be reconfigured */
        p_cfg->transfer_settings_u.transfer_settings_word = SCI_B_I2C_PRV_DTC_TX_FOR_READ_TRANSFER_SETTINGS;
...
static void sci_b_i2c_reconfigure_interrupts_for_transfer (sci_b_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if (NULL != p_transfer_tx)
    {
        ...
        /* If TCLK is slower than the bus clock, then wait for the internal state to be updated. */
 #if BSP_FEATURE_SCI_HAS_SISSR
        FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SISSR, 0U);
 #else
        FSP_HARDWARE_REGISTER_WAIT(p_ctrl->p_reg->SCR0, 0U);
        ...
        if (p_ctrl->read)
        {
            ...
            p_info->transfer_settings_u.transfer_settings_word = SCI_B_I2C_PRV_DTC_TX_FOR_READ_TRANSFER_SETTINGS;
            p_info->p_src = (void *) &g_dummy_write_data_for_read_op;
        }
        else                           /* This is a write operation */
        {
            /* Re-adjust address modes */
            p_info->transfer_settings_u.transfer_settings_word = SCI_B_I2C_PRV_DTC_TX_FOR_WRITE_TRANSFER_SETTINGS;
        }
...
static void sci_b_i2c_txi_send_data (sci_b_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    else
    {
        ...
        if (1U == p_ctrl->remain)
        {
            p_ctrl->p_reg->SIMR_b.IICACKT = 1;
        }
    }

    /* Write 1 byte data to data register
     *( See Figure "Example of the Procedure for Master Transmission Operations in Simple I2C Mode" and
     * Example of the Procedure for Master Reception Operations in Simple I2C Mode" in the relevant hardware manual).
...
static void sci_b_i2c_tei_send_address (sci_b_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if (0U == p_ctrl->addr_loaded)
    {
        ...
        data = (uint8_t) ((1U == p_ctrl->addr_total) ? p_ctrl->addr_low : p_ctrl->addr_high);
    }
    ...
    else if ((p_ctrl->read) && (p_ctrl->addr_loaded == 2U))
    {
        ...
        data = (uint8_t) (p_ctrl->addr_high | (uint8_t) SCI_B_I2C_PRV_CODE_READ);
    }
...
static void sci_b_i2c_txi_process_nack (sci_b_i2c_instance_ctrl_t * const p_ctrl)
{
    ...
    if ((!p_ctrl->read) || (true == p_ctrl->do_dummy_read))
    {
         ...
         * - IICDL, IICINTM, IICCSC, and IICACKT settings must be preserved.
         */
        p_ctrl->p_reg->SIMR = SCI_B_I2C_PRV_GENERATE_REQUEST(p_ctrl->p_reg->SIMR, R_SCI_B0_SIMR_IICSTPREQ_Msk);
    }
...
static void sci_b_i2c_issue_restart_or_stop (sci_b_i2c_instance_ctrl_t * const p_ctrl)
{
     ...
     * - IICDL, IICINTM, IICCSC, and IICACKT settings must be preserved.
     */
    uint32_t simr = p_ctrl->p_reg->SIMR;
    simr &= ~(R_SCI_B0_SIMR_IICSDAS_Msk | R_SCI_B0_SIMR_IICSCLS_Msk);
    simr |= (1U << R_SCI_B0_SIMR_IICSDAS_Pos) | (1U << R_SCI_B0_SIMR_IICSCLS_Pos);

    if (true == p_ctrl->restart)
    {
        /* The IICRSTARREQ, IICSDAS, IICSCLS bits must be set simultaneously. */
        p_ctrl->p_reg->SIMR = (R_SCI_B0_SIMR_IICRSTAREQ_Msk | simr);
        p_ctrl->restarted   = true;
    }
    else
    {
        /* The IICRSTPREQ, IICSDAS, IICSCLS bits must be set simultaneously. */
        p_ctrl->p_reg->SIMR = (R_SCI_B0_SIMR_IICSTPREQ_Msk | simr);
    }
}
